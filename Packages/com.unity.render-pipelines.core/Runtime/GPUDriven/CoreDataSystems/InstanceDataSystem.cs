...
namespace UnityEngine.Rendering
{
    internal partial class InstanceDataSystem : IDisposable
    {
        ...
        private readonly int[] m_ScratchWindParamAddressArray = new int[
            (int)SpeedTreeWindParamIndex.MaxWindParamsCount * 4
        ];
        ...
        public InstanceDataSystem(
            int instancesCPUCapacity,
        ...
        )
        {
            ...
            m_InstanceDataBuffer = new GPUInstanceDataBuffer(
                ref m_ArchetypeManager.GetRef(),
                new GPUInstanceDataBufferLayout(1, Allocator.Temp) { { m_DefaultGPUComponents.defaultGOArchetype, 0 } },
                resources
            );
            ...
            m_RendererToInstanceMap = new NativeParallelHashMap<EntityId, InstanceHandle>(
                instancesCPUCapacity,
        ...
        private void EnsureProbeBuffersCapacity(int capacity)
        {
            ...
            if (m_ProbeUpdateDataQueueBuffer == null || m_ProbeUpdateDataQueueBuffer.count < capacity)
            {
                m_ProbeUpdateDataQueueBuffer?.Dispose();
                m_ProbeOcclusionUpdateDataQueueBuffer?.Dispose();
                m_ProbeUpdateDataQueueBuffer = new ComputeBuffer(
                    capacity,
                    UnsafeUtility.SizeOf<SHUpdatePacket>(),
                    ComputeBufferType.Structured
                );
                m_ProbeOcclusionUpdateDataQueueBuffer = new ComputeBuffer(
                    capacity,
        ...
        private void EnsureTransformBuffersCapacity(int capacity)
        {
            ...
            if (m_TransformUpdateDataQueueBuffer == null || m_TransformUpdateDataQueueBuffer.count < transformsCapacity)
            {
                m_TransformUpdateDataQueueBuffer?.Dispose();
                m_BoundingSpheresUpdateDataQueueBuffer?.Dispose();
                m_TransformUpdateDataQueueBuffer = new ComputeBuffer(
                    transformsCapacity,
                    UnsafeUtility.SizeOf<TransformUpdatePacket>(),
                    ComputeBufferType.Structured
                );
                if (m_EnableBoundingSpheres)
                    m_BoundingSpheresUpdateDataQueueBuffer = new ComputeBuffer(
                        capacity,
        ...
        private JobHandle ScheduleInterpolateProbesAndUpdateTetrahedronCache(
            int queueCount,
        ...
        )
        {
            ...
            var totalBatchCount =
                1 + (queueCount / CalculateInterpolatedLightAndOcclusionProbesBatchJob.k_CalculatedProbesPerBatch);
        ...
        )
        {
            ...
            m_InstanceDataBuffer.QueryInstanceGPUIndices(
                m_RenderWorld,
                probeInstanceQueue.GetSubArray(0, queueCount),
                gpuIndices
            );
            ...
            m_TransformUpdateCS.SetInt(
                InstanceTransformUpdateIDs._SHUpdateVec4Offset,
                m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.shCoefficients)
            );
            m_TransformUpdateCS.SetBuffer(
                m_ProbeUpdateKernel,
                InstanceTransformUpdateIDs._ProbeUpdateIndexQueue,
                m_UpdateIndexQueueBuffer
            );
            m_TransformUpdateCS.SetBuffer(
                m_ProbeUpdateKernel,
                InstanceTransformUpdateIDs._ProbeUpdateDataQueue,
                m_ProbeUpdateDataQueueBuffer
            );
            m_TransformUpdateCS.SetBuffer(
                m_ProbeUpdateKernel,
                InstanceTransformUpdateIDs._ProbeOcclusionUpdateDataQueue,
                m_ProbeOcclusionUpdateDataQueueBuffer
            );
            m_TransformUpdateCS.SetBuffer(
                m_ProbeUpdateKernel,
                InstanceTransformUpdateIDs._OutputProbeBuffer,
                m_InstanceDataBuffer.nativeBuffer
            );
        ...
        private void DispatchMotionUpdateCommand(
        ...
        )
        {
            ...
            m_InstanceDataBuffer.QueryInstanceGPUIndices(
                m_RenderWorld,
                transformInstanceQueue.GetSubArray(0, motionQueueCount),
                gpuIndices
            );
            ...
            m_TransformUpdateCS.SetInt(
                InstanceTransformUpdateIDs._TransformUpdateOutputL2WVec4Offset,
                m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.objectToWorld)
            );
            m_TransformUpdateCS.SetInt(
                InstanceTransformUpdateIDs._TransformUpdateOutputW2LVec4Offset,
                m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.worldToObject)
            );
            m_TransformUpdateCS.SetInt(
                InstanceTransformUpdateIDs._TransformUpdateOutputPrevL2WVec4Offset,
                m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.matrixPreviousM)
            );
            m_TransformUpdateCS.SetInt(
                InstanceTransformUpdateIDs._TransformUpdateOutputPrevW2LVec4Offset,
                m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.matrixPreviousMI)
            );
            m_TransformUpdateCS.SetBuffer(
                m_MotionUpdateKernel,
                InstanceTransformUpdateIDs._TransformUpdateIndexQueue,
                m_UpdateIndexQueueBuffer
            );
            m_TransformUpdateCS.SetBuffer(
                m_MotionUpdateKernel,
                InstanceTransformUpdateIDs._OutputTransformBuffer,
                m_InstanceDataBuffer.nativeBuffer
            );
        ...
        )
        {
            ...
            m_InstanceDataBuffer.QueryInstanceGPUIndices(
                m_RenderWorld,
                transformInstanceQueue.GetSubArray(0, transformQueueCount),
                gpuIndices
            );
            ...
            if (m_EnableBoundingSpheres)
                m_BoundingSpheresUpdateDataQueueBuffer.SetData(
            ...
            m_TransformUpdateCS.SetBuffer(
                kernel,
                InstanceTransformUpdateIDs._TransformUpdateDataQueue,
                m_TransformUpdateDataQueueBuffer
            );
            if (m_EnableBoundingSpheres)
            {
                Assert.IsTrue(m_DefaultGPUComponents.boundingSphere.valid);
                m_TransformUpdateCS.SetInt(
                    InstanceTransformUpdateIDs._BoundingSphereOutputVec4Offset,
                    m_InstanceDataBuffer.GetComponentGPUUIntOffset(m_DefaultGPUComponents.boundingSphere)
                );
                m_TransformUpdateCS.SetBuffer(
                    kernel,
                    InstanceTransformUpdateIDs._BoundingSphereDataQueue,
                    m_BoundingSpheresUpdateDataQueueBuffer
                );
            }
            m_TransformUpdateCS.SetBuffer(
                kernel,
        ...
        private void DispatchWindDataCopyHistoryCommand(NativeArray<GPUInstanceIndex> gpuIndices)
        {
            ...
            for (int i = 0; i < (int)SpeedTreeWindParamIndex.MaxWindParamsCount; ++i)
                m_ScratchWindParamAddressArray[i * 4] = m_InstanceDataBuffer.GetComponentGPUAddress(
                    m_DefaultGPUComponents.speedTreeWind[i]
                );
            m_WindDataUpdateCS.SetInts(
                InstanceWindDataUpdateIDs._WindParamAddressArray,
                m_ScratchWindParamAddressArray
            );
            for (int i = 0; i < (int)SpeedTreeWindParamIndex.MaxWindParamsCount; ++i)
                m_ScratchWindParamAddressArray[i * 4] = m_InstanceDataBuffer.GetComponentGPUAddress(
                    m_DefaultGPUComponents.speedTreeWindHistory[i]
                );
            m_WindDataUpdateCS.SetInts(
                InstanceWindDataUpdateIDs._WindHistoryParamAddressArray,
                m_ScratchWindParamAddressArray
            );

            m_WindDataUpdateCS.SetBuffer(
                kernel,
                InstanceWindDataUpdateIDs._WindDataUpdateIndexQueue,
                m_UpdateIndexQueueBuffer
            );
            m_WindDataUpdateCS.SetBuffer(
                kernel,
                InstanceWindDataUpdateIDs._WindDataBuffer,
                m_InstanceDataBuffer.nativeBuffer
            );
        ...
        {
            var transformUpdateInstanceQueue = new NativeArray<InstanceHandle>(
                m_RenderWorld.instanceCount,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
        ...
        )
        {
            ...
            // When we reinitialize we have the current and the previous matrices per transform.
            var transformUpdateDataQueue = new NativeArray<TransformUpdatePacket>(
                initialize ? instances.Length * 2 : instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var boundingSpheresUpdateDataQueue = new NativeArray<float4>(
                m_EnableBoundingSpheres ? instances.Length : 0,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            ...
            if (updateProbes)
            {
                probeInstanceQueue = new NativeArray<InstanceHandle>(
                    instances.Length,
                    Allocator.TempJob,
                    NativeArrayOptions.UninitializedMemory
                );
                compactTetrahedronCache = new NativeArray<int>(
                    instances.Length,
                    Allocator.TempJob,
                    NativeArrayOptions.UninitializedMemory
                );
                probeQueryPosition = new NativeArray<Vector3>(
                    instances.Length,
                    Allocator.TempJob,
                    NativeArrayOptions.UninitializedMemory
                );
                probeUpdateDataQueue = new NativeArray<SphericalHarmonicsL2>(
                    instances.Length,
                    Allocator.TempJob,
                    NativeArrayOptions.UninitializedMemory
                );
                probeOcclusionUpdateDataQueue = new NativeArray<Vector4>(
                    instances.Length,
                    Allocator.TempJob,
                    NativeArrayOptions.UninitializedMemory
                );
            ...
            var transformJobRanges = JaggedJobRange.FromSpanWithMaxBatchSize(
                jaggedLocalToWorldMatrices,
                TransformUpdateJob.MaxBatchSize,
                Allocator.TempJob
            );
            ...
            {
                InterpolateProbesAndUpdateTetrahedronCache(
                    probesQueueCount,
                    probeInstanceQueue,
                    compactTetrahedronCache,
                    probeQueryPosition,
            ...
            if (transformQueueCount > 0)
                DispatchTransformUpdateCommand(
                    initialize,
                    transformQueueCount,
                    transformUpdateInstanceQueue,
                    transformUpdateDataQueue,
                    boundingSpheresUpdateDataQueue
                );
        ...
        {
            var probeInstanceQueue = new NativeArray<InstanceHandle>(
                instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var compactTetrahedronCache = new NativeArray<int>(
                instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var probeQueryPosition = new NativeArray<Vector3>(
                instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var probeUpdateDataQueue = new NativeArray<SphericalHarmonicsL2>(
                instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
            var probeOcclusionUpdateDataQueue = new NativeArray<Vector4>(
                instances.Length,
                Allocator.TempJob,
                NativeArrayOptions.UninitializedMemory
            );
        ...
        {
            Assert.IsTrue(
                m_GPUResizingPolicy == GPUCapacityResizingPolicy.DoubleOnGrow
                    || m_GPUResizingPolicy == GPUCapacityResizingPolicy.DoubleOnGrow_HalveOnQuarterShrink,
            ...
            if (needLayoutUpdate)
            {
                ...
                m_InstanceDataBuffer.SetGPULayout(
                    m_CmdBuffer,
                    ref m_ArchetypeManager.GetRef(),
                    newLayout,
        ...
        public void AllocateNewInstances(
        ...
        )
        {
            ...
            {
                HandleInstancesAllocations(
                    InstanceAllocatorVariant.AllocOnly,
                    jaggedInstanceIDs,
                    instances,
                    archetypes,
                    newInstancesCount
        ...
        public void ReallocateExistingGPUInstances(
            NativeArray<InstanceHandle> instances,
            NativeArray<GPUArchetypeHandle> archetypes
        )
        {
            using (new ProfilerMarker("ReallocateExistingGPUInstances").Auto())
            {
                ...
                HandleInstancesAllocations(
                    InstanceAllocatorVariant.GPUReallocOnly,
                    dummyRenderers,
                    instances,
                    archetypes,
                    0
        ...
        public void AllocOrGPUReallocInstances(
            JaggedSpan<EntityId> jaggedInstanceIDs,
            NativeArray<InstanceHandle> instances,
            NativeArray<GPUArchetypeHandle> archetypes,
            int newInstancesCount
        )
        {
            ...
            {
                HandleInstancesAllocations(
                    InstanceAllocatorVariant.AllocOrGPURealloc,
                    jaggedInstanceIDs,
                    instances,
                    archetypes,
                    newInstancesCount
        ...
        private unsafe void HandleInstancesAllocations(
        ...
        )
        {
            ...
            {
                InstanceDataSystemBurst.AllocateInstances(
                    allocVariant,
                    jaggedInstanceIDs,
                    archetypes,
                    instanceAllocators,
        ...
        public unsafe void FreeInstances(NativeArray<InstanceHandle> instances)
        {
            fixed (InstanceAllocators* instanceAllocators = &m_InstanceAllocators)
            {
                InstanceDataSystemBurst.FreeInstances(
                    instances,
                    instanceAllocators,
                    ref m_RenderWorld,
                    ref m_RendererToInstanceMap
        ...
        public void UpdateInstanceData(
            ...
            NativeParallelHashMap<EntityId, GPUInstanceIndex> lodGroupDataMap
        )
        {
        ...
        public GPUInstanceUploadData CreateInstanceUploadData(
        ...
        public void InitializeInstanceTransforms(
            NativeArray<InstanceHandle> instances,
            JaggedSpan<float4x4> jaggedLocalToWorldMatrices,
            JaggedSpan<float4x4> jaggedPrevLocalToWorldMatrices,
            bool anyInstanceUseBlendProbes
        )
        {
            UpdateInstanceTransformsData(
                ...
                instances,
                jaggedLocalToWorldMatrices,
                jaggedPrevLocalToWorldMatrices,
        ...
        public JobHandle ScheduleQueryRendererInstancesJob(
            JaggedSpan<EntityId> jaggedRenderers,
            NativeArray<InstanceHandle> instances,
        ...
        public void QueryRendererInstances(
            NativeArray<EntityId> renderers,
            NativeArray<InstanceHandle> instances,
            UnsafeAtomicCounter32 notFoundInstancesCount = default
        )
        {
        ...
        public JobHandle ScheduleQuerySortedMeshInstancesJob(
            NativeArray<EntityId> sortedMeshes,
            NativeList<InstanceHandle> instances
        )
        {
        ...
        public unsafe void GetVisibleTreeInstances(
            in ParallelBitArray compactedVisibilityMasks,
            in ParallelBitArray processedBits,
            NativeList<EntityId> visibeTreeRenderers,
            NativeList<InstanceHandle> visibeTreeInstances,
            bool becomeVisibleOnly,
...
#pragma warning disable CS0162
            ...
            if (totalTreeCount != totalTreeCountTruth)
                Debug.LogError(
                    $"Cached total tree count ({totalTreeCount}) does not match total tree count truth ({totalTreeCountTruth})."
                );
...
#pragma warning restore CS0162
        ...
        private static class InstanceTransformUpdateIDs
        {
            ...
            public static readonly int _TransformUpdateOutputL2WVec4Offset = Shader.PropertyToID(
                "_TransformUpdateOutputL2WVec4Offset"
            );
            public static readonly int _TransformUpdateOutputW2LVec4Offset = Shader.PropertyToID(
                "_TransformUpdateOutputW2LVec4Offset"
            );
            public static readonly int _TransformUpdateOutputPrevL2WVec4Offset = Shader.PropertyToID(
                "_TransformUpdateOutputPrevL2WVec4Offset"
            );
            public static readonly int _TransformUpdateOutputPrevW2LVec4Offset = Shader.PropertyToID(
                "_TransformUpdateOutputPrevW2LVec4Offset"
            );
            public static readonly int _BoundingSphereOutputVec4Offset = Shader.PropertyToID(
                "_BoundingSphereOutputVec4Offset"
            );
            ...
            public static readonly int _ProbeOcclusionUpdateDataQueue = Shader.PropertyToID(
                "_ProbeOcclusionUpdateDataQueue"
            );
        ...
        private static class InstanceWindDataUpdateIDs
        {
            ...
            public static readonly int _WindHistoryParamAddressArray = Shader.PropertyToID(
                "_WindHistoryParamAddressArray"
            );
        }
