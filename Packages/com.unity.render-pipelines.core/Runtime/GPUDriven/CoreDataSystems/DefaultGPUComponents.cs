...
namespace UnityEngine.Rendering
{
    ...
    internal static class DefaultShaderPropertyID
    {
        ...
        public static readonly int unity_RendererUserValuesPropertyEntry = Shader.PropertyToID(
            "unity_RendererUserValuesPropertyEntry"
        );

        public static readonly int[] DOTS_ST_WindParams = new int[(int)SpeedTreeWindParamIndex.MaxWindParamsCount];
        public static readonly int[] DOTS_ST_WindHistoryParams = new int[
            (int)SpeedTreeWindParamIndex.MaxWindParamsCount
        ];
    ...
    internal struct DefaultGPUComponents
    {
        ...
        {
            shCoefficients = archetypeManager.CreateComponent<SHCoefficients>(
                DefaultShaderPropertyID.unity_SHCoefficients,
                true
            );
            lightmapScaleOffset = archetypeManager.CreateComponent<Vector4>(
                DefaultShaderPropertyID.unity_LightmapST,
                true
            );
            objectToWorld = archetypeManager.CreateComponent<PackedMatrix>(
                DefaultShaderPropertyID.unity_ObjectToWorld,
                true
            );
            worldToObject = archetypeManager.CreateComponent<PackedMatrix>(
                DefaultShaderPropertyID.unity_WorldToObject,
                true
            );
            matrixPreviousM = archetypeManager.CreateComponent<PackedMatrix>(
                DefaultShaderPropertyID.unity_MatrixPreviousM,
                true
            );
            matrixPreviousMI = archetypeManager.CreateComponent<PackedMatrix>(
                DefaultShaderPropertyID.unity_MatrixPreviousMI,
                true
            );
            rendererUserValues = archetypeManager.CreateComponent<uint>(
            ...
            speedTreeWind = new NativeArray<GPUComponentHandle>(
                (int)SpeedTreeWindParamIndex.MaxWindParamsCount,
                Allocator.Persistent
            );
            speedTreeWindHistory = new NativeArray<GPUComponentHandle>(
                (int)SpeedTreeWindParamIndex.MaxWindParamsCount,
                Allocator.Persistent
            );

            for (int i = 0; i < (int)SpeedTreeWindParamIndex.MaxWindParamsCount; ++i)
                speedTreeWind[i] = archetypeManager.CreateComponent<Vector4>(
                    DefaultShaderPropertyID.DOTS_ST_WindParams[i],
                    true
                );
            for (int i = 0; i < (int)SpeedTreeWindParamIndex.MaxWindParamsCount; ++i)
                speedTreeWindHistory[i] = archetypeManager.CreateComponent<Vector4>(
                    DefaultShaderPropertyID.DOTS_ST_WindHistoryParams[i],
                    true
                );
            ...
            lightProbesComponentSet = new GPUComponentSet() { shCoefficients };
