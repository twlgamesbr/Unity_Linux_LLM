...
namespace UnityEditor.Rendering
{
    ...
    class LensFlareComponentSRPEditor : Editor
    {
        ...
        void OnEnable()
        {
            ...
            PropertyFetcher<LensFlareComponentSRP> entryPoint = new PropertyFetcher<LensFlareComponentSRP>(
                serializedObject
            );
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (renderPipelineAssetType != null && renderPipelineAssetType.Name == "HDRenderPipelineAsset")
            {
                if (
                    !(bool)
                        Type.GetType(
                                "UnityEditor.Rendering.HighDefinition.HDEditorUtils,Unity.RenderPipelines.HighDefinition.Editor"
                            )
            ...
            Light light = null;
            if (lensFlareData != null && (light = lensFlareData.GetComponent<Light>()) != null)
            {
            ...
            EditorGUILayout.LabelField(Styles.generalData.text, EditorStyles.boldLabel);
            {
                ...
                if (
                    lensFlareData.lensFlareData != null
                    && (
                        lensFlareData.lensFlareData.HasAModulateByLightColorElement()
                        || lensFlareData.attenuationByLightShape
                    )
                )
                {
        ...
        static class Styles
        {
            ...
            public static readonly GUIContent lensFlareData = EditorGUIUtility.TrTextContent(
                "Lens Flare Data",
                "Specifies the SRP Lens Flare Data asset this component uses."
            );
            public static readonly GUIContent newButton = EditorGUIUtility.TrTextContent(
                "New",
                "Create a new SRP Lens Flare Data asset."
            );
            public static readonly GUIContent cloneButton = EditorGUIUtility.TrTextContent(
                "Clone",
                "Create a new SRP Lens Flare Data asset and copy the content of the currently assigned data."
            );
            public static readonly GUIContent intensity = EditorGUIUtility.TrTextContent(
                "Intensity",
                "Sets the intensity of the lens flare."
            );
            public static readonly GUIContent scale = EditorGUIUtility.TrTextContent(
                "Scale",
                "Sets the scale of the lens flare."
            );
            public static readonly GUIContent maxAttenuationDistance = EditorGUIUtility.TrTextContent(
                "Attenuation Distance",
                "Sets the distance, in meters, between the start and the end of the Distance Attenuation Curve."
            );
            public static readonly GUIContent distanceAttenuationCurve = EditorGUIUtility.TrTextContent(
                "Attenuation Distance Curve",
                "Specifies the curve that reduces the effect of the lens flare  based on the distance between the GameObject this asset is attached to and the Camera."
            );
            public static readonly GUIContent maxAttenuationScale = EditorGUIUtility.TrTextContent(
                "Scale Distance",
                "Sets the distance, in meters, between the start and the end of the Scale Attenuation Curve."
            );
            public static readonly GUIContent scaleByDistanceCurve = EditorGUIUtility.TrTextContent(
                "Scale Distance Curve",
                "Specifies the curve used to calculate the size of the lens flare based on the distance between the GameObject this asset is attached to, and the Camera."
            );
            public static readonly GUIContent attenuationByLightShape = EditorGUIUtility.TrTextContent(
                "Attenuation By Light Shape",
                "When enabled, if the component is attached to a light, automatically reduces the effect of the lens flare based on the type and shape of the light."
            );
            public static readonly GUIContent radialScreenAttenuationCurve = EditorGUIUtility.TrTextContent(
                "Screen Attenuation Curve",
                "Specifies the curve that modifies the intensity of the lens flare based on its distance from the edge of the screen."
            );
            public static readonly GUIContent enableOcclusion = EditorGUIUtility.TrTextContent(
                "Screen Space Occlusion",
                "When enabled, the renderer uses the depth buffer to occlude (partially or completely) the lens flare. Partial occlusion also occurs when the lens flare is partially offscreen."
            );
            public static readonly GUIContent environmentOcclusion = EditorGUIUtility.TrTextContent(
                "Environment Occlusion",
                "When enabled, environment effects supported by the render pipeline can be used to occlude lens flares.\nThis may include opacity from volumetric clouds, background clouds, fog and water."
            );
            public static readonly GUIContent occlusionRadius = EditorGUIUtility.TrTextContent(
                "Occlusion Radius",
                "Sets the radius, in meters, around the light used to compute the occlusion of the lens flare. If this area is half occluded by geometry (or half off-screen), the intensity of the lens flare is cut by half."
            );
            public static readonly GUIContent sampleCount = EditorGUIUtility.TrTextContent(
                "Sample Count",
            ...
            public static readonly GUIContent occlusionOffset = EditorGUIUtility.TrTextContent(
                "Occlusion Offset",
                "Sets the offset of the occlusion area in meters between the GameObject this asset is attached to, and the Camera. A positive value moves the occlusion area closer to the Camera."
            );
            public static readonly GUIContent occlusionRemapCurve = EditorGUIUtility.TrTextContent(
                "Occlusion Remap Curve",
                "Specifies the curve used to remap the occlusion of the flare. By default, the occlusion is linear, between 0 and 1. This can be specifically useful to occlude flare more drastically when behind clouds."
            );
            public static readonly GUIContent allowOffScreen = EditorGUIUtility.TrTextContent(
                "Allow OffScreen",
                "When enabled, allows the lens flare to affect the scene even when it is outside the Camera's field of view."
            );
            public static readonly GUIContent volumetricCloudOcclusion = EditorGUIUtility.TrTextContent(
                "Volumetric Clouds",
                "When enabled, HDRP uses the volumetric clouds texture (in screen space) for the occlusion."
            );
            public static readonly GUIContent lightOverride = EditorGUIUtility.TrTextContent(
                "Light Override",
                "Specifies the light component where the color and shape values are fetched from when using \"Modulate By Light Color\" or \"Attenuation By Light Shape\" properties on a Lens Flare Element. If nothing is specified, the light component from this gameobject is used."
            );
        }
    }
}
