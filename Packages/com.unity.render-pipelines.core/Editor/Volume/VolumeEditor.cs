...
namespace UnityEditor.Rendering
{
    ...
    sealed class VolumeEditor : Editor
    {
        ...
        static class Styles
        {
            public static readonly string isGlobalDropdownTooltip = L10n.Tr(
                "Global Volumes affect the Camera wherever the Camera is in the Scene. Local Volumes affect the Camera if they encapsulate the Camera within the bounds of their Collider."
            );
            public static readonly GUIContent addBoxCollider = EditorGUIUtility.TrTextContent(
                "Add a Box Collider"
            );
            public static readonly GUIContent sphereBoxCollider = EditorGUIUtility.TrTextContent(
                "Add a Sphere Collider"
            );
            public static readonly GUIContent capsuleBoxCollider = EditorGUIUtility.TrTextContent(
                "Add a Capsule Collider"
            );
            public static readonly GUIContent meshBoxCollider = EditorGUIUtility.TrTextContent(
                "Add a Mesh Collider"
            );
            public static readonly GUIContent addColliderFixMessage =
                EditorGUIUtility.TrTextContentWithIcon(
                    "Add a Collider to this GameObject to set boundaries for the local Volume.",
                    CoreEditorStyles.iconWarn
                );
            public static readonly GUIContent disableColliderFixMessage =
                EditorGUIUtility.TrTextContentWithIcon(
                    "Global Volumes do not need a collider. Disable or remove the collider.",
                    CoreEditorStyles.iconWarn
                );
            public static readonly GUIContent physicsBackendDisabledMessage =
                EditorGUIUtility.TrTextContentWithIcon(
                    "Local Volumes are unavailable when the Physics GameObject SDK is set to None. Either choose a different Physics SDK in Project Settings > Physics, or set the Mode to Global.",
                    CoreEditorStyles.iconWarn
                );
            public static readonly GUIContent physicsModuleDisabledMessage =
                EditorGUIUtility.TrTextContentWithIcon(
                    "Local Volumes are unavailable without the Physics module. Enable the Physics module, or set the Mode to Global.",
                    CoreEditorStyles.iconWarn
                );
            public static readonly GUIContent enableColliderFixMessage =
                EditorGUIUtility.TrTextContentWithIcon(
                    "Local Volumes need a collider enabled. Enable the collider.",
                    CoreEditorStyles.iconWarn
                );
            public static readonly GUIContent newLabel = EditorGUIUtility.TrTextContent(
                "New",
                "Create a new profile."
            );
            public static readonly GUIContent saveLabel = EditorGUIUtility.TrTextContent(
                "Save",
                "Save the instantiated profile"
            );
            public static readonly GUIContent cloneLabel = EditorGUIUtility.TrTextContent(
                "Clone",
                "Create a new profile and copy the content of the currently assigned profile."
            );
            public static readonly GUIContent enableAll = EditorGUIUtility.TrTextContent(
                "Enable All"
            );
            public static readonly GUIContent disableAll = EditorGUIUtility.TrTextContent(
                "Disable All"
            );
            public static readonly GUIContent removeAll = EditorGUIUtility.TrTextContent(
                "Remove All"
            );
        }
        ...
        VolumeProfile profileRef =>
            targetVolume.HasInstantiatedProfile()
                ? targetVolume.profile
                : targetVolume.sharedProfile;
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            var isGlobalDropdown = new PopupField<int>(
                "Mode",
                modeList,
                m_IsGlobal.boolValue ? 1 : 0,
                ModeIntToString,
                ModeIntToString
            );
            ...
            isGlobalDropdown.TrackPropertyValue(
                m_IsGlobal,
                property =>
                {
            ...
            root.Q("collider-fixme-box__container")
                .Add(
                    new IMGUIContainer(() =>
                    {
                        ...
                        if (m_IsGlobal.boolValue) // Blend radius is not needed for global volumes
                        {
                            if (hasCollider && collider.enabled)
                                CoreEditorUtils.DrawFixMeBox(
                                    Styles.disableColliderFixMessage,
                                    () => SetColliderEnabledWithUndo(collider, false)
                                );
                        ...
                        else
                        {
                            ...
                            bool physicsBackendIsNone = Physics
                                .GetCurrentIntegrationInfo()
                                .isFallback;
                            ...
                            if (hasCollider)
                            {
                                if (!collider.enabled)
                                    CoreEditorUtils.DrawFixMeBox(
                                        Styles.enableColliderFixMessage,
                                        () => SetColliderEnabledWithUndo(collider, true)
                                    );
                            }
                            else
                            {
                                CoreEditorUtils.DrawFixMeBox(
                                    Styles.addColliderFixMessage,
                                    AddColliderWithUndo
                                );
                            }
            ...
            root.Q<Image>("volume-profile-objectfield__contextmenu-image").image =
                CoreEditorStyles.paneOptionsIcon;
            root.Q<Button>(classes: "volume-profile-objectfield__contextmenu").clicked +=
                OnVolumeProfileContextClick;
            root.Q<Button>("volume-profile-new-button").clicked += CreateNewProfile;

            var volumeProfileObjectField = root.Q<ObjectField>(
                classes: "volume-profile-objectfield"
            );
            volumeProfileObjectField.objectType = typeof(VolumeProfile);
            volumeProfileObjectField.TrackPropertyValue(
                m_Profile,
                _ =>
                {
            ...
            volumeProfileObjectField.schedule.Execute(() =>
                volumeProfileObjectField.RegisterValueChangedCallback(evt =>
                {
            ...
            priorityPropertyField.schedule.Execute(() =>
                priorityPropertyField.RegisterCallback<ChangeEvent<float>>(evt =>
                {
                    VolumePriorityUsageAnalytic.Send(targetVolume);
                })
            );

            root.Q("volume-profile-component-container")
                .Add(
                    new IMGUIContainer(() =>
                    {
        ...
        {
            root.Q<HelpBox>("volume-profile-empty-helpbox").style.display =
                profileRef != null ? DisplayStyle.None : DisplayStyle.Flex;
            root.Q<Button>("volume-profile-new-button").style.display =
                profileRef != null ? DisplayStyle.None : DisplayStyle.Flex;
            root.Q<Label>("volume-profile-instance-profile-label").style.display =
                targetVolume.HasInstantiatedProfile() ? DisplayStyle.Flex : DisplayStyle.None;
        }
        ...
        void CloneProfile()
        {
            ...
            path = CoreEditorUtils.IsAssetInReadOnlyPackage(path)
                ...
                ? AssetDatabase.GenerateUniqueAssetPath(
                    Path.Combine("Assets", Path.GetFileName(path))
                )
        ...
        void OnVolumeProfileContextClick()
        {
            ...
            if (profileRef != null)
            {
                ...
                var cloneLabel = targetVolume.HasInstantiatedProfile()
                    ? Styles.saveLabel
                    : Styles.cloneLabel;
                ...
                else
                {
                    menu.AddItem(
                        ...
                        () =>
                        {
                            VolumeProfileUtils.SetComponentEditorsExpanded(
                                m_ComponentList.editors,
                                false
                            );
                        }
                    );
                ...
                else
                {
                    ...
                    menu.AddItem(
                        VolumeProfileUtils.Styles.resetAll,
                        false,
                        () => m_ComponentList.ResetAllComponents()
                    );
                }
                ...
                {
                    menu.AddItem(
                        VolumeProfileUtils.Styles.copyAllSettings,
                        false,
                ...
                if (VolumeComponentCopyPaste.CanPaste(profileRef.components))
                    menu.AddItem(
                        ...
                        () =>
                        {
                            VolumeComponentCopyPaste.PasteSettings(
                                profileRef.components,
                                profileRef
                            );
        ...
        void AddColliderWithUndo()
        {
            var menu = new GenericMenu();
            menu.AddItem(
                Styles.addBoxCollider,
                false,
                () => Undo.AddComponent<BoxCollider>(targetVolume.gameObject)
            );
            menu.AddItem(
                Styles.sphereBoxCollider,
                false,
                () => Undo.AddComponent<SphereCollider>(targetVolume.gameObject)
            );
            menu.AddItem(
                Styles.capsuleBoxCollider,
                false,
                () => Undo.AddComponent<CapsuleCollider>(targetVolume.gameObject)
            );
            menu.AddItem(
                Styles.meshBoxCollider,
                false,
                () =>
                {
