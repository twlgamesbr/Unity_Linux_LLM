...
namespace UnityEditor.Rendering
{
    ...
    public static class AssetReimportUtils
    {
        ...
        public static void ReimportAll<TAsset>(
            out double duration,
            out uint numberOfAssetsReimported,
            Func<string, bool> importNeedDelegate = null
        )
        {
            ...
            {
                string[] distinctGuids = AssetDatabase.FindAssets($"t:{typeof(TAsset).Name}", null);
                ...
                try
                {
                    ...
                    for (int i = 0, total = distinctGuids.Length; i < total; ++i)
                    {
                        ...
                        EditorUtility.DisplayProgressBar(
                            $"{typeof(TAsset).Name} Upgrader re-import",
                            $"({i} of {total}) {path}",
                            (float)i / (float)total
                        );
