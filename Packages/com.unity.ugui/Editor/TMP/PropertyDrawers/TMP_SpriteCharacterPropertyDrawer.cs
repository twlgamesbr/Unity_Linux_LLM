    ...
    public class TMP_SpriteCharacterPropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (GUI.enabled == false)
            {
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x, rect.y, 75f, 18),
                    new GUIContent("Index: <color=#FFFF80>" + spriteCharacterIndex + "</color>"),
                    style
                );

                EditorGUI.LabelField(
                    new Rect(rect.x + 75f, rect.y, 120f, 18),
                    new GUIContent(
                        "Unicode: <color=#FFFF80>0x" + prop_SpriteUnicode.intValue.ToString("X") + "</color>"
                    ),
                    style
                );
                EditorGUI.LabelField(
                    new Rect(rect.x + 195f, rect.y, rect.width - 255, 18),
                    new GUIContent("Name: <color=#FFFF80>" + prop_SpriteName.stringValue + "</color>"),
                    style
                );

                EditorGUI.LabelField(
                    new Rect(rect.x, rect.y + 18, 120, 18),
                    new GUIContent("Glyph ID: <color=#FFFF80>" + prop_SpriteGlyphIndex.intValue + "</color>"),
                    style
                );
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x, rect.y + 36, 80, 18),
                    new GUIContent("Scale: <color=#FFFF80>" + prop_SpriteScale.floatValue + "</color>"),
                    style
                );
            }
            else // Display editable fields
            {
                ...
                string unicode = EditorGUI.DelayedTextField(
                    new Rect(rect.x + 75f, rect.y, 120, 18),
                    "Unicode:",
                    prop_SpriteUnicode.intValue.ToString("X")
                );
                ...
                EditorGUI.DelayedIntField(
                    new Rect(rect.x, rect.y + 18, 100, 18),
                    prop_SpriteGlyphIndex,
                ...
                if (glyphIndex == m_GlyphSelectedForEditing && GUI.enabled)
                {
                    if (spriteAsset != null)
                    {
                        ...
                        if (elementIndex != -1)
                        {
                            ...
                            SerializedProperty prop_SpriteGlyphTable = property.serializedObject.FindProperty(
                                "m_GlyphTable"
                            );

                            SerializedProperty prop_SpriteGlyph = prop_SpriteGlyphTable.GetArrayElementAtIndex(
                                elementIndex
                            );
                            ...
                            EditorGUI.DrawRect(
                                new Rect(newRect.x + 62, newRect.y - 20, newRect.width - 62, newRect.height - 5),
                                new Color(0.1f, 0.1f, 0.1f, 0.45f)
                            );
                            EditorGUI.DrawRect(
                                new Rect(newRect.x + 63, newRect.y - 19, newRect.width - 64, newRect.height - 7),
        ...
        void DrawSpriteGlyph(uint glyphIndex, Rect position, SerializedProperty property)
        {
            if (m_GlyphLookupDictionary == null)
                m_GlyphLookupDictionary = TMP_PropertyDrawerUtilities.GetGlyphProxyLookupDictionary(
            ...
            GUI.DrawTextureWithTexCoords(
                new Rect(
                    spriteTexPosition.x + alignmentOffset.x,
                    spriteTexPosition.y + alignmentOffset.y,
                    spriteSize.x,
                    spriteSize.y
                ),
                tex,
