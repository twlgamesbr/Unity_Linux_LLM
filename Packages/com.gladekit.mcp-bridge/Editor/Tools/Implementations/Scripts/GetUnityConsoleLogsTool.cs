    ...
    public class GetUnityConsoleLogsTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            return ToolUtils.CreateErrorResponse(
                "Could not read Unity Console (LogEntries API not available for this Unity version)."
            );
        }
        ...
        /// </summary>
        private static (List<string> entries, string error) TryGetLogEntriesViaReflection(
            int maxEntries
        )
        {
            ...
            try
            {
                ...
                if (logEntriesType == null)
                    return (
                        null,
                        "Unity Console API not found: LogEntries type missing. This Unity version may use a different internal API."
                    );
                ...
                MethodInfo getEntryMethod = ResolveGetEntryInternal(
                    logEntriesType,
                    logEntryType,
                    staticAny
                );
                if (getCountMethod == null)
                    return (
                        null,
                        "Unity Console API not found: GetCount method missing on LogEntries."
                    );
                if (getEntryMethod == null)
                    return (
                        null,
                        "Unity Console API not found: GetEntryInternal method missing or signature not matched."
                    );
                ...
                for (
                    int i = startIndex + take - 1;
                    i >= startIndex && entries.Count < maxEntries;
                    i--
                )
                {
                    ...
                    string message =
                        GetReflectionFieldString(logEntryType, entryObj, "message")
                        ?? GetReflectionFieldString(logEntryType, entryObj, "condition")
                        ?? "";
                    string stackTrace =
                        GetReflectionFieldString(logEntryType, entryObj, "stackTrace") ?? "";
                    int mode = 0;
        ...
        private static Type ResolveLogEntryType()
        {
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                t =
                    ...
                    ?? asm.GetType("UnityEditor.LogEntry");
                if (t != null)
                    return t;
            }
            try
        ...
        private static MethodInfo ResolveGetEntryInternal(
            Type logEntriesType,
            Type logEntryType,
            BindingFlags staticAny
        )
        {
            MethodInfo m = logEntriesType.GetMethod(
                "GetEntryInternal",
                staticAny,
                null,
            ...
            foreach (var method in logEntriesType.GetMethods(staticAny))
            {
                ...
                if (second == logEntryType)
                    return method;
                if (second.IsByRef && second.GetElementType() == logEntryType)
                    return method;
            }
            return null;
        }
