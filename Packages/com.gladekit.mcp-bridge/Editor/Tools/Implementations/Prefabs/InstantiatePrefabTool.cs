...
namespace GladeAgenticAI.Core.Tools.Implementations.Prefabs
{
    public class InstantiatePrefabTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            UnityEngine.GameObject prefab = AssetDatabase.LoadAssetAtPath<UnityEngine.GameObject>(
                prefabPath
            );
            ...
            if (prefab == null)
            {
                if (matches != null && matches.Count > 1)
                {
                    ...
                    return ToolUtils.CreateErrorResponse(
                        $"Prefab not found at '{prefabPath}'. Found multiple matches for '{prefabName}': {options}"
                    );
                }
            ...
            UnityEngine.GameObject instance =
                PrefabUtility.InstantiatePrefab(prefab) as UnityEngine.GameObject;
            if (instance == null)
            {
                return ToolUtils.CreateErrorResponse(
                    $"Failed to instantiate prefab from '{prefabPath}'"
                );
            }
            ...
            var extras = new Dictionary<string, object> { { "gameObjectPath", instance.name } };

            return ToolUtils.CreateSuccessResponse(
                $"Instantiated prefab '{prefabPath}' as '{instance.name}'",
                extras
            );
        }
    }
