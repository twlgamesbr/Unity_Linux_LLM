...
{
    public class AddAnimatorTransitionConditionsTool : ITool
    {
        public string Name => "add_animator_transition_conditions";
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            if (!args.ContainsKey("conditions"))
                return ToolUtils.CreateErrorResponse("conditions is required");
            ...
            {
                if (args["transitionIndex"] is int ti) transitionIndex = ti;
                else if (args["transitionIndex"] is float tf) transitionIndex = (int)tf;
            ...
            if (isAnyState)
            {
                var transitions = stateMachine.anyStateTransitions.Where(t => t.destinationState == destState).ToList();
                if (transitionIndex < 0 || transitionIndex >= transitions.Count)
                    return ToolUtils.CreateErrorResponse($"No Any State transition found from '{fromState}' to '{toState}' at index {transitionIndex}");
                transition = transitions[transitionIndex];
            }
            else
            {
                var transitions = sourceState.transitions.Where(t => t.destinationState == destState).ToList();
                if (transitionIndex < 0 || transitionIndex >= transitions.Count)
                    return ToolUtils.CreateErrorResponse($"No transition found from '{fromState}' to '{toState}' at index {transitionIndex}");
                transition = transitions[transitionIndex];
            }

            var conditionsSummary = new List<string>();
            int conditionsAdded = AddAnimatorTransitionConditionsInternal(controller, transition, args["conditions"], conditionsSummary);
            ...
            return ToolUtils.CreateSuccessResponse($"Added conditions to transition from '{fromState}' to '{toState}'{conditionsMsg}", extras);
        }
