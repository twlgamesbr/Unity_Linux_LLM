    ...
    public class GetComponentInspectorPropertiesTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            string gameObjectPath = args.ContainsKey("gameObjectPath")
                ...
                : "";
            string componentTypeName = args.ContainsKey("componentType")
            ...
            if (obj == null)
                return ToolUtils.CreateErrorResponse(
                    $"GameObject '{ToolUtils.EscapeJsonString(gameObjectPath)}' not found"
                );
            ...
            foreach (var comp in components)
            {
                ...
                if (
                    string.Equals(type.Name, componentTypeName, StringComparison.OrdinalIgnoreCase)
                    || string.Equals(
                        type.FullName,
                        componentTypeName,
            ...
            if (target == null)
                return ToolUtils.CreateErrorResponse(
                    $"Component '{ToolUtils.EscapeJsonString(componentTypeName)}' not found on '{ToolUtils.EscapeJsonString(gameObjectPath)}'"
                );

            bool onlyReferences = false;
            if (args.ContainsKey("onlyReferences"))
            {
                if (args["onlyReferences"] is bool b)
                    onlyReferences = b;
                else
                    bool.TryParse(args["onlyReferences"]?.ToString(), out onlyReferences);
            }

            bool onlyUnassigned = false;
            if (args.ContainsKey("onlyUnassigned"))
            {
                if (args["onlyUnassigned"] is bool b)
                    onlyUnassigned = b;
                else
                    bool.TryParse(args["onlyUnassigned"]?.ToString(), out onlyUnassigned);
            }

            bool onlyTopLevel = true;
            if (args.ContainsKey("onlyTopLevel"))
            {
                if (args["onlyTopLevel"] is bool b)
                    onlyTopLevel = b;
                else
                    bool.TryParse(args["onlyTopLevel"]?.ToString(), out onlyTopLevel);
            }

            var propertiesFilter = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            if (
                (
                    args.ContainsKey("propertyFilter") ? args["propertyFilter"]
                    : args.ContainsKey("properties") ? args["properties"]
                    : null
                )
                is IEnumerable<object> propObjs
            )
            {
            ...
            sb.Append(
                $",\"componentType\":\"{ToolUtils.EscapeJsonString(target.GetType().Name)}\""
            );
            ...
            while (prop.NextVisible(enterChildren))
            {
                ...
                if (prop.name == "m_Script")
                    continue;
                if (onlyTopLevel && prop.depth > 0)
                    continue;
                if (
                    propertiesFilter.Count > 0
                    && !propertiesFilter.Contains(prop.name)
                    && !propertiesFilter.Contains(prop.propertyPath)
                )
                    continue;

                bool isReference = prop.propertyType == SerializedPropertyType.ObjectReference;
                if (onlyReferences && !isReference)
                    continue;
                ...
                sb.Append(
                    $",\"type\":\"{ToolUtils.EscapeJsonString(prop.propertyType.ToString())}\""
                );
