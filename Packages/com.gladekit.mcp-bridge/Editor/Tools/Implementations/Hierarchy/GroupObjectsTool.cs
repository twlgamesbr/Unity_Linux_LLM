        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            if (paths.Count == 0)
            {
                return ToolUtils.CreateErrorResponse(
                    "No objects to group. Provide gameObjectPaths or select objects."
                );
            }

            string groupName = args.ContainsKey("groupName")
            ...
            {
                if (args["centerPivot"] is bool b)
                    centerPivot = b;
                else
                    bool.TryParse(args["centerPivot"].ToString(), out centerPivot);
            }
            ...
            return ToolUtils.CreateSuccessResponse(
                $"Created group '{groupName}' with {objects.Count} child(ren)",
                extras
            );
        }
    }
