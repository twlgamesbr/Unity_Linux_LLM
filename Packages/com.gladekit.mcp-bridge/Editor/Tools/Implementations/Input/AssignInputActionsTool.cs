    ...
    public class AssignInputActionsTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            return ToolUtils.CreateErrorResponse(
                "Cannot assign InputActionAsset: The project is not using the new Input System. The new Input System package (com.unity.inputsystem) must be installed and enabled in Project Settings > Player > Active Input Handling."
            );
#else
            ...
            PlayerInput playerInput =
                obj.GetComponent<PlayerInput>() ?? Undo.AddComponent<PlayerInput>(obj);
            playerInput.actions = asset;
            EditorUtility.SetDirty(playerInput);

            return ToolUtils.CreateSuccessResponse(
                $"Assigned InputActionAsset to '{gameObjectPath}'"
            );
#endif
        }
    }
