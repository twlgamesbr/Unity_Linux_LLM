    ...
    public class SetInputActionBindingsTool : ITool
    {
        ...
        public string Execute(Dictionary<string, object> args)
        {
            ...
            return ToolUtils.CreateErrorResponse(
                "Cannot modify InputActionAsset: The project is not using the new Input System. The new Input System package (com.unity.inputsystem) must be installed and enabled in Project Settings > Player > Active Input Handling."
            );
#else
            ...
            {
                if (args["replaceBindings"] is bool b)
                    replaceBindings = b;
                else
                    bool.TryParse(args["replaceBindings"].ToString(), out replaceBindings);
            }
            ...
            if (
                mapsObj is string mapsJson
                && ToolUtils.TryParseJsonArrayToList(mapsJson, out var parsedMaps)
            )
                mapsObj = parsedMaps;
            ...
            foreach (var mapObj in mapsList)
            {
                ...
                if (!mapDict.ContainsKey("actions"))
                    continue;
                var actionsObj = mapDict["actions"];
                if (
                    actionsObj is string actionsJson
                    && ToolUtils.TryParseJsonArrayToList(actionsJson, out var parsedActions)
                )
                ...
                foreach (var actionObj in actionsList)
                {
                    ...
                    string actionName = actionDict.ContainsKey("name")
                        ? actionDict["name"].ToString()
                        : "";
                    string actionType = actionDict.ContainsKey("type")
                        ? actionDict["type"].ToString()
                        : "Value";
                    ...
                    if (action == null)
                    {
                        action = map.AddAction(
                            actionName,
                            ToolUtils.ParseInputActionType(actionType)
                        );
                    }
                    ...
                    if (!actionDict.ContainsKey("bindings"))
                        continue;
                    var bindingsObj = actionDict["bindings"];
                    if (
                        bindingsObj is string bindingsJson
                        && ToolUtils.TryParseJsonArrayToList(bindingsJson, out var parsedBindings)
                    )
                        bindingsObj = parsedBindings;
                    if (!(bindingsObj is List<object> bindingsList))
                        continue;
                    ...
                    foreach (var bindObj in bindingsList)
                    {
                        if (!(bindObj is Dictionary<string, object> bindDict))
                            continue;
                        string path = bindDict.ContainsKey("path")
                            ? bindDict["path"].ToString()
                            : "";
                        if (string.IsNullOrEmpty(path))
                            continue;
                        var binding = action.AddBinding(path);
                        if (bindDict.ContainsKey("interactions"))
                            binding.WithInteractions(bindDict["interactions"].ToString());
                        if (bindDict.ContainsKey("processors"))
                            binding.WithProcessors(bindDict["processors"].ToString());
                        if (bindDict.ContainsKey("groups"))
                            binding.WithGroups(bindDict["groups"].ToString());
                        bindingsAdded++;
                    }
                }
            }
