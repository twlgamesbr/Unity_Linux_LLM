...
namespace GladeAgenticAI.Core.Tools
{
    ...
    public static class ToolUtils
    {
        ...
        /// </summary>
        public static string BuildStringArrayResultWithCount(
            string key,
        ...
        public static GameObject FindGameObjectByPath(string path)
        {
            ...
            {
                if (
                    string.Equals(root.name, segments[0].Trim(), StringComparison.OrdinalIgnoreCase)
                )
                {
                    current = root;
                    break;
        ...
        /// </summary>
        private static GameObject FindGameObjectByNameInScene(
            string name,
        ...
        public static string GetDefaultShaderForRenderPipeline()
        {
            ...
            if (renderPipelineAsset != null)
            {
                ...
                // URP detection
                if (
                    assetType.Contains("UniversalRenderPipelineAsset")
                ...
                // HDRP detection
                if (
                    assetType.Contains("HDRenderPipelineAsset")
            ...
            if (targetType.IsEnum)
            {
                ...
                catch
                {
                    throw new ArgumentException(
                        $"Cannot convert '{valueStr}' to enum {targetType.Name}"
                    );
                }
            }
            ...
            if (typeof(UnityEngine.Object).IsAssignableFrom(targetType))
            {
                ...
                )
                {
                    ...
                    if (foundObj != null)
                    {
                        ...
                        throw new ArgumentException(
                            $"GameObject '{assetPath}' found, but it does not have a component of type '{targetType.Name}'. Available components: {string.Join(", ", foundObj.GetComponents<Component>().Select(c => c.GetType().Name))}"
                        );
                    }

                    // If not found in scene, throw error (don't try asset path for scene objects)
                    throw new ArgumentException(
                        $"GameObject or Component not found at path '{assetPath}'. Make sure the GameObject exists in the scene hierarchy (e.g., 'Canvas/Ammo' or 'Canvas/Reload/ammoText')."
                    );
                }
                ...
                UnityEngine.Object asset =
                    ToolUtils.LoadAssetAtPathCaseInsensitive<UnityEngine.Object>(assetPath);

                if (asset == null)
                {
                    // Try to find similar assets for better error message
                    string fileName = System.IO.Path.GetFileName(assetPath);
                    string searchName = !string.IsNullOrEmpty(fileName)
                    ...
                    if (!string.IsNullOrEmpty(searchName))
                    {
                        ...
                        {
                            suggestion =
                                $" Similar assets found: {string.Join(", ", similarPaths)}";
                        }
                    }

                    throw new ArgumentException(
                        $"Asset not found at path '{assetPath}'. Make sure the asset exists and the path is correct (e.g., 'Assets/Sprites/MySprite.png').{suggestion}"
                    );
                }
                ...
                if (targetType == typeof(Sprite) && asset is Texture2D)
                {
                    ...
                    if (importer != null && importer.textureType != TextureImporterType.Sprite)
                    {
                        ...
                        UnityEngine.Object[] allAssets = AssetDatabase.LoadAllAssetsAtPath(
                            assetPath
                        );
                        ...
                        throw new ArgumentException(
                            $"Converted texture at '{assetPath}' to Sprite import type, but could not load the Sprite. The texture may need to be reimported manually."
                        );
                    }
                ...
                if (!targetType.IsAssignableFrom(asset.GetType()))
                {
                    ...
                    {
                        additionalHelp =
                            " Note: Automatic conversion from Texture2D to Sprite was attempted. If this error persists, the texture may need manual configuration via set_sprite_import_settings.";
                    }
                    throw new ArgumentException(
                        $"Asset at path '{assetPath}' is of type '{asset.GetType().Name}' but property requires type '{targetType.Name}'.{additionalHelp}"
                    );
                }
        ...
        public static List<string> GetPathsFromArgsOrSelection(
        ...
        public static Dictionary<string, object> CheckColliderConflicts(GameObject obj)
        {
            ...
            {
                warnings.Add(
                    $"WARNING: GameObject has CharacterController AND {existingColliders.Count} other collider(s). CharacterController includes its own collider - additional colliders may cause conflicts."
                );
            }
            else if (hasCharacterController)
            {
                warnings.Add(
                    "INFO: GameObject has CharacterController (includes built-in collider). Adding additional colliders may cause conflicts."
                );
            }
            ...
            {
                warnings.Add(
                    $"WARNING: GameObject has {existingColliders.Count} colliders. Multiple colliders on the same object may cause unexpected physics behavior."
                );
            }
        ...
        /// </summary>
        public static Dictionary<string, object> CalculateColliderSizeFromBounds(
            Bounds bounds,
            string colliderType
        )
        {
            ...
            switch (colliderType.ToLower())
            {
                ...
                case "capsule":
                {
                    ...
                    if (size.x >= size.y && size.x >= size.z)
                        direction = 0; // X is longest
                    else if (size.z >= size.x && size.z >= size.y)
                        direction = 2; // Z is longest
        ...
        public static Dictionary<string, object> SuggestPrimitiveColliderType(GameObject obj)
        {
            ...
            float dimensionVariance =
                (
                    Mathf.Abs(size.x - size.y)
                    + Mathf.Abs(size.y - size.z)
                    + Mathf.Abs(size.x - size.z)
                ) / maxDim;
            ...
            if (
                size.y > size.x * 1.5f
                && size.y > size.z * 1.5f
                && Mathf.Abs(size.x - size.z) / maxDim < 0.3f
            )
            {
                ...
                size.x > size.y * 1.5f
                && size.x > size.z * 1.5f
                && Mathf.Abs(size.y - size.z) / maxDim < 0.3f
            )
            {
                ...
                size.z > size.x * 1.5f
                && size.z > size.y * 1.5f
                && Mathf.Abs(size.x - size.y) / maxDim < 0.3f
            )
            {
            ...
            if (isRoughlySpherical)
            {
                ...
                result["reason"] =
                    $"Mesh is roughly spherical (dimensions: {size.x:F2}, {size.y:F2}, {size.z:F2})";
            }
            ...
            else
            {
                ...
                result["reason"] =
                    $"Mesh is box-like (dimensions: {size.x:F2}, {size.y:F2}, {size.z:F2})";
            }
        ...
        public static void CollectHierarchyPaths(
            GameObject obj,
            List<string> paths,
        ...
        /// Returns default when missing or unparseable.
        /// </summary>
        public static float GetFloatArg(
            Dictionary<string, object> args,
            string key,
        ...
        public static string ResolveTemplatePath(string fileName)
        {
            ...
            foreach (var guid in AssetDatabase.FindAssets(searchName))
            {
                ...
                if (
                    ...
                    && path.Replace('\\', '/')
                        .EndsWith("/Templates/" + leaf, StringComparison.OrdinalIgnoreCase)
                    && File.Exists(path)
                )
                {
                    return path;
        ...
        /// </summary>
        public static string ValidateRequiredArgs(
            Dictionary<string, object> args,
            params string[] keys
        )
        {
        ...
        public static int ParseLayerMask(object value)
        {
            ...
            if (
                ...
                || s.Equals("Everything", StringComparison.OrdinalIgnoreCase)
            )
                return ~0;
            if (int.TryParse(s, out int mask))
                return mask;
            var parts = s.Split(',');
        ...
        public static System.Type FindAnimationBindingType(string typeName)
        {
            ...
            if (type != null && IsValidAnimationBindingType(type))
                return type;
            ...
            foreach (var assembly in System.AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                for (int i = 0; i < types.Length; i++)
                {
                    ...
                    if (t == null)
                        continue;
                    if (!IsValidAnimationBindingType(t))
                        continue;
                    if (t.Name == trimmed)
                        return t;
                }
        ...
        /// </summary>
        public static T LoadAssetAtPathCaseInsensitive<T>(string assetPath)
            where T : UnityEngine.Object
        {
            ...
            foreach (var guid in guids)
            {
                ...
                if (string.Equals(foundFileName, fileName, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    string foundDir = System
                        .IO.Path.GetDirectoryName(foundPath)
                        ?.Replace('\\', '/');
        ...
        public static string NormalizeAssetPath(string assetPath, out string actualPath)
        {
            ...
            if (!string.IsNullOrEmpty(fileName) && !string.IsNullOrEmpty(directory))
            {
                ...
                foreach (var guid in guids)
                {
                    ...
                    if (
                        string.Equals(foundFileName, fileName, StringComparison.OrdinalIgnoreCase)
            ...
            switch (prop.propertyType)
            {
                ...
                case UnityEditor.SerializedPropertyType.ObjectReference:
                    return prop.objectReferenceValue != null
                        ? $"\"{EscapeJsonString(prop.objectReferenceValue.name)}\""
                        : "null";
