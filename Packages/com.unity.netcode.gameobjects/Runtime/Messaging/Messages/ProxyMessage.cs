        ...
        public unsafe void Handle(ref NetworkContext context)
        {
            ...
            if (
                !networkManager.SpawnManager.SpawnedObjects.TryGetValue(
                    WrappedMessage.Metadata.NetworkObjectId,
            ...
            )
            {
                ...
                if (networkManager.LogLevel == LogLevel.Developer)
                {
                    NetworkLog.LogWarning(
                        $"[{WrappedMessage.Metadata.NetworkObjectId}, {WrappedMessage.Metadata.NetworkBehaviourId}, {WrappedMessage.Metadata.NetworkRpcMethodId}] An RPC called on a {nameof(NetworkObject)} that is not in the spawned objects list. Please make sure the {nameof(NetworkObject)} is spawned before calling RPCs."
                    );
                }
                return;
            }
            ...
            {
                var networkBehaviour = networkObject.GetNetworkBehaviourAtOrderIndex(
                    WrappedMessage.Metadata.NetworkBehaviourId
                );

                RpcInvokePermission permission = NetworkBehaviour.__rpc_permission_table[networkBehaviour.GetType()][
                    WrappedMessage.Metadata.NetworkRpcMethodId
                ];
                ...
                if (!hasPermission)
                {
                    if (networkManager.LogLevel <= LogLevel.Developer)
                    {
                        NetworkLog.LogErrorServer(
                            $"Rpc message received from client-{context.SenderId} who does not have permission to perform this operation!"
                        );
                    }
                    return;
                }
            ...
            {
                WrappedMessage.WriteBuffer.WriteBytesSafe(
                    WrappedMessage.ReadBuffer.GetUnsafePtr(),
                    WrappedMessage.ReadBuffer.Length
                );
