                ...
                switch (Override)
                {
                    case NetworkPrefabOverride.None:
                    {
                        ...
                        throw new InvalidOperationException(
                            $"Prefab field is not set or is not a {nameof(NetworkObject)}"
                        );
                    }
                    case NetworkPrefabOverride.Prefab:
                    {
                        if (
                            ...
                            && SourcePrefabToOverride.TryGetComponent(out NetworkObject networkObject)
                        )
                        {
                            return networkObject.GlobalObjectIdHash;
                        }
                        ...
                        throw new InvalidOperationException(
                            $"Source Prefab field is not set or is not a {nameof(NetworkObject)}"
                        );
                    }
                ...
                switch (Override)
                {
                    ...
                    case NetworkPrefabOverride.Hash:
                    {
                        if (
                            ...
                            && OverridingTargetPrefab.TryGetComponent(out NetworkObject networkObject)
                        )
                        {
                            return networkObject.GlobalObjectIdHash;
                        }
                        ...
                        throw new InvalidOperationException(
                            $"Target Prefab field is not set or is not a {nameof(NetworkObject)}"
                        );
                    }
                    default:
                        throw new ArgumentOutOfRangeException();
        ...
        public bool Validate(int index = -1)
        {
            ...
            if (Override == NetworkPrefabOverride.None)
            {
                ...
                if (networkObject == null)
                {
                    ...
                    {
                        NetworkLog.LogWarning(
                            $"{NetworkPrefabHandler.PrefabDebugHelper(this)} is missing a {nameof(NetworkObject)} component (entry will be ignored)."
                        );
                    }
            ...
            switch (Override)
            {
                case NetworkPrefabOverride.Hash:
                {
                    if (SourceHashToOverride == 0)
                    {
                        ...
                        {
                            NetworkLog.LogWarning(
                                $"{nameof(NetworkPrefab)} {nameof(SourceHashToOverride)} is zero (entry will be ignored)."
                            );
                        }
                ...
                case NetworkPrefabOverride.Prefab:
                {
                    if (SourcePrefabToOverride == null)
                    {
                        ...
                        {
                            NetworkLog.LogWarning(
                                $"{nameof(NetworkPrefab)} {nameof(SourcePrefabToOverride)} is null (entry will be ignored)."
                            );
                            return false;
                        }
                    }
                    ...
                    if (!SourcePrefabToOverride.TryGetComponent(out networkObject))
                    {
                        ...
                        {
                            NetworkLog.LogWarning(
                                $"{nameof(NetworkPrefab)} ({SourcePrefabToOverride.name}) is missing a {nameof(NetworkObject)} component (entry will be ignored)."
                            );
                        }
            ...
            if (OverridingTargetPrefab == null)
            {
                ...
                switch (Override)
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"{nameof(NetworkPrefab)} override entry {SourceHashToOverride} will be removed and ignored."
                        );
                        break;
                    }
                    case NetworkPrefabOverride.Prefab:
                    {
                        Debug.LogWarning(
                            $"{nameof(NetworkPrefab)} override entry ({SourcePrefabToOverride.name}) will be removed and ignored."
                        );
                        break;
                    }
                }
