...
namespace Unity.Netcode.Editor
{
    ...
    public class NetworkPrefabsEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            m_NetworkPrefabsList = new ReorderableList(
                serializedObject,
            ...
            m_NetworkPrefabsList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                ...
                var networkSourceHashProp = networkPrefab.FindPropertyRelative(
                    nameof(NetworkPrefab.SourceHashToOverride)
                );
                var networkSourcePrefabProp = networkPrefab.FindPropertyRelative(
                    nameof(NetworkPrefab.SourcePrefabToOverride)
                );
                var networkTargetPrefabProp = networkPrefab.FindPropertyRelative(
                    nameof(NetworkPrefab.OverridingTargetPrefab)
                );
                var networkOverrideProp = networkPrefab.FindPropertyRelative(nameof(NetworkPrefab.Override));
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x + rect.width - 70, rect.y, 60, EditorGUIUtility.singleLineHeight),
                    "Override"
                );
                if (networkOverrideEnum == NetworkPrefabOverride.None)
                {
                    if (
                        EditorGUI.Toggle(
                            new Rect(rect.x + rect.width - 15, rect.y, 10, EditorGUIUtility.singleLineHeight),
                            false
                        )
                    )
                    {
                ...
                else
                {
                    ...
                    {
                        EditorGUI.PropertyField(
                            new Rect(rect.x + 110, rect.y, rect.width - 190, EditorGUIUtility.singleLineHeight),
                            networkSourcePrefabProp,
                            GUIContent.none
                        );
                    }
                    else
                    ...
                    EditorGUI.PropertyField(
                        new Rect(rect.x + 110, rect.y, rect.width - 110, EditorGUIUtility.singleLineHeight),
                        networkTargetPrefabProp,
                        GUIContent.none
                    );
                }
            };
