...
namespace Unity.Netcode.Editor.CodeGen
{
    internal sealed class INetworkMessageILPP : ILPPInterface
    {
        ...
        public override bool WillProcess(ICompiledAssembly compiledAssembly) =>
            ...
            || compiledAssembly.Name == CodeGenHelpers.ComponentsAssemblyName;
        ...
        public override ILPostProcessResult Process(ICompiledAssembly compiledAssembly)
        {
            ...
            if (mainModule != null)
            {
                ...
                {
                    var types = mainModule
                        .GetTypes()
                        .Where(t =>
                            t.Resolve().HasInterface(CodeGenHelpers.INetworkMessage_FullName) && !t.Resolve().IsAbstract
                        )
                        .ToList();
        ...
        private const string k_CreateMessageAndGetVersionName = nameof(
            NetworkMessageManager.CreateMessageAndGetVersion
        );
        ...
        private bool ImportReferences(ModuleDefinition moduleDefinition)
        {
            ...
            {
                if (
                    messageHandlerTypeDef == null
                    && netcodeTypeDef.Name == nameof(NetworkMessageManager.MessageHandler)
                )
                {
                ...
                if (
                    messageWithHandlerTypeDef == null
                    && netcodeTypeDef.Name == nameof(NetworkMessageManager.MessageWithHandler)
                )
                {
            ...
            m_MessageManager_MessageHandler_Constructor_TypeRef = moduleDefinition.ImportReference(
                messageHandlerTypeDef.GetConstructors().First()
            );
            m_MessageManager_VersionGetter_Constructor_TypeRef = moduleDefinition.ImportReference(
                versionGetterTypeDef.GetConstructors().First()
            );
            ...
            foreach (var fieldDef in messageWithHandlerTypeDef.Fields)
            {
                switch (fieldDef.Name)
                {
                    case nameof(NetworkMessageManager.MessageWithHandler.MessageType):
                        m_MessageManager_MessageWithHandler_MessageType_FieldRef = moduleDefinition.ImportReference(
                            fieldDef
                        );
                        break;
                    case nameof(NetworkMessageManager.MessageWithHandler.Handler):
                        m_MessageManager_MessageWithHandler_Handler_FieldRef = moduleDefinition.ImportReference(
                            fieldDef
                        );
                        break;
                    case nameof(NetworkMessageManager.MessageWithHandler.GetVersion):
                        m_MessageManager_MessageWithHandler_GetVersion_FieldRef = moduleDefinition.ImportReference(
                            fieldDef
                        );
                        break;
                }
            ...
            foreach (var fieldDef in ilppMessageProviderTypeDef.Fields)
            {
                switch (fieldDef.Name)
                {
                    case nameof(ILPPMessageProvider.__network_message_types):
                        m_ILPPMessageProvider___network_message_types_FieldRef = moduleDefinition.ImportReference(
                            fieldDef
                        );
                        break;
                }
            ...
            foreach (var methodDef in listTypeDef.Methods)
            {
                switch (methodDef.Name)
                {
                    case "Add":
                        ...
                        m_List_Add_MethodRef.DeclaringType = listTypeDef.MakeGenericInstanceType(
            ...
            foreach (var methodDef in messageManagerSystemTypeDef.Methods)
            {
                switch (methodDef.Name)
                {
                    ...
                    case k_CreateMessageAndGetVersionName:
                        m_MessageManager_CreateMessageAndGetVersion_MethodRef = moduleDefinition.ImportReference(
                            methodDef
                        );
                        break;
                }
        ...
        private void CreateInstructionsToRegisterType(
            ILProcessor processor,
            List<Instruction> instructions,
            TypeReference type,
            MethodReference receiveMethod,
            MethodReference versionMethod
        )
        {
