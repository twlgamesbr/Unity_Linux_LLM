...
";
        for (int i = 0; i < _overrideAsset.overrideList.Count; i++)
        {
            ...
            if (overrideData.type == ShaderPropertyType.Color || overrideData.type == ShaderPropertyType.Vector)
            {
                ...
                generatedStruct =
                    $@"    [MaterialProperty(""{@overrideData.name}"")]
...
public class MaterialOverrideAssetEditor : Editor
{
    ...
    /// <summary>
    /// Draws the custom inspector for the material override asset.
    /// </summary>
    public override void OnInspectorGUI()
    {
        ...
        if (dirtyGameObjects)
        {
            foreach (var overrideComponent in FindObjectsByType<MaterialOverride>())
            {
