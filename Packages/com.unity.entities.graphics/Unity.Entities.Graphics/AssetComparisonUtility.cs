...
namespace Unity.Rendering
{
    ...
    internal static partial class AssetComparisonUtility
    {
        ...
        [AutoStaticsCleanup]
        static Dictionary<EntityId, AssetIdentifier> s_AssetIdentifierCache = new();
        ...
        internal static AssetIdentifier GetAssetIdentifier<T>(UnityObjectRef<T> assetRef) where T : Object
        {
            ...
            if (s_AssetIdentifierCache.TryGetValue(entityId, out var cached))
                return cached;
            ...
            if (obj != null)
            {
                if (UnityEditor.AssetDatabase.TryGetGUIDAndLocalFileIdentifier(obj, out var guid, out var localId))
                {
            ...
            s_AssetIdentifierCache[entityId] = identifier;
            return identifier;
#else
...
#else
            return a.Id.entityId.CompareTo(b.Id.entityId);
#endif
        }
    }
}
