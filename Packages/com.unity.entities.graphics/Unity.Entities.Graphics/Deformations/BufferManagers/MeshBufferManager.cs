...
namespace Unity.Rendering
{
    internal class MeshBufferManager
    {
        ...
        static readonly int k_MaxSize = (int)
            math.min(SystemInfo.maxGraphicsBufferSize / UnsafeUtility.SizeOf<VertexData>(), int.MaxValue);
        ...
        public int ActiveDeformedMeshBufferIndex =>
            (m_DeformedMeshData != null) ? m_DeformedMeshData.ActiveBufferIndex : 0;
#else
        ...
        {
#if ENABLE_DOTS_DEFORMATION_MOTION_VECTORS
            m_DeformedMeshData = new ComputeDoubleBufferWrapper<VertexData>(
                k_CurrentFrameBufferProperty,
                k_PreviousFrameBufferProperty,
                k_ChunkSize
            );
#else
            m_DeformedMeshData = new ComputeBufferWrapper<VertexData>(
                Shader.PropertyToID("_DeformedMeshData"),
                k_ChunkSize
            );
#endif
                ...
                if (newSize > k_MaxSize)
                {
                    ...
                    if (requiredSize > k_MaxSize)
                        UnityEngine.Debug.LogWarning(
                            "The world contains too many deformed meshes to fit into a single GraphicsBuffer. Not all deformed meshes are guaranteed to render correctly. Reduce the number of active deformed meshes."
                        );
