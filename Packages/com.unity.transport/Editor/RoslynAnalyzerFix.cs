...
namespace Unity.Networking.Editor
{
    public class RoslynAnalyzerFix : AssetPostprocessor
    {
        ...
        public static string OnGeneratedCSProject(string path, string content)
        {
            ...
            if (
                content.Contains(
                    @"Packages\com.unity.transport\Analyzers\Unity.Transport.Analyzers.dll"
                )
            )
            {
                var newDoc = content;

                string[] lines = newDoc.Split(
                ...
                xDocument.Root?.Add(
                    new XElement(
                        xNamespace + "ItemGroup",
                        new XElement(xNamespace + "Analyzer", new XAttribute("Include", fullPath))
                    )
                );
