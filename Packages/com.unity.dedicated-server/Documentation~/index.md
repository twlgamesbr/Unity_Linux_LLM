...
When using Multiplayer Roles stripping in conjunction with prefab instantiation via `InstantiateAsync` from the Editor, prefabs containing children assigned specific roles (e.g., Client) may not be stripped correctly based on the active editor role (e.g., Server). Specifically, prefabs instantiated during runtime will retain all their original children, regardless of role assignments and the active editor role.

**Workaround** - Ensure that role-based stripping behavior is handled explicitly in your scripts or prefabs, rather than relying solely on automatic stripping during runtime.
