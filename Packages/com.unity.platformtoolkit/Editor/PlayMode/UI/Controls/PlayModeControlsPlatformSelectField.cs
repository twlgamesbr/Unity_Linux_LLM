...
namespace Unity.PlatformToolkit.PlayMode
{
    internal class PlayModeControlsPlatformSelectField : VisualElement, IDisposable
    {
        ...
        private void RefreshSelection()
        {
            ...
            if (
                m_Dropdown.value != null
                && m_PlayModeControlsView.CapabilityOptions.TryGetValue(m_Dropdown.value, out var capability)
                && capability == currentCapability
            )
                return;

            m_Dropdown.choices = m_PlayModeControlsView.CapabilityOptionNames;
            var key = m_PlayModeControlsView
                .CapabilityOptions.FirstOrDefault(kvp => kvp.Value == currentCapability)
                .Key;
            if (key != null)
