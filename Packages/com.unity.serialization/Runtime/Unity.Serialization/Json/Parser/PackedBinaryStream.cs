...
namespace Unity.Serialization.Json
{
    ...
    unsafe struct UnsafePackedBinaryStream : IDisposable
    {
        ...
        public UnsafePackedBinaryStream(int initialTokenCapacity, int initialBufferCapacity, Allocator label)
        {
            ...
            Tokens = (BinaryToken*)
                UnsafeUtility.Malloc(
                    sizeof(BinaryToken) * initialTokenCapacity,
                    UnsafeUtility.AlignOf<BinaryToken>(),
                    m_Label
                );
            Handles = (HandleData*)
                UnsafeUtility.Malloc(
                    sizeof(HandleData) * initialTokenCapacity,
                    UnsafeUtility.AlignOf<HandleData>(),
                    m_Label
                );
            ...
            Buffer = (byte*)
                UnsafeUtility.Malloc(sizeof(byte) * initialBufferCapacity, UnsafeUtility.AlignOf<byte>(), m_Label);
            BufferCapacity = initialBufferCapacity;
            BufferPosition = 0;

            for (var i = 0; i < initialTokenCapacity; i++)
            {
                Tokens[i] = new BinaryToken { HandleIndex = i };

                Handles[i] = new HandleData { DataIndex = i, DataVersion = 1 };
            }
        }
        ...
        internal T* GetBufferPtr<T>(int tokenIndex)
            where T : unmanaged
        {
        ...
        internal T* GetBufferPtr<T>(Handle handle)
            where T : unmanaged
        {
        ...
        internal void EnsureTokenCapacity(int newLength)
        {
            ...
            {
                Tokens[index] = new BinaryToken { HandleIndex = index };

                Handles[index] = new HandleData { DataIndex = index, DataVersion = 1 };
            }
        ...
        static T* Resize<T>(T* buffer, int fromLength, int toLength, Allocator label)
            where T : unmanaged
        {
    ...
    public unsafe struct PackedBinaryStream : IDisposable, IEquatable<PackedBinaryStream>
    {
        ...
        UnsafePackedBinaryStream* m_Data;
        ...
        public PackedBinaryStream(Allocator label)
            : this(k_DefaultTokenCapacity, k_DefaultBufferCapacity, label) { }
        ...
        {
            m_Label = label;
            m_Data = (UnsafePackedBinaryStream*)
                UnsafeUtility.Malloc(
                    sizeof(UnsafePackedBinaryStream),
                    UnsafeUtility.AlignOf<UnsafePackedBinaryStream>(),
                    m_Label
                );
