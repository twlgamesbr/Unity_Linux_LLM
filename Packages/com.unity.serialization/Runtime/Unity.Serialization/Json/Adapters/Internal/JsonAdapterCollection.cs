...
namespace Unity.Serialization.Json
{
    ...
    struct JsonAdapterCollection
    {
        ...
        public struct Enumerator
        {
            ...
            public Enumerator(
                List<IJsonAdapter> userDefinedAdapters,
                List<IJsonAdapter> globalAdapters,
                JsonAdapter internalAdapter
            )
            {
                ...
                m_Current = null;
                m_State =
                    null != userDefinedAdapters ? State.User
                    : null != globalAdapters ? State.Global
                    : null != internalAdapter ? State.Internal
                    : State.End;
                m_Index = -1;
            }
                ...
                for (; ; )
                {
                    ...
                    switch (m_State)
                    {
                        case State.User:
                            ...
                            m_State =
                                null != m_GlobalAdapters ? State.Global
                                : null != m_InternalAdapter ? State.Internal
                                : State.End;
                            m_Index = -1;
                            break;
                        case State.Global:
        ...
        public Enumerator GetEnumerator(JsonAdapterFilter filter = JsonAdapterFilter.All) =>
            new Enumerator(
                filter.HasFlag(JsonAdapterFilter.UserDefined) ? UserDefined : null,
                filter.HasFlag(JsonAdapterFilter.Global) ? Global : null,
                filter.HasFlag(JsonAdapterFilter.Internal) ? InternalAdapter : null
            );
