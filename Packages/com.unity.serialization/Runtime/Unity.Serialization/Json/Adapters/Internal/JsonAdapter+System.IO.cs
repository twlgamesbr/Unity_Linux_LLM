...
{
    partial class JsonAdapter : IJsonAdapter<DirectoryInfo>, IJsonAdapter<FileInfo>
    {
        void IJsonAdapter<DirectoryInfo>.Serialize(
            in JsonSerializationContext<DirectoryInfo> context,
            DirectoryInfo value
        )
        {
            if (null == value)
        ...
        {
            return context.SerializedValue.AsStringView().Equals("null")
                ? null
                : new DirectoryInfo(context.SerializedValue.ToString());
        }
        ...
        FileInfo IJsonAdapter<FileInfo>.Deserialize(in JsonDeserializationContext<FileInfo> context)
        {
            return context.SerializedValue.AsStringView().Equals("null")
                ? null
                : new FileInfo(context.SerializedValue.ToString());
        }
    }
}
