...
namespace UnityEditor.Rendering.Universal
{
    ...
    class ScreenSpaceLensFlareEditor : VolumeComponentEditor
    {
        ...
        static GUIContent s_Intensity = EditorGUIUtility.TrTextContent(
            "Intensity",
            "Sets the global intensity of the Screen Space Lens Flare effect. When set to 0, the pass is skipped. "
        );
        static GUIContent s_TintColor = EditorGUIUtility.TrTextContent(
            "Tint Color",
            "Sets the color used to tint all the flares. "
        );
        static GUIContent s_BloomMip = EditorGUIUtility.TrTextContent(
            "Bloom Mip Bias",
            "Controls the Bloom mip used as a source for the Lens Flare effect. A high value will result in a blurrier result for all the flares."
        );
        static GUIContent s_FirstFlareIntensity = EditorGUIUtility.TrTextContent(
            "Regular Multiplier",
            "Controls the intensity of the Regular Flare sample. Those flares are sampled using scaled screen coordinates."
        );
        static GUIContent s_SecondaryFlareIntensity = EditorGUIUtility.TrTextContent(
            "Reversed Multiplier",
            "Controls the intensity of the Reversed Flare sample. Those flares are sampled using scaled and flipped screen coordinates."
        );
        static GUIContent s_WarpedFlareIntensity = EditorGUIUtility.TrTextContent(
            "Warped Multiplier",
            "Controls the intensity of the Warped Flare sample. Those flares are sampled using polar screen coordinates."
        );
        static GUIContent s_WarpedFlareScale = EditorGUIUtility.TrTextContent(
            "Scale",
            "Sets the scale of the Warped Flare sample. A value of 1,1 will keep this flare circular. Set values below 1 can lead to stretching artifacts on the side of the screen."
        );
        static GUIContent s_Samples = EditorGUIUtility.TrTextContent(
            "Samples",
        ...
        static GUIContent s_SampleDimmer = EditorGUIUtility.TrTextContent(
            "Sample Dimmer",
            "Controls the value by which each additionnal sample is multiplied. This parameter has an effect only after the first sample."
        );
        static GUIContent s_VignetteEffect = EditorGUIUtility.TrTextContent(
            "Vignette Effect",
            "Controls the intensity of the vignette effect to occlude the Lens Flare effect at the center of the screen."
        );
        static GUIContent s_StartingPosition = EditorGUIUtility.TrTextContent(
            "Starting Position",
            "Controls the starting position of the flares in screen space relative to their source."
        );
        static GUIContent s_Scale = EditorGUIUtility.TrTextContent(
            "Scale",
            "Controls the scale at which the flares are sampled. This parameter only impacts regular, reversed and warped flares."
        );
        static GUIContent s_ChromaticAbberationIntensity = EditorGUIUtility.TrTextContent(
            "Intensity",
        ...
        static GUIContent s_StreaksIntensity = EditorGUIUtility.TrTextContent(
            "Multiplier",
        ...
        static GUIContent s_StreaksOrientation = EditorGUIUtility.TrTextContent(
            "Orientation",
            "Controls the orientation streaks effect in degrees. A value of 0 creates horizontal streaks."
        );
        static GUIContent s_StreaksLength = EditorGUIUtility.TrTextContent(
            "Length",
            "Controls the length of streaks effect. A value of one creates streaks about the width of the screen."
        );
        static GUIContent s_StreaksThreshold = EditorGUIUtility.TrTextContent(
            "Threshold",
            "Controls the threshold of streak effect. A high value makes the effect more localised on the high intensity areas of the screen. When using a non HDR Camera, a high threshold can remove streaks entirely."
        );
        static GUIContent s_Resolution = EditorGUIUtility.TrTextContent(
            "Resolution",
