...
namespace UnityEditor.Rendering.Universal
{
    ...
    sealed class UniversalSampleBufferNode
            ...
            IMayRequireNDCPosition
    {
        ...
        {
            AddSlot(
                new ScreenPositionMaterialSlot(
                    k_ScreenPositionSlotId,
                    k_ScreenPositionSlotName,
                    k_ScreenPositionSlotName,
                    ScreenSpaceType.Default
                )
            );
            ...
            RemoveSlotsNameNotMatching(new[] { k_ScreenPositionSlotId, k_OutputSlotId });
        }
                        ...
                        using (s.BlockScope())
                        {
                            switch (bufferType)
                            {
                                ...
                                case BufferType.MotionVectors:
                                    ...
                                    s.AppendLine(
                                        $"return LOAD_TEXTURE2D_X_LOD(_MotionVectorTexture, pixelCoords, 0).xy;"
                                    );
                                    break;
                                case BufferType.BlitSource:
                                    s.AppendLine("uint2 pixelCoords = uint2(uv * _ScreenSize.xy);");
        ...
        public void GenerateNodeCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            ...
            sb.AppendLine(
                $"$precision{channelCount} {GetVariableNameForSlot(k_OutputSlotId)} = {GetFunctionName()}({uv}.xy);"
            );
        }
