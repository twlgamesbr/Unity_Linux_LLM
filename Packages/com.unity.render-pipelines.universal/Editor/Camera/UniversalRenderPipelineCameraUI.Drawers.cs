...
namespace UnityEditor.Rendering.Universal
{
    ...
    static partial class UniversalRenderPipelineCameraUI
    {
        ...
        static readonly AdditionalPropertiesState<ExpandableAdditional, Camera> k_ExpandedAdditionalState = new(
            0,
            "URP"
        );
        ...
        public static readonly CED.IDrawer SectionStackSettings = CED.Conditional(
            ...
            CED.FoldoutGroup(
                ...
                k_ExpandedState,
                FoldoutOption.Indent,
                CED.Group(DrawerStackCameras)
            )
        );

        public static readonly CED.IDrawer[] Inspector =
        {
            CED.Group(PrepareTileOnlyModeWarning, DrawerCameraType),
        ...
        static void DrawerProjection(UniversalRenderPipelineSerializedCamera p, Editor owner)
        {
            ...
            bool pixelPerfectEnabled =
                camera.TryGetComponent<PixelPerfectCamera>(out var pixelPerfectCamera) && pixelPerfectCamera.enabled;
        ...
        static void DrawerCameraType(UniversalRenderPipelineSerializedCamera p, Editor owner)
        {
            ...
            CameraRenderType camType = scriptableRenderer.SupportsCameraStackingType(CameraRenderType.Overlay)
                ? originalCamType
                : CameraRenderType.Base;
            EditorGUI.BeginDisabledGroup(scriptableRenderer.SupportedCameraStackingTypes() == 0);
            camType = (CameraRenderType)
        ...
        static void PrepareTileOnlyModeWarning(UniversalRenderPipelineSerializedCamera serialized, Editor owner)
        {
            ...
            var firstAdditionalData = (UniversalAdditionalCameraData)
                serialized.serializedAdditionalDataObject.targetObjects[0];
            ...
            {
                var additionalCameraData = (UniversalAdditionalCameraData)
                    serialized.serializedAdditionalDataObject.targetObjects[i];
            ...
            lastTileOnlyModeInfos = new TileOnlyModeInfos(
                sb.ToString(),
                assetToOpen: targetSameAsset ? rendererData : null
            );
        }

        static void DisplayTileOnlyModeWarning(
            SerializedProperty prop,
            Func<SerializedProperty, bool> shouldDisplayWarning,
            GUIContent label,
        ...
        )
        {
            ...
            if (lastTileOnlyModeInfos.assetToOpen != null)
                CoreEditorUtils.DrawFixMeBox(
                    string.Format(
                        Styles.formatterTileOnlyMode,
                        label == null ? prop.displayName : label.text,
                        lastTileOnlyModeInfos.rendererName
                    ),
                    MessageType.Warning,
                    "Open",
