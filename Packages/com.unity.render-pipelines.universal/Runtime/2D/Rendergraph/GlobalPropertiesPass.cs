...
namespace UnityEngine.Rendering.Universal
{
    internal class GlobalPropertiesPass : ScriptableRenderPass
    {
        ...
        private static readonly ProfilingSampler m_SetGlobalPropertiesProfilingSampler = new ProfilingSampler(
            k_SetGlobalProperties
        );
        ...
        internal static void Setup(RenderGraph graph, ContextContainer frameData, bool useLights)
        {
            ...
            using (
                var builder = graph.AddRasterRenderPass<PassData>(
                    k_SetGlobalProperties,
                    out var passData,
                    m_SetGlobalPropertiesProfilingSampler
                )
            )
            {
                ...
                if (
                    ...
                    && pixelPerfectCamera.enabled
                    && pixelPerfectCamera.offscreenRTSize != Vector2Int.zero
                )
                ...
                if (useLights)
                {
                    ...
                    var lightLookupTexture = graph.ImportTexture(
                        Light2DLookupTexture.GetLightLookupTexture_Rendergraph()
                    );
                    var fallOffTexture = graph.ImportTexture(
                        Light2DLookupTexture.GetFallOffLookupTexture_Rendergraph()
                    );
                ...
                if (rendererData.useCameraSortingLayerTexture)
                    builder.SetGlobalTextureAfterPass(
                        universal2DResourceData.cameraSortingLayerTexture,
                        CopyCameraSortingLayerPass.k_CameraSortingLayerTextureId
                    );
