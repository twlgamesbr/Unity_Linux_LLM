...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public partial class DrawObjectsPass : ScriptableRenderPass
    {
        ...
        internal DrawObjectsPass(
            URPProfileId profileId,
            bool opaque,
            RenderPassEvent evt,
            RenderQueueRange renderQueueRange,
            LayerMask layerMask,
        ...
        internal static void ExecutePass(
            RasterCommandBuffer cmd,
            PassData data,
            RendererList rendererList,
            RendererList objectsWithErrorRendererList,
            bool yFlip
        )
        {
            ...
            float alphaToMaskAvailable =
                ((data.cameraData.cameraTargetDescriptor.msaaSamples > 1) && data.isOpaque) ? 1.0f : 0.0f;
        ...
        internal void InitPassData(
            UniversalCameraData cameraData,
            ref PassData passData,
            uint batchLayerMask,
            bool isActiveTargetBackBuffer = false
        )
        {
        ...
        )
        {
            ...
            else
            {
                ...
                RenderingUtils.CreateRendererListObjectsWithError(
                    renderGraph,
                    ref renderingData.cullResults,
                    camera,
                    filterSettings,
                    sortFlags,
                    ref passData.objectsWithErrorRendererListHdl
                );
            }
        }
        ...
        {
            return cameraData.renderer.useDepthPriming
                && isOpaque
                && (cameraData.renderType == CameraRenderType.Base || cameraData.clearDepth);
        }
        ...
        internal void Render(
            ...
            in TextureHandle mainShadowsTexture,
            in TextureHandle additionalShadowsTexture,
            uint batchLayerMask = uint.MaxValue,
            bool isMainOpaquePass = false
        )
        {
            ...
            )
            {
                ...
                builder.SetRenderFunc(
                    static (PassData data, RasterGraphContext context) =>
                    {
                        ...
                        bool yFlip = RenderingUtils.IsHandleYFlipped(
                            context,
                            in (data.albedoHdl.IsValid() ? ref data.albedoHdl : ref data.depthHdl)
                        );
                        ...
                        if (useScreenSpaceIrradiance)
                        {
                            context.cmd.SetGlobalTexture(
                                ShaderPropertyId.screenSpaceIrradiance,
                                data.screenSpaceIrradianceHdl
                            );
                        }
                        ...
                        ExecutePass(
                            context.cmd,
                            data,
                            data.rendererListHdl,
                            data.objectsWithErrorRendererListHdl,
                            yFlip
                        );
                    }
                );
    ...
    internal class DrawObjectsWithRenderingLayersPass : DrawObjectsPass
    {
        ...
        /// <param name="stencilState">The stencil settings to use with this poss.</param>
        /// <param name="stencilReference">The stencil reference value to use with this pass.</param>
        public DrawObjectsWithRenderingLayersPass(
            URPProfileId profilerTag,
            bool opaque,
            RenderPassEvent evt,
            RenderQueueRange renderQueueRange,
            LayerMask layerMask,
        ...
        {
            using (
                var builder = renderGraph.AddRasterRenderPass<RenderingLayersPassData>(
                    passName,
                    out var passData,
                    profilingSampler
                )
            )
            {
                ...
                builder.SetRenderFunc(
                    static (RenderingLayersPassData data, RasterGraphContext context) =>
                    {
                        ...
                        bool yFlip = RenderingUtils.IsHandleYFlipped(
                            context,
                            in (
                                data.basePassData.albedoHdl.IsValid()
                                    ? ref data.basePassData.albedoHdl
                                    : ref data.basePassData.depthHdl
                            )
                        );
                        ...
                        ExecutePass(
                            context.cmd,
                            data.basePassData,
                            data.basePassData.rendererListHdl,
                            data.basePassData.objectsWithErrorRendererListHdl,
                            yFlip
                        );
