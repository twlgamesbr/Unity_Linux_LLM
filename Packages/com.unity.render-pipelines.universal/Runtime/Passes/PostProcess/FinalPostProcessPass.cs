...
namespace UnityEngine.Rendering.Universal
{
    internal sealed class FinalPostProcessPass : PostProcessPass
    {
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            // Final blit pass
            using (
                var builder = renderGraph.AddRasterRenderPass<PostProcessingFinalBlitPassData>(
                    passName,
                    out var passData,
                    profilingSampler
                )
            )
            {
                ...
                if (cameraData.xr.enabled)
                {
                    ...
                    bool passSupportsFoveation = !Experimental.Rendering.XRSystem.foveatedRenderingCaps.HasFlag(
                ...
                builder.SetRenderFunc(
                    static (PostProcessingFinalBlitPassData data, RasterGraphContext context) =>
                    {
                        var cmd = context.cmd;
                        ...
                        switch (filteringOperation)
                        {
                            ...
                            case FilteringOperation.TaaSharpening:
                                ...
                                FSRUtils.SetRcasConstantsLinear(
                                    cmd,
                                    data.cameraData.taaSettings.contrastAdaptiveSharpening
                            ...
                            case FilteringOperation.FsrSharpening:
                                ...
                                float sharpness = data.cameraData.fsrOverrideSharpness
                                    ? data.cameraData.fsrSharpness
                                    : FSRUtils.kDefaultSharpnessLinear;
                                ...
                                if (data.cameraData.fsrSharpness > 0.0f)
                                {
                                    ...
                                    material.EnableKeyword(
                                        requireHDROutput
                                            ? ShaderKeywordStrings.EasuRcasAndHDRInput
                                            : ShaderKeywordStrings.Rcas
                                    );
                        ...
                        material.SetVector(
                            ...
                            PostProcessUtils.CalcShaderSourceSize(sourceTextureHdl)
                        );
                        ...
                        if (data.cameraData.xr.enabled && data.cameraData.xr.hasValidVisibleMesh)
                        {
                            ...
                            data.cameraData.xr.RenderVisibleMeshCustomMaterial(
                                cmd,
                                data.cameraData.xr.occlusionMeshScale,
                                material,
                                xrPropertyBlock,
                                1,
                                context.GetTextureUVOrigin(in data.sourceTexture)
                                    == context.GetTextureUVOrigin(in data.destinationTexture)
                            );
                        }
                        else
#endif
