    ...
    unsafe struct ConvexHullBuilder
    {
        ...
        public void Rebuild2D()
        {
            ...
            int numVertices = 0;
            {
                ...
                float3 direction = 1.0f / math.max(1e-10f, aabb.Extents); // Use the min aabb extent as regressand
                ...
                for (int j = 0; j < numFaceEdges[i]; j++)
                {
                    ...
                    if (dot < k_cosMaxMergeAngle)
                    {
                        if (dot < k_cosSharpAngle)
                        {
                            ...
                            edgePlane.Distance -= simplificationTolerance / 2.0f; // push the edge plane out slightly so it only becomes active if the face planes change significiantly
