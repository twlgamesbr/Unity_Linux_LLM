...
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.CapsuleCollider>)
)]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.CylinderCollider>)
)]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.SphereCollider>)
)]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.PolygonCollider>)
)]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.ConvexCollider>)
)]
[assembly: RegisterGenericComponentType(typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.MeshCollider>))]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.CompoundCollider>)
)]
[assembly: RegisterGenericComponentType(
    typeof(Unity.Physics.DummyColliderFakesComponent<Unity.Physics.TerrainCollider>)
)]
...
namespace Unity.Physics
{
    ...
    static class ColliderHeaderFakes
    {
        ...
        public static ColliderHeader GetHeaderForColliderType<T>()
            where T : ICollider
        {
            ...
            throw new Exception(
                $"The Collider typeIndex {index}, does not map to a supported ColliderType, please update the checks above."
            );
        }
    ...
    static class SafetyChecks
    {
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckMaterialGetterValid(
            ColliderType type,
            ColliderKey colliderKey,
            in FixedString32Bytes methodName
        )
        {
            if (type == ColliderType.Compound && colliderKey == ColliderKey.Empty)
                throw new InvalidOperationException(
                    $"Calling {methodName}() on a CompoundCollider requires a non-empty ColliderKey!"
                );
        }

        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckSimulationStageAndThrow(
            SimulationScheduleStage currentStage,
            SimulationScheduleStage expectedStage
        )
        {
            if (currentStage != expectedStage)
                throw new Exception(
                    $"It is not possible to do the operation in {currentStage} simulation stage. Required stage is {expectedStage}"
                );
        }
        ...
        public static unsafe void CheckMemorySizeAndThrow(byte* begin, byte* end, ulong expectedByteSize)
        {
            ...
            if (actualByteSize != expectedByteSize)
                throw new InvalidOperationException(
                    $"Memory size mismatch. Expected {expectedByteSize} bytes, but was {actualByteSize} bytes."
                );
        }

        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckAreEqualAndThrow<T>(T expected, T actual)
        ...
        public static void CheckInRangeAndThrow(int value, int2 range, in FixedString32Bytes paramName)
        {
            if (value < range.x || value > range.y)
                throw new ArgumentOutOfRangeException(
                    $"{paramName}",
                    $"{value} is out of range [{range.x}, {range.y}]."
                );
        }
        ...
        public static void CheckWithinThresholdAndThrow(float value, float threshold, in FixedString32Bytes paramName)
        {
            if (value < -threshold || value > threshold)
                throw new ArgumentOutOfRangeException(
                    $"{paramName}",
                    $"{value} cannot exceed threshold of {threshold}"
                );
        }

        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckNotEmptyAndThrow<T>(NativeArray<T> array, in FixedString32Bytes paramName)
            where T : struct
        {
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckCoplanarAndThrow(
            float3 vertex0,
            float3 vertex1,
            float3 vertex2,
            float3 vertex3,
            in FixedString32Bytes paramName
        )
        {
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckTriangleIndicesInRangeAndThrow(
            ...
            for (var i = 0; i < triangles.Length; ++i)
            {
                if (math.any(triangles[i] < 0) || math.any(triangles[i] >= numVertices))
                    throw new ArgumentException(
                        $"{paramName}",
                        $"Triangle {triangles[i]} contained index out of range [0, {numVertices - 1}]"
                    );
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Geometry_CheckFiniteAndThrow(
            float3 value,
            in FixedString32Bytes paramName,
            in FixedString32Bytes propertyName
        )
        {
            if (math.any(!math.isfinite(value)))
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Geometry_CheckFiniteAndPositiveAndThrow(
            float value,
            in FixedString32Bytes paramName,
            in FixedString32Bytes propertyName
        )
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Geometry_CheckValidAndThrow(
            quaternion q,
            in FixedString32Bytes paramName,
            in FixedString32Bytes propertyName
        )
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckValidAndThrow(
            NativeArray<float3> points,
            in FixedString32Bytes pointsName,
            in ConvexHullGenerationParameters generationParameters,
            in FixedString32Bytes paramName
        )
        {
            Geometry_CheckFiniteAndPositiveAndThrow(
                generationParameters.BevelRadius,
                paramName,
                nameof(ConvexHullGenerationParameters.BevelRadius)
            );
        ...
        public static void CheckValidAndThrow(in BoxGeometry geometry, in FixedString32Bytes paramName)
        {
            ...
            if (geometry.BevelRadius < 0f || geometry.BevelRadius > math.cmin(geometry.Size) * 0.5f)
                throw new ArgumentException(
                    $"{paramName}",
                    $"{nameof(BoxGeometry.BevelRadius)} must be greater than or equal to 0 and less than or equal to half the smallest size dimension."
                );
        }
        ...
        public static void CheckValidAndThrow(in CylinderGeometry geometry, in FixedString32Bytes paramName)
        {
            ...
            Geometry_CheckFiniteAndPositiveAndThrow(
                ...
                paramName,
                nameof(CylinderGeometry.BevelRadius)
            );
            if (geometry.BevelRadius < 0f || geometry.BevelRadius > math.min(geometry.Height * 0.5f, geometry.Radius))
                throw new ArgumentException(
                    $"{paramName}",
                    $"{nameof(CylinderGeometry.BevelRadius)} must be greater than or equal to 0 and less than or equal to half the smallest size dimension."
                );
            if (
                geometry.SideCount < CylinderGeometry.MinSideCount
                || geometry.SideCount > CylinderGeometry.MaxSideCount
            )
                throw new ArgumentException(
                    $"{paramName}",
                    $"{nameof(CylinderGeometry.SideCount)} must be greater than or equal to {CylinderGeometry.MinSideCount} and less than or equal to {CylinderGeometry.MaxSideCount}."
                );
        }
        ...
        public static void CheckValidBasisPriorityAndThrow(int3 basisPriority)
        {
            ...
            {
                throw new ArgumentException(
                    $"The provided {nameof(basisPriority)} is invalid. Every basis vector index must appear exactly once."
                );
            }
        }
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void CheckValidCloneRequestAndThrow(
            CollisionWorld world,
            bool deepCopyDynamicColliders,
            bool deepCopyStaticColliders,
            NativeList<int> deepCopyRigidBodyList
        )
        {
            ...
            {
                throw new ArgumentException(
                    "deepCopyRigidBodyIndexList.Length must be less than or equal to the number of rigid bodies."
                );
            }
            ...
            foreach (var index in deepCopyRigidBodyList)
            {
                ...
                if (
                    deepCopyDynamicColliders && index < numDynamicBodies
                    || deepCopyStaticColliders && index > numDynamicBodies
                )
                {
                    throw new ArgumentException(
                        "deepCopyRigidBodyIndexList contains the index of a rigid body that is already deep copied using the deep copy input arguments."
                    );
                }
            }
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void ThrowInvalidOperationException(FixedString512Bytes message = default) =>
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void ThrowNotSupportedException(FixedString512Bytes message = default) =>
        ...
        [Conditional(CompilationSymbols.CollectionsChecksSymbol), Conditional(CompilationSymbols.DebugChecksSymbol)]
        public static void LogWarning(FixedString64Bytes message = default) => UnityEngine.Debug.LogWarning(message);
