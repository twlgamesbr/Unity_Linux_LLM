...
{
    internal abstract class BaseJointBaker<T> : Baker<T>
        where T : UnityEngine.Component
    {
        ...
        protected static quaternion GetJointFrameOrientation(float3 axis, float3 secondaryAxis)
        {
            ...
            return new BodyFrame { Axis = a, PerpendicularAxis = p }
                .AsRigidTransform()
                .rot;
        }
        ...
        protected bool3 GetAxesWithMotionType(
            ...
            ConfigurableJointMotion x,
            ConfigurableJointMotion y,
            ConfigurableJointMotion z
        ) => new bool3(x == motionType, y == motionType, z == motionType);

        protected void ConvertSpringDamperSettings(
            float inSpringConstant,
            float inDampingCoefficient,
            float inConstrainedMass,
            out float outSpringFrequency,
            out float outDampingRatio
        )
        {
            ...
            if (inSpringConstant <= threshold)
            {
                ...
                outDampingRatio =
                    inDampingCoefficient <= threshold ? Constraint.DefaultDampingRatio : inDampingCoefficient;
            }
            else
            {
                // Case: k!=0, c=0: Calculate for k and use damping ratio as 0
                // Case: k!=0, c!=0: Calculate both terms
                outSpringFrequency = JacobianUtilities.CalculateSpringFrequencyFromSpringConstant(
                    inSpringConstant,
                    inConstrainedMass
                );
                outDampingRatio =
                    inDampingCoefficient <= threshold
                        ? 0.0f
                        : JacobianUtilities.CalculateDampingRatio(
                            inSpringConstant,
                            inDampingCoefficient,
                            inConstrainedMass
                        );
            }
        }
        ...
        protected float GetConstrainedBodyInertia(in UnityEngine.Joint joint)
        {
            ...
            {
                approxInertia +=
                    1f / 3f * (rigidBody.inertiaTensor.x + rigidBody.inertiaTensor.y + rigidBody.inertiaTensor.z);
            }
            ...
            if (connectedBody != null && !connectedBody.isKinematic)
            {
                approxInertia +=
                    1f
                    / 3f
                    * (connectedBody.inertiaTensor.x + connectedBody.inertiaTensor.y + connectedBody.inertiaTensor.z);
            }
        ...
        )
        {
            ...
            RigidTransform bFromA = isConnectedBodyConverted
...
#if UNITY_EDITOR
            bool isPrefabAsset =
                UnityEditor.PrefabUtility.GetPrefabAssetType(joint) != UnityEditor.PrefabAssetType.NotAPrefab
                && UnityEditor.PrefabUtility.GetPrefabInstanceStatus(joint)
            ...
            if (isPrefabAsset && joint.autoConfigureConnectedAnchor)
            {
                ...
                var bodyBTransform = joint.connectedBody
                    ? new RigidTransform(joint.connectedBody.transform.rotation, joint.connectedBody.transform.position)
                    : RigidTransform.identity;
        ...
        protected CombinedJoint CreateConfigurableJoint(
            ...
            SoftJointLimitSpring linearSpring,
            bool3 angularFree,
            bool3 angularLocks,
            bool3 angularLimited,
        ...
        )
        {
            ...
            if (angularLimited[0])
            {
                ConvertSpringDamperSettings(
                    angularXLimitSpring.spring,
                    angularXLimitSpring.damper,
                    constrainedMass,
                    out var springFrequency,
                    out var dampingRatio
                );
        ...
        protected PhysicsJoint ConvertMotor(
            quaternion jointFrameOrientation,
            UnityEngine.Joint joint,
            JacobianType motorType,
            float target,
            float maxImpulseForMotor,
            float springFrequencyIn = Constraint.DefaultSpringFrequency,
            float springDampingIn = Constraint.DefaultDampingRatio
        )
        {
            var bodyAFromJoint = new BodyFrame { };
            var bodyBFromJoint = new BodyFrame { };
            ...
            ConvertSpringDamperSettings(
                ...
                springDampingIn,
                constrainedMass,
                out var springFrequencyOut,
                out var springDampingOut
            );

            switch (motorType)
            {
                case JacobianType.PositionMotor:
                    jointData = PhysicsJoint.CreatePositionMotor(
                        bodyAFromJoint,
                        bodyBFromJoint,
                        target,
                        maxImpulseForMotor,
                ...
                case JacobianType.LinearVelocityMotor:
                    jointData = PhysicsJoint.CreateLinearVelocityMotor(
                        bodyAFromJoint,
                        bodyBFromJoint,
                        target,
                        maxImpulseForMotor,
                ...
                case JacobianType.RotationMotor:
                    jointData = PhysicsJoint.CreateRotationalMotor(
                        bodyAFromJoint,
                        bodyBFromJoint,
                        target,
                        maxImpulseForMotor,
                ...
                case JacobianType.AngularVelocityMotor:
                    jointData = PhysicsJoint.CreateAngularVelocityMotor(
                        bodyAFromJoint,
                        bodyBFromJoint,
                        target,
                        maxImpulseForMotor,
        ...
        protected SolverType GetSolverType(UnityEngine.Component c)
        {
            var solverTypeAuthoring = c.gameObject.GetComponent<SolverTypeAuthoring>();
            if (solverTypeAuthoring != null && solverTypeAuthoring.enabled)
            {
                return solverTypeAuthoring.JointSolverType;
            }
            return SolverType.Iterative;
        }

        protected Entity CreateJointEntity(
            uint worldIndex,
        ...
        protected void CreateJointEntities(
            uint worldIndex,
            PhysicsConstrainedBodyPair constrainedBodyPair,
            SolverType solverType,
        ...
        )
        {
            ...
            foreach (var joint in joints)
            {
                ...
                AddComponent(jointEntity, new PhysicsSolverType { Value = solverType });
    ...
    class CharacterBaker : BaseJointBaker<CharacterJoint>
    {
        void ConvertCharacterJoint(CharacterJoint joint)
        {
            ...
            var jointData = CreateConfigurableJoint(
                jointFrameOrientation,
                joint,
                linearLocks,
                linearLimited,
                ...
                joint.twistLimitSpring,
                joint.swing1Limit,
                joint.swing2Limit,
                joint.swingLimitSpring
            );

            var worldIndex = GetWorldIndex(joint);
            var solverType = GetSolverType(joint);

            using (
                var joints = new NativeArray<PhysicsJoint>(2, Allocator.Temp)
                {
                    [0] = jointData.LinearJoint,
                    [1] = jointData.AngularJoint,
                }
            )
            using (var jointEntities = new NativeList<Entity>(2, Allocator.Temp))
            {
                CreateJointEntities(worldIndex, GetConstrainedBodyPair(joint), solverType, joints, jointEntities);
            }
        }
    ...
    {
        private void CreateJointEntityBlocks(
        ...
        )
        {
            ...
            for (int i = 0; i < constraints.Length; i += 3)
            {
                ...
                var solverType = GetSolverType(joint);
                CreateJointEntity(worldIndex, GetConstrainedBodyPair(joint), solverType, thisJoint);
            }
        }

        void ConvertLinearDofs(
            UnityEngine.ConfigurableJoint joint,
            float constrainedMass,
            quaternion jointFrameOrientation,
            ref FixedList512Bytes<Constraint> constraints
        )
        {
            var linearLocks = GetAxesWithMotionType(
                ConfigurableJointMotion.Locked,
                joint.xMotion,
                joint.yMotion,
                joint.zMotion
            );
            var linearLimited = GetAxesWithMotionType(
            ...
            IdentifyIfLinearMotor(
                joint,
                out bool3 isPositionMotor,
                out bool3 isLinearVelocityMotor,
                out float3 maxForceForLinearMotor
            );

            Assert.IsTrue(
                math.csum(new int3(isLinearVelocityMotor)) <= 1,
                $"Unity.Physics doesn't fully support double and triple motorization of linear velocity at the moment, only one of those motors will work, game object {joint.name}"
            );
            ...
            if (math.any(linearLimited))
            {
                ConvertSpringDamperSettings(
                    ...
                    joint.linearLimitSpring.damper,
                    constrainedMass,
            ...
            float3 linearStiffness = new float3(
                joint.xDrive.positionSpring,
                joint.yDrive.positionSpring,
                joint.zDrive.positionSpring
            );
            float3 linearDamping = new float3(
                joint.xDrive.positionDamper,
                joint.yDrive.positionDamper,
                joint.zDrive.positionDamper
            );
            ...
            for (int axis = 0; axis < 3; ++axis)
            {
                ...
                ConvertSpringDamperSettings(
                    linearStiffness[axis],
                    linearDamping[axis],
                    constrainedMass,
                    out float motorFrequency,
                    out float motorDampingRatio
                );
                ...
                constraints.Add(
                    new Constraint
                    {
                        ...
                        Type = isPositionMotor[axis]
                            ? ConstraintType.PositionMotor
                            : ConstraintType.LinearVelocityMotor,
        ...
        {
            var angularLocks = GetAxesWithMotionType(
                ConfigurableJointMotion.Locked,
                joint.angularXMotion,
                joint.angularYMotion,
                joint.angularZMotion
            );
            var angularLimited = GetAxesWithMotionType(
                ConfigurableJointMotion.Limited,
                joint.angularXMotion,
                joint.angularYMotion,
                joint.angularZMotion
            ...
            IdentifyIfAngularMotor(
                joint,
                out bool3 isRotationMotor,
                out bool3 isAngularVelocityMotor,
                out float3 maxForceForAngularMotor
            );

            Assert.IsTrue(
                joint.rotationDriveMode != RotationDriveMode.Slerp,
                $"Slerp drive mode is not supported by the conversion at the moment, defaulting to twist and swing instead, game object {joint.name}"
            );
            ...
            if (math.any(angularLimited))
            {
                ...
                if (angularLimited.x)
                {
                    ConvertSpringDamperSettings(
                        ...
                        constrainedMass,
                        out float springFrequencyX,
                        out float dampingRatioX
                    );
                ...
                if (angularLimited.y)
                {
                    ConvertSpringDamperSettings(
                        ...
                        joint.angularYZLimitSpring.damper,
                        constrainedMass,
                        out float springFrequencyYZ,
                        out float dampingRatioYZ
                    );
            ...
            for (int axis = 0; axis < 3; ++axis)
            {
                ...
                constraints.Add(
                    ...
                    {
                        ConstrainedAxes = new bool3(axis == 0, axis == 1, axis == 2),
                        Type = isRotationMotor[axis]
                            ? ConstraintType.RotationMotor
                            : ConstraintType.AngularVelocityMotor,
                        Min = -math.INFINITY,
                        Max = math.INFINITY,
                        SpringFrequency = motorFrequency,
                        DampingRatio = motorDampingRatio,
                        MaxImpulse = maxForceForAngularMotor * Time.fixedDeltaTime,
                        Target = isRotationMotor[axis]
                            ? -math.radians(joint.targetRotation.eulerAngles)
        ...
        private void IdentifyIfLinearMotor(
            UnityEngine.ConfigurableJoint joint,
            out bool3 isPositionMotor,
            out bool3 isLinearVelocityMotor,
            out float3 maxForceForLinearMotor
        )
        {
            ...
            CheckPerAxis(
                ...
                xDrive.positionDamper,
                xDrive.maximumForce,
                name,
                out isPositionMotor.x,
                out isLinearVelocityMotor.x
            );

            CheckPerAxis(
                targetPosition.y,
                targetVelocity.y,
                yDrive.positionSpring,
                yDrive.positionDamper,
                yDrive.maximumForce,
                name,
                out isPositionMotor.y,
                out isLinearVelocityMotor.y
            );

            CheckPerAxis(
                targetPosition.z,
                targetVelocity.z,
                zDrive.positionSpring,
                zDrive.positionDamper,
                zDrive.maximumForce,
                name,
                out isPositionMotor.z,
                out isLinearVelocityMotor.z
            );

            maxForceForLinearMotor = new float3(xDrive.maximumForce, yDrive.maximumForce, zDrive.maximumForce);
        }

        private void IdentifyIfAngularMotor(
            UnityEngine.ConfigurableJoint joint,
            out bool3 isPositionMotor,
            out bool3 isVelocityMotor,
            out float3 maxForceForMotor
        )
        {
            // shortcuts for the code below
            ...
            CheckPerAxis(
                ...
                name,
                out isPositionMotor.x,
                out isVelocityMotor.x
            );

            CheckPerAxis(
                targetRotation.value.y,
                targetVelocity.y,
                yzDrive.positionSpring,
                yzDrive.positionDamper,
                yzDrive.maximumForce,
                name,
                out isPositionMotor.y,
                out isVelocityMotor.y
            );
        ...
        internal static void CheckPerAxis(
            ...
            float force,
            string name,
            out bool isPositionMotor,
            out bool isVelocityMotor
        )
        {
    ...
    class FixedBaker : BaseJointBaker<FixedJoint>
    {
        void ConvertFixedJoint(FixedJoint joint)
        {
            ...
            jointData.SetImpulseEventThresholdAllConstraints(
                joint.breakForce * Time.fixedDeltaTime,
                joint.breakTorque * Time.fixedDeltaTime
            );
            ...
            Entity entity = CreateJointEntity(worldIndex, GetConstrainedBodyPair(joint), solverType, jointData);
        }
    ...
    class HingeBaker : BaseJointBaker<HingeJoint>
    {
        void ConvertHingeJoint(HingeJoint joint)
        {
            ...
            ConvertSpringDamperSettings(
                joint.spring.spring,
                joint.spring.damper,
                GetConstrainedBodyInertia(joint),
                out var springFrequency,
                out var dampingRatio
            );
    ...
    class SpringBaker : BaseJointBaker<SpringJoint>
    {
        void ConvertSpringJoint(SpringJoint joint)
        {
            ...
            float3 jointAnchorB = joint.connectedBody
                ? GetScaledLocalAnchorPosition(joint.connectedBody.transform, joint.connectedAnchor)
                : joint.connectedAnchor;
            ...
            RigidTransform bFromBSource = isConnectedBodyConverted
                ? RigidTransform.identity
                : Math.DecomposeRigidBodyTransform(joint.connectedBody.transform.localToWorldMatrix);
            ...
            var springFrequency = JacobianUtilities.CalculateSpringFrequencyFromSpringConstant(
                joint.spring,
                GetConstrainedBodyMass(joint)
            );
            var dampingRatio = JacobianUtilities.CalculateDampingRatio(joint.spring, joint.damper, mass);

            var jointData = PhysicsJoint.CreateLimitedDistance(
                jointAnchorA,
                jointAnchorB,
                distanceRange,
