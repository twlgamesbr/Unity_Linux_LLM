    ...
    class Renderer : IDisposable
    {
...
#if UNITY_EDITOR
            resources.lineMaterial = AssetDatabase.LoadAssetAtPath<Material>(
                Path.Combine(debugDirName, $"{lineMaterialFileName}.mat")
            );
            resources.meshMaterial = AssetDatabase.LoadAssetAtPath<Material>(
                Path.Combine(debugDirName, $"{meshMaterialFileName}.mat")
            );
        ...
        void UpdateDynamicColorsData()
        {
            ...
            m_DrawData.m_ColorData[ColorIndex.DynamicMesh.value] = new float4(
                ...
                PhysicsVisualizationSettings.rigidbodyColor.a
            ); // Dynamic
            m_DrawData.m_ColorData[ColorIndex.StaticMesh.value] = new float4(
                ...
                PhysicsVisualizationSettings.staticColor.a
            ); // Static
            m_DrawData.m_ColorData[ColorIndex.KinematicMesh.value] = new float4(
                ...
                PhysicsVisualizationSettings.kinematicColor.a
            ); // Kinematic
#else
            m_Unmanaged.m_ColorData[ColorIndex.DynamicMesh.value] = m_Unmanaged.m_ColorData[ColorIndex.BrightRed.value];
            m_Unmanaged.m_ColorData[ColorIndex.StaticMesh.value] = m_Unmanaged.m_ColorData[
                ColorIndex.BrightBlack.value
            ];
            m_Unmanaged.m_ColorData[ColorIndex.KinematicMesh.value] = m_Unmanaged.m_ColorData[
                ColorIndex.BrightBlue.value
            ];
#endif
        }
        ...
        void CopyFromCpuToGpu()
        {
            ...
            // Line Buffer
            {
                var updateLineBuffer = RecreateBuffer<LineBuffer.Instance>(
                    ref m_LineVertexBuffer,
                    m_DrawData.m_LineBuffer.Size
                );
            ...
            // Triangle Buffer
            {
                var updateTriangleBuffer = RecreateBuffer<TriangleBuffer.Instance>(
                    ref m_TriangleInstanceBuffer,
                    m_DrawData.m_TriangleBuffer.Size
                );
            ...
            var scales = new float4(
                1.0f / FractionalCellsWide,
                1.0f / FractionalCellsTall,
                1.0f / PixelsWide,
                1.0f / PixelsTall
            );
        ...
        bool RecreateBuffer<T>(ref ComputeBuffer computeBuffer, int expectedLength)
            where T : struct
        {
            bool updateBuffer = computeBuffer == null || computeBuffer.count != expectedLength;
            if (updateBuffer)
            {
                ...
                computeBuffer = new ComputeBuffer(expectedLength, UnsafeUtility.SizeOf<T>());
            }
