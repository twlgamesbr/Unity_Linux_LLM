            ...
            if (m_SerializedVersion < k_LatestVersion)
            {
                ...
                if (PrefabUtility.IsPartOfAnyPrefab(this) || gameObject.scene.IsValid())
                {
                    ...
                    if (string.IsNullOrEmpty(scenePath))
                    {
                        ...
                        Debug.LogWarning(
                            "A Force Unique Collider component in game object '"
                                + gameObjectPath
                    ...
                    {
                        Debug.LogWarning(
                            "A Force Unique Collider component in a scene needs to be upgraded. To apply the upgrade, open and re-save the scene '"
                                + scenePath
                                + "', "
                                + "or use the automatic version upgrade tool under 'Tools -> Unity Physics -> Upgrade Force Unique Collider Versions'"
                        );
