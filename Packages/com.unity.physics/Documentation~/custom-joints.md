...
Finally, the optional [`PhysicsSolverType`](xref:Unity.Physics.PhysicsSolverType) component can be added to a joint entity in order to select the type of solver that should be used to simulate the joint's dynamics. If this component is not present, the default **Iterative Solver** will be used, which provides fast and approximate behavior. Instead, with this component, the **Direct Solver** can be selected, which yields accurate results but is more computationally demanding. It is ideal for situations involving complex jointed mechanisms with many joints, as well as high mass or stiffness ratios and thereby allows creating more advanced physics-based game elements. See the [Constraint Solvers](constraint-solvers.md) section for more details.
