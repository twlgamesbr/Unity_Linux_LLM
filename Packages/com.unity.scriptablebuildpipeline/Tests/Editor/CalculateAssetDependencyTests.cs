...
";
        ...
        // Embedding this model in code and only creating it when the test actually runs so it doesn't exist outside tests.
        string kTestModel = @"g cube
...
";
        ...
        private void CreateTestModel(string path)
        {
            if (!File.Exists(path))
            {
                File.WriteAllText(path, kTestModel);
                AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport | ImportAssetOptions.ForceUpdate);
            }
        }
