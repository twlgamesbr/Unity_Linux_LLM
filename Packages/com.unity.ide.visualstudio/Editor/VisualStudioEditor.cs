...
namespace Microsoft.Unity.VisualStudio.Editor
{
    ...
    public class VisualStudioEditor : IExternalCodeEditor
    {
        CodeEditor.Installation[] IExternalCodeEditor.Installations =>
            _discoverInstallations.Result.Values.Select(v => v.ToCodeEditorInstallation()).ToArray();
        ...
        static VisualStudioEditor()
        {
            ...
            _discoverInstallations = AsyncOperation<Dictionary<string, IVisualStudioInstallation>>.Run(
        ...
        internal static bool IsEnabled =>
            CodeEditor.CurrentEditor is VisualStudioEditor && UnityInstallation.IsMainUnityEditorProcess;
        ...
        {
            if (
                !TryGetVisualStudioInstallationForPath(CodeEditor.CurrentEditorInstallation, true, out var installation)
            )
                return;
        ...
        internal virtual bool TryGetVisualStudioInstallationForPath(
            string editorPath,
            bool lookupDiscoveredInstallations,
            out IVisualStudioInstallation installation
        )
        {
            ...
            if (
                lookupDiscoveredInstallations && _discoverInstallations.Result.TryGetValue(editorPath, out installation)
            )
                return true;
        ...
        {
            var result = TryGetVisualStudioInstallationForPath(
                editorPath,
                lookupDiscoveredInstallations: false,
                out var vsi
            );
        ...
        public void OnGUI()
        {
            ...
            var style = new GUIStyle { richText = true, margin = new RectOffset(0, 4, 0, 0) };

            GUILayout.Label(
                $"<size=10><color=grey>{package.displayName} v{package.version} enabled</color></size>",
                style
            );
        ...
        private static void SettingsButton(
            ...
            string guiMessage,
            string toolTip,
            IVisualStudioInstallation installation
        )
        {
            var generator = installation.ProjectGenerator;
        ...
        private static bool OpenFromInstallation(
            IVisualStudioInstallation installation,
            string path,
            int line,
            int column
        ...
        private static bool IsProjectGeneratedFor(
            string path,
        ...
        )
        {
            ...
            var relativePath = path.NormalizeWindowsToUnix()
