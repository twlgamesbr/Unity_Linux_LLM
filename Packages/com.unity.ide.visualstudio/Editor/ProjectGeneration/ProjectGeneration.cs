...
namespace Microsoft.Unity.VisualStudio.Editor
{
    ...
    public class ProjectGeneration : IGenerator
    {
        ...
        const string m_SolutionProjectEntryTemplate =
        ...
        public bool SyncIfNeeded(IEnumerable<string> affectedFiles, IEnumerable<string> reimportedFiles)
        {
            using (solutionSyncMarker.Auto())
            {
                ...
                var affectedNames = affected
                    ...
                    .Select(name => name.Split(new[] { ".dll" }, StringSplitOptions.RemoveEmptyEntries)[0]);
        ...
        private void RefreshCurrentInstallation()
        {
            ...
            editor?.TryGetVisualStudioInstallationForPath(
                CodeEditor.CurrentEditorInstallation,
                lookupDiscoveredInstallations: true,
        ...
        {
            _supportedExtensions = new HashSet<string> { "dll", "asmdef", "additionalfile" };
        ...
        private ResponseFileData[] ParseResponseFileData(Assembly assembly)
        {
            ...
            Dictionary<string, ResponseFileData> responseFilesData =
                assembly.compilerOptions.ResponseFiles.ToDictionary(
                    x => x,
                    x => m_AssemblyNameProvider.ParseResponseFile(x, ProjectDirectory, systemReferenceDirectories)
                );
        ...
        private Dictionary<string, string> GenerateAllAssetProjectParts()
        {
            ...
            foreach (string asset in m_AssemblyNameProvider.GetAllAssetPaths())
            {
                ...
                if (
                    IsSupportedFile(asset, out var extensionWithoutDot)
        ...
        internal virtual void IncludeAsset(StringBuilder builder, IncludeAssetTag tag, string asset)
        {
            ...
            if (Path.IsPathRooted(filename) && packageInfo != null)
            {
                ...
                var linkPath = XmlFilename(
                    SkipPathPrefix(asset.NormalizePathSeparators(), packageInfo.assetPath.NormalizePathSeparators())
                );
        ...
        )
        {
            ...
            var internalAssemblyReferences = assembly
                .assemblyReferences.Where(i => !i.sourceFiles.Any(ShouldFileBePartOfSolution))
                .Select(i => i.outputPath);
            var allReferences = assembly
                .compiledAssemblyReferences.Union(responseRefs)
                .Union(references)
            ...
            if (0 < assembly.assemblyReferences.Length)
            {
                projectBuilder.Append("  <ItemGroup>").Append(k_WindowsNewline);
                foreach (
                    var reference in assembly.assemblyReferences.Where(i =>
        ...
        internal virtual void AppendProjectReference(
            Assembly assembly,
            Assembly reference,
        ...
        {
            return Path.Combine(
                ProjectDirectory.NormalizePathSeparators(),
        ...
        private static IEnumerable<string> GetOtherArguments(ResponseFileData[] responseFileData, string name)
        {
            ...
            foreach (var argument in lines)
            {
                ...
                var key = argument.Substring(1, index - 1).Trim();
        ...
        private void SetAnalyzerAndSourceGeneratorProperties(
            Assembly assembly,
            ResponseFileData[] responseFileData,
        ...
        {
            return path.MakeAbsolutePath().NormalizePathSeparators();
        }

        private string[] ToNormalizedPaths(IEnumerable<string> values)
        {
            return values.Where(a => !string.IsNullOrEmpty(a)).Select(a => ToNormalizedPath(a)).Distinct().ToArray();
        }
        ...
        internal void GetProjectHeaderProperties(ProjectProperties properties, StringBuilder headerBuilder)
        {
            ...
            headerBuilder
                ...
                .Append(string.Join(";", properties.Defines))
                .Append(@"</DefineConstants>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <AllowUnsafeBlocks>")
                .Append(properties.Unsafe)
                .Append(@"</AllowUnsafeBlocks>")
                .Append(k_WindowsNewline);
            headerBuilder.Append(@"  </PropertyGroup>").Append(k_WindowsNewline);
        }
        ...
        internal static void GetProjectHeaderAnalyzers(ProjectProperties properties, StringBuilder headerBuilder)
        {
            if (!string.IsNullOrEmpty(properties.RulesetPath))
            {
                headerBuilder.Append(@"  <PropertyGroup>").Append(k_WindowsNewline);
                headerBuilder
                    .Append(@"    <CodeAnalysisRuleSet>")
                    .Append(properties.RulesetPath)
                    .Append(@"</CodeAnalysisRuleSet>")
                    .Append(k_WindowsNewline);
                headerBuilder.Append(@"  </PropertyGroup>").Append(k_WindowsNewline);
            }
            ...
            if (!string.IsNullOrEmpty(properties.AnalyzerConfigPath))
            {
                headerBuilder.Append(@"  <ItemGroup>").Append(k_WindowsNewline);
                headerBuilder
                    .Append(@"    <EditorConfigFiles Include=""")
                    .Append(properties.AnalyzerConfigPath)
                    .Append(@""" />")
                    .Append(k_WindowsNewline);
                headerBuilder.Append(@"  </ItemGroup>").Append(k_WindowsNewline);
            }
        ...
        internal void GetProjectHeaderVstuFlavoring(
            ProjectProperties properties,
        ...
        )
        {
            ...
            headerBuilder
                .Append(@"    <UnityProjectGenerator>Package</UnityProjectGenerator>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <UnityProjectGeneratorVersion>")
                .Append(properties.FlavoringPackageVersion)
                .Append(@"</UnityProjectGeneratorVersion>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <UnityProjectGeneratorStyle>")
                .Append(Style)
                .Append("</UnityProjectGeneratorStyle>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <UnityProjectType>")
                .Append(properties.FlavoringProjectType)
                .Append(@"</UnityProjectType>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <UnityBuildTarget>")
                .Append(properties.FlavoringBuildTarget)
                .Append(@"</UnityBuildTarget>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <UnityVersion>")
                .Append(properties.FlavoringUnityVersion)
                .Append(@"</UnityVersion>")
                .Append(k_WindowsNewline);
            headerBuilder.Append(@"  </PropertyGroup>").Append(k_WindowsNewline);
        }
        ...
        internal virtual string SolutionText(IEnumerable<Assembly> assemblies, Solution previousSolution = null)
        {
            ...
            string projectConfigurationsText = string.Join(
                k_WindowsNewline,
        ...
        internal List<SolutionProjectEntry> GetSolutionProjects(
        ...
        )
        {
            ...
            if (previousSolution != null)
            {
                ...
                var externalProjects = previousSolution
                    .Projects.Where(p =>
                        p.IsSolutionFolderProjectFactory() || !FileUtility.IsFileInProjectRootDirectory(p.FileName)
                    )
        ...
        private string GetProjectActiveConfigurations(string projectGuid)
        {
            ...
            return string.Format(solutionProjectConfigurationTemplate, projectGuid);
        }
