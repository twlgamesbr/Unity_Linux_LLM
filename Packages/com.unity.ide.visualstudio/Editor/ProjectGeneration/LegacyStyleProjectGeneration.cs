...
namespace Microsoft.Unity.VisualStudio.Editor
{
    internal class LegacyStyleProjectGeneration : ProjectGeneration
    {
        ...
        public LegacyStyleProjectGeneration(
            ...
            : base(tempDirectory, assemblyNameProvider, fileIoProvider, guidGenerator) { }

        public LegacyStyleProjectGeneration(string tempDirectory)
            : base(tempDirectory) { }
        ...
        internal override void GetProjectHeader(ProjectProperties properties, out StringBuilder headerBuilder)
        {
            ...
            headerBuilder
                .Append(@"    <TargetFrameworkVersion>v4.7.1</TargetFrameworkVersion>")
                .Append(k_WindowsNewline);
            ...
            headerBuilder
                ...
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <ImplicitlyExpandNETStandardFacades>false</ImplicitlyExpandNETStandardFacades>")
                .Append(k_WindowsNewline);
            headerBuilder
                .Append(@"    <ImplicitlyExpandDesignTimeFacades>false</ImplicitlyExpandDesignTimeFacades>")
                .Append(k_WindowsNewline);
            headerBuilder.Append(@"  </PropertyGroup>").Append(k_WindowsNewline);
        ...
        internal static void GetProjectHeaderConfigurations(ProjectProperties properties, StringBuilder headerBuilder)
        {
            ...
            headerBuilder
                .Append($"    <OutputPath>{@"Temp\bin\Release\".NormalizePathSeparators()}</OutputPath>")
                .Append(k_WindowsNewline);
            headerBuilder.Append(@"    <ErrorReport>prompt</ErrorReport>").Append(k_WindowsNewline);
            headerBuilder.Append(@"    <WarningLevel>4</WarningLevel>").Append(k_WindowsNewline);
            headerBuilder.Append(@"  </PropertyGroup>").Append(k_WindowsNewline);
        ...
        )
        {
            ...
            projectBuilder
                .Append("      <Project>{")
                .Append(ProjectGuid(referenceName))
                .Append("}</Project>")
                .Append(k_WindowsNewline);
