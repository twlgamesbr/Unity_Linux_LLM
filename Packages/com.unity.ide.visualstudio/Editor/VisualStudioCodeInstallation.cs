    ...
    internal class VisualStudioCodeInstallation : VisualStudioInstallation
    {
        ...
        public static bool TryDiscoverInstallation(string editorPath, out IVisualStudioInstallation installation)
        {
            ...
            installation = new VisualStudioCodeInstallation()
            {
                ...
                Name =
                    "Visual Studio Code"
                    + (isPrerelease ? " - Insider" : string.Empty)
                    + (version != null ? $" [{version.ToString(3)}]" : string.Empty),
                Path = editorPath,
...
#if UNITY_EDITOR_WIN
            var localAppPath = IOPath.Combine(
        ...
        private static readonly Regex DesktopFileExecEntry = new Regex(
            @"Exec=(\S+)",
...
"
    ...
    ""dotnet.defaultSolution"": """
                + IOPath.GetFileName(ProjectGenerator.SolutionFile())
                + @""",
