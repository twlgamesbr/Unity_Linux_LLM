...
namespace EditorAttributes.Editor
{
    ...
    public class ApplyMaterialDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            MemberInfo materialMemberInfo = ReflectionUtils.GetValidMemberInfo(
                applyMaterialAttribute.MaterialMemberName,
                property
            );
            ...
            if (materialMemberType != typeof(Material))
                return new HelpBox(
                    $"<b>{applyMaterialAttribute.MaterialMemberName}</b> is not a valid Material",
                    HelpBoxMessageType.Error
                );
            ...
            UpdateVisualElement(
                ...
                () =>
                {
                    var materialMemberValue =
                        ReflectionUtils.GetMemberInfoValue(materialMemberInfo, property) as Material;

                    propertyField.style.unityMaterial =
                        materialMemberValue == null
...
#else
            return new HelpBox(
                "ApplyMaterial Attribute requires <b>Unity 6.3</b> or newer",
