...
internal class UnsafeQueueTests_InJobs : CollectionsTestCommonBase
{
    ...
    {
        public UnsafeQueue<int>.ParallelWriter queue;
    ...
    public void Enqueue()
    {
        ...
        for (int i = 0; i < queueSize; ++i)
        {
            ...
            Assert.IsTrue(allValues.TryAdd(enqueued), "Job enqueued same value multiple times");
        }
    ...
    public void EnqueueDequeueMultipleQueuesInMultipleJobs()
    {
        ...
        for (int i = 0; i < 10; ++i)
        {
            var q1 = new UnsafeQueue<int>(CommonRwdAllocator.Handle);
            var q2 = new UnsafeQueue<int>(CommonRwdAllocator.Handle);
            var q3 = new UnsafeQueue<int>(CommonRwdAllocator.Handle);
            var q4 = new UnsafeQueue<int>(CommonRwdAllocator.Handle);
    ...
    {
        using (var queue = new UnsafeQueue<int>(CommonRwdAllocator.Handle))
        {
            new EnqueueJob
            {
                Queue = queue.AsParallelWriter()
    ...
    [Test]
    public void UnsafeQueue_ParallelWriter()
    {
        const int queueSize = 100 * 1024;
        var queue = new UnsafeQueue<int>(CommonRwdAllocator.Handle);
        var writeStatus = CollectionHelper.CreateNativeArray<int>(queueSize, CommonRwdAllocator.Handle);
