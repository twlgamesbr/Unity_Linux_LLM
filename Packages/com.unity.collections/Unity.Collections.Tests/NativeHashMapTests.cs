...
#pragma warning restore 0649
    ...
    {
        using var container = new UnsafeHashMap<ulong, ulong>(4, CommonRwdAllocator.Handle);
        Assert.DoesNotThrow(() => container.Add(0, 5));
#if ENABLE_UNITY_COLLECTIONS_CHECKS || UNITY_DOTS_DEBUG
        Assert.Throws<ArgumentException>(() => container.Add(0, 6));
#else
        Assert.DoesNotThrow(() => container.Add(0, 6)); // Ensures the write gracefully fails when collections checks are disabled.
#endif
        Assert.IsFalse(container.TryAdd(0, 7));
        Assert.AreEqual(container[0], 5);
    }

    [Test]
    public void NativeHashMap_Resize_Via_Add_Works()
    {
