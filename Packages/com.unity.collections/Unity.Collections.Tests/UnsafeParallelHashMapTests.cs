...
internal class UnsafeParallelHashMapTests : CollectionsTestCommonBase
{
    ...
    [Test]
    public void UnsafeParallelHashMap_HashMapSameKey()
    {
        using (var hashMap = new UnsafeParallelHashMap<int, int>(4, Allocator.Persistent))
        {
    ...
    [Test]
    public void UnsafeParallelHashMap_Resize_Via_Add_Works()
    {
        using var hashMap = new UnsafeParallelHashMap<int, int>(0, Allocator.Persistent);
        int target = hashMap.Capacity + 2;
    ...
    [Test]
    public void UnsafeParallelHashMap_ToKeyValueArrays()
    {
        using (var set = new UnsafeParallelHashMap<int, int>(8, CommonRwdAllocator.Handle) { {3,3}, {0,0}, {1,1}, {2,2}, })
        {
            var keys = set.GetKeyArray(CommonRwdAllocator.Handle);
            keys.Sort();
            for (int i = 0, num = set.Count(); i < num; i++)
            {
                Assert.AreEqual(keys[i], i);
            }
            var values = set.GetValueArray(CommonRwdAllocator.Handle);
            values.Sort();
            for (int i = 0, num = set.Count(); i < num; i++)
            {
                Assert.AreEqual(values[i], i);
            }
            var keyValueArray = set.GetKeyValueArrays(CommonRwdAllocator.Handle);
