...
namespace UnityEngine.InputSystem.Plugins.InputForUI
{
    ...
    internal class InputActionAssetVerifier : ProjectWideActionsAsset.IInputActionAssetVerifier
    {
        ...
        )
        {
            ...
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.PointAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: nameof(Vector2)
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.MoveAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: nameof(Vector2)
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.SubmitAction,
                actionType: InputActionType.Button,
                expectedControlType: "Button"
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.CancelAction,
                actionType: InputActionType.Button,
                expectedControlType: "Button"
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.LeftClickAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: "Button"
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.MiddleClickAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: "Button"
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.RightClickAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: "Button"
            );
            context.Verify(
                actionNameOrId: InputSystemProvider.Actions.ScrollWheelAction,
                actionType: InputActionType.PassThrough,
                expectedControlType: nameof(Vector2)
            );
        }
        ...
        private struct Context
        {
            ...
            {
                reporter.Report(
                    $"InputActionMap with path '{actionMap}' in asset \"{GetAssetReference()}\" {problem}. {errorSuffix}"
                );
            }
            ...
            {
                reporter.Report(
                    $"InputAction with path '{actionNameOrId}' in asset \"{GetAssetReference()}\" {problem}. {errorSuffix}"
                );
            }
            ...
            )
            {
                ...
                if (action == null)
                {
                    ...
                    ActionWarning(
                        actionNameOrId,
                        $"has 'type' set to '{nameof(InputActionType)}.{action.type}', but '{nameof(InputActionType)}.{actionType}' was expected"
                    );
                else if (
                    ...
                    ActionWarning(
                        actionNameOrId,
                        $"has 'expectedControlType' set to '{action.expectedControlType}', but '{expectedControlType}' was expected"
                    );
            }
