...
namespace UnityEngine.InputSystem.Editor
{
    ...
    public static class InputActionCodeGenerator
    {
        ...
        private const string kClassExample =
...
#region Interface implementation of MyActions.IPlayerActions
    ...
    public void OnAttack(InputAction.CallbackContext context)
    {
        ...
        public static string GenerateWrapperCode(InputActionAsset asset, Options options = default)
        {
            ...
            var writer = new Writer { buffer = new StringBuilder() };
            ...
            // Begin class.
            writer.DocSummary(
                $"Provides programmatic access to <see cref=\"InputActionAsset\" />, "
            ...
            foreach (var map in maps)
            {
                ...
                foreach (var action in map.actions)
                {
                    ...
                    writer.WriteLine(
                        $"m_{mapName}_{actionName} = m_{mapName}.FindAction(\"{action.name}\", throwIfNotFound: true);"
                    );
                }
            }
            writer.EndBlock();
            ...
            foreach (var map in maps)
            {
                var mapName = CSharpCodeHelpers.MakeIdentifier(map.name);
                writer.WriteLine(
                    $"UnityEngine.Debug.Assert(!m_{mapName}.enabled, \"This will cause a leak and performance issues, {options.className}.{mapName}.Disable() has not been called.\");"
                );
            }
            writer.EndBlock();
            writer.WriteLine();
            ...
            writer.DocInherit(
                classNamePrefix + nameof(InputActionAsset.FindBinding) + "(InputBinding, out InputAction)"
            );
            ...
            foreach (var map in maps)
            {
                ...
                writer.WriteLine(
                    string.Format("private List<I{0}> m_{0}CallbackInterfaces = new List<I{0}>();", mapTypeName)
                );
                ...
                foreach (var action in map.actions)
                {
                    ...
                    writer.WriteLine($"public InputAction @{actionName} => m_Wrapper.m_{mapName}_{actionName};");
                }
                ...
                writer.DocSummary(
                    $"Implicitly converts an <see ref=\"{mapTypeName}\" /> to an <see ref=\"InputActionMap\" /> instance."
                );
                writer.WriteLine(
                ...
                writer.WriteLine(
                    $"if (instance == null || m_Wrapper.m_{mapTypeName}CallbackInterfaces.Contains(instance)) return;"
                );
                ...
                writer.DocSummary(
                    $"Unregisters <param cref=\"instance\" /> and unregisters all input action callbacks via <see cref=\"{mapTypeName}.UnregisterCallbacks(I{mapTypeName})\" />."
                );
                ...
                writer.DocSummary(
                    $"Provides a new <see cref=\"{mapTypeName}\" /> instance referencing this action map."
                );
            ...
            foreach (var scheme in schemes)
            {
                ...
                writer.BeginBlock();
                writer.WriteLine(
                    $"if (m_{identifier}SchemeIndex == -1) m_{identifier}SchemeIndex = asset.FindControlSchemeIndex(\"{scheme.name}\");"
                );
            ...
            foreach (var map in maps)
            {
                ...
                writer.DocSummary(
                    $"Interface to implement callback methods for all input action callbacks associated with input actions defined by \"{map.name}\" which allows adding and removing callbacks."
                );
                ...
                foreach (var action in map.actions)
                {
                    ...
                    writer.DocSummary(
                        $"Method invoked when associated input action \"{action.name}\" is either <see cref=\"UnityEngine.InputSystem.InputAction.started\" />, <see cref=\"UnityEngine.InputSystem.InputAction.performed\" /> or <see cref=\"UnityEngine.InputSystem.InputAction.canceled\" />."
                    );
