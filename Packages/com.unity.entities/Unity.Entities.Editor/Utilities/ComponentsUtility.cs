...
namespace Unity.Entities.Editor
{
    static class ComponentsUtility
    {
        ...
        public static IEnumerable<ComponentViewData> GetComponentDataFromQuery(this EntityQuery query)
        {
            ...
            return desc
                .All.Concat(desc.Any)
                .Select(t => new ComponentViewData(
                    t.GetManagedType(),
                    TypeUtility.GetTypeDisplayName(t.GetManagedType()),
                    t.AccessModeType,
                    GetComponentKind(t)
                ))
                .Concat(
                    desc.None.Select(t => new ComponentViewData(
                        t.GetManagedType(),
                        TypeUtility.GetTypeDisplayName(t.GetManagedType()),
                        ComponentType.AccessMode.Exclude,
                        GetComponentKind(t)
                    ))
                )
                .Concat(
                    desc.Disabled.Select(t => new ComponentViewData(
                        t.GetManagedType(),
                        TypeUtility.GetTypeDisplayName(t.GetManagedType()),
                        ComponentType.AccessMode.ReadWrite,
                        GetComponentKind(t),
                        ComponentViewData.QueryOptions.Disabled
                    ))
                )
                .Concat(
                    desc.Present.Select(t => new ComponentViewData(
                        t.GetManagedType(),
                        TypeUtility.GetTypeDisplayName(t.GetManagedType()),
                        ComponentType.AccessMode.ReadWrite,
                        GetComponentKind(t),
                        ComponentViewData.QueryOptions.Present
                    ))
                )
                .Concat(
                    desc.Absent.Select(t => new ComponentViewData(
                        t.GetManagedType(),
                        TypeUtility.GetTypeDisplayName(t.GetManagedType()),
                        ComponentType.AccessMode.ReadWrite,
                        GetComponentKind(t),
                        ComponentViewData.QueryOptions.Absent
                    ))
                )
                .OrderBy(x => x);
        }

        public static ComponentViewData.ComponentKind GetComponentKind(ComponentType componentType) =>
