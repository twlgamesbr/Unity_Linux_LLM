    ...
    class ComponentMatchingSystems
    {
        ...
        unsafe void GetMatchingSystems(List<SystemQueriesViewData> systems)
        {
            foreach (var system in m_AllSystemProxies)
            {
                ...
                using var _ = ListPool<QueryViewData>.Get(out var queryViewDataList);
                using var __ = ListPool<EntityQuery>.Get(out var entityQueriesList);
                var i = 0;
                foreach (var query in systemStatePtr->EntityQueries)
                {
                    ...
                    {
                        entityQueriesList.Add(query);
                        queryViewDataList.Add(new QueryViewData(i, query, system, m_World));
                    }
                }
