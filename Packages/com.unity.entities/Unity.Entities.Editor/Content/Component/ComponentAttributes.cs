...
namespace Unity.Entities.Editor
{
    ...
    class ComponentAttributesInspector : PropertyInspector<ComponentAttributes>
    {
        ...
        public override VisualElement Build()
        {
            ...
            attributesSection.Add(
                new ComponentAttributeView(Strings.EnableableLabel, TypeManager.IsEnableable(typeInfo.TypeIndex))
            );
            ...
            if (typeInfo.Category == TypeManager.TypeCategory.BufferData)
            {
                ...
                attributesSection.Add(
                    new ComponentAttributeView(Strings.BufferCapacityLabel, typeInfo.BufferCapacity.ToString())
                );
                attributesSection.Add(
                    new ComponentAttributeView(
                        Strings.BufferOverheadLabel,
                        FormattingUtility.BytesToString((ulong)bufferHeaderSize)
                    )
                );
            }
            ...
            attributesSection.Add(
                new ComponentAttributeView(
                    Strings.TypeSizeLabel,
                    FormattingUtility.BytesToString((ulong)typeInfo.TypeSize)
                )
            );
            attributesSection.Add(
                new ComponentAttributeView(
                    Strings.SizeInChunkLabel,
                    FormattingUtility.BytesToString((ulong)typeInfo.SizeInChunk)
                )
            );
            attributesSection.Add(
                new ComponentAttributeView(
                    Strings.ComponentAlignmentInChunkLabel,
                    FormattingUtility.BytesToString((ulong)typeInfo.AlignmentInChunkInBytes)
                )
            );
    ...
    {
        public ComponentAttributeView(string label, bool value)
            : this(label, value.ToString()) { }

        public ComponentAttributeView(string label, int value)
            : this(label, value.ToString()) { }

        public ComponentAttributeView(string label, ulong value)
            : this(label, value.ToString()) { }
