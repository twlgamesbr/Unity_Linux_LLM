...
class RuntimeContentManagerProfilerModule : ProfilerModule
{
    static readonly ProfilerCounterDescriptor[] k_Counters = new ProfilerCounterDescriptor[]
    {
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_LoadedObjectsCounterName,
        ...
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_LoadedArchivesCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_LoadObjectRequestsCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_ReleaseObjectRequestsCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(RuntimeContentManagerProfiler.k_ObjectRefsCounterName, ProfilerCategory.Loading),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_ProcessCommandsFrameTimeCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_UnloadSceneRequestsCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_LoadedScenesCounterName,
            ProfilerCategory.Loading
        ),
        new ProfilerCounterDescriptor(
            RuntimeContentManagerProfiler.k_LoadSceneRequestsCounterName,
            ProfilerCategory.Loading
        ),
    };

    public RuntimeContentManagerProfilerModule()
        : base(k_Counters) { }
...
class RuntimeContentManagerProfilerModuleView : ProfilerModuleViewController
{
    ...
    public RuntimeContentManagerProfilerModuleView(ProfilerWindow profilerWindow)
        : base(profilerWindow) { }
    ...
    protected override VisualElement CreateView()
    {
        ...
        columns.Add(
            new Column
            {
                bindHeader = e => (e as Label).text = "Content",
                makeHeader = MakeItemLabel,
                makeCell = MakeItemLabel,
                bindCell = BindCellId,
                destroyCell = DestroyLabel,
                width = 400,
            }
        );
        columns.Add(
            new Column
            {
                bindHeader = e => (e as Label).text = "Reference Count",
                makeHeader = MakeItemLabel,
                makeCell = MakeItemLabel,
                bindCell = BindCellValueRefCount,
                destroyCell = DestroyLabel,
                width = 20,
            }
        );
        columns.Add(
            new Column
            {
                bindHeader = e => (e as Label).text = "Reference Count Total",
                makeHeader = MakeItemLabel,
                makeCell = MakeItemLabel,
                bindCell = BindCellValueRefCountTotal,
                destroyCell = DestroyLabel,
                width = 20,
            }
        );
        columns.Add(
            new Column
            {
                bindHeader = e => (e as Label).text = "Details",
                makeHeader = MakeItemLabel,
                makeCell = MakeItemLabel,
                bindCell = BindCellDetail,
                destroyCell = DestroyLabel,
                width = 500,
            }
        );
    ...
    void BindCellDetail(VisualElement e, int index)
    {
        ...
        if (r.type == 2)
        {
            if (r.frameData.id.GenerationType == WeakReferenceGenerationType.SubSceneObjectReferences)
                (e as Label).text =
                    $"SubScene Objects Reference: {r.frameData.id.GlobalId.AssetGUID}:{r.frameData.id.GlobalId.SceneObjectIdentifier0}";
            else
    ...
    private void ReloadData(long frame)
    {
        ...
        {
            int itemId = 0;
            var archivesData = frameData.GetFrameMetaData<RuntimeContentManagerProfilerFrameData>(
                RuntimeContentManagerProfiler.Guid,
                0
            );
            var filesData = frameData.GetFrameMetaData<RuntimeContentManagerProfilerFrameData>(
                RuntimeContentManagerProfiler.Guid,
                1
            );
            var objectsData = frameData.GetFrameMetaData<RuntimeContentManagerProfilerFrameData>(
                RuntimeContentManagerProfiler.Guid,
                2
            );
            if (archivesData.IsCreated && archivesData.Length > 0)
            {
                var filesToObjects = new Dictionary<int, List<TreeViewItemData<RCMProfilerFrameUIData>>>(
                    filesData.Length
                );
                for (int i = 0; i < objectsData.Length; i++)
                {
                    var s = new RCMProfilerFrameUIData
                    {
                        frameData = objectsData[i],
                        type = 2,
                        totalRefCount = objectsData[i].refCount,
                    };
                    ...
                    if (!filesToObjects.TryGetValue(s.parent, out var fileObjectList))
                        filesToObjects.Add(
                            s.parent,
                            fileObjectList = new List<TreeViewItemData<RCMProfilerFrameUIData>>()
                        );
                    fileObjectList.Add(item);
                }

                var archivesToFiles = new Dictionary<int, List<TreeViewItemData<RCMProfilerFrameUIData>>>(
                    archivesData.Length
                );
                for (int i = 0; i < filesData.Length; i++)
                {
                    var s = new RCMProfilerFrameUIData
                    {
                        frameData = filesData[i],
                        type = 1,
                        totalRefCount = filesData[i].refCount,
                    };
                    ...
                    if (!archivesToFiles.TryGetValue(s.parent, out var archiveFileList))
                        archivesToFiles.Add(
                            s.parent,
                            archiveFileList = new List<TreeViewItemData<RCMProfilerFrameUIData>>()
                        );
                    archiveFileList.Add(item);
                }

                var archiveItemList = archivesData
                    .Select(o =>
                    {
                        var s = new RCMProfilerFrameUIData
                        {
            ...
            {
                var objectsItemList = objectsData
                    .Select(s => new TreeViewItemData<RCMProfilerFrameUIData>(
                        itemId++,
                        new RCMProfilerFrameUIData
                        {
                            frameData = s,
                            type = 2,
                            totalRefCount = s.refCount,
                        }
                    ))
                    .ToList();
