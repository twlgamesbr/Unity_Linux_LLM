...
namespace Unity.Entities.Editor
{
    partial class HierarchyContextMenu
    {
        void BuildSceneContextMenu(DropdownMenu menu, Scene scene)
        {
            ...
            {
                menu.AppendAction(
                    L10n.Tr("Set Active Scene"),
                    a => EditorSceneManager.SetActiveScene((Scene)a.userData),
                    a =>
                        hasMultipleScenes && SceneManagerBridge.CanSetAsActiveScene(scene)
            ...
            if (scene.isLoaded)
            {
                ...
                object userData = (
                    scene,
                    isNotPlayingAndSceneIsAuthoring: !EditorApplication.isPlaying
                        || EditorSceneManagerBridge.IsAuthoringScene(scene),
                    hasMultipleScenes
                );
                ...
                menu.AppendAction(
                    ...
                    a =>
                    {
                        ...
                        return isNotPlayingAndSceneIsAuthoring
                            ? DropdownMenuAction.Status.Normal
                            : DropdownMenuAction.Status.Disabled;
                    },
                    userData
                ...
                menu.AppendAction(
                    L10n.Tr("Save Scene As"),
                    SaveSceneAs,
                    a =>
                    {
                        var (scene, isNotPlayingAndSceneIsAuthoring, _) = ((Scene, bool, bool))a.userData;
                        return isNotPlayingAndSceneIsAuthoring && !scene.isSubScene
                            ? DropdownMenuAction.Status.Normal
                            : DropdownMenuAction.Status.Disabled;
                    },
                    userData
                ...
                menu.AppendAction(
                    ...
                    a =>
                    {
                        ...
                        return isNotPlayingAndSceneIsAuthoring && hasMultipleScenes
                            ? DropdownMenuAction.Status.Normal
                            : DropdownMenuAction.Status.Disabled;
                    },
                    userData
            ...
            if (!scene.isSubScene)
            {
                if (scene.isLoaded)
                {
                    ...
                    object userData = (
                        scene,
                        canUnloadScenes: !EditorApplication.isPlaying
                            && !string.IsNullOrEmpty(scene.path)
                            && SceneManager.sceneCount > 1
                    );

                    menu.AppendAction(
                        L10n.Tr("Unload Scene"),
                        UnloadScene,
                        a =>
                        {
                            var (_, canUnloadScenes) = ((Scene, bool))a.userData;
                            return canUnloadScenes
                                ? DropdownMenuAction.Status.Normal
                                : DropdownMenuAction.Status.Disabled;
                        },
                        userData
                ...
                // Remove
                menu.AppendAction(
                    L10n.Tr("Remove Scene"),
                    RemoveScene,
                    a =>
                        EditorApplication.isPlaying || SceneManager.sceneCount == 1
                            ? DropdownMenuAction.Status.Disabled
                            : DropdownMenuAction.Status.Normal,
                    scene
                );
            ...
            menu.AppendSeparator();
            menu.AppendAction(
                L10n.Tr("Select Scene Asset"),
                SelectSceneAsset,
                a =>
                    string.IsNullOrEmpty(((Scene)a.userData).path)
                        ? DropdownMenuAction.Status.Disabled
                        : DropdownMenuAction.Status.Normal,
                scene
            );
            ...
            menu.AppendSeparator();
            menu.AppendAction(
                L10n.Tr("Prefab/Check for Unused Overrides"),
        ...
        static void RemoveScene(DropdownMenuAction obj) => CloseScene(removeScene: true, (Scene)obj.userData);
        ...
        static bool UserAllowedDiscardingChanges(params Scene[] modifiedScenes)
        {
            ...
            string message = LocalizationDatabaseBridge.GetLocalizedString(
            ...
            return EditorUtility.DisplayDialog(
                title,
                message,
                LocalizationDatabaseBridge.GetLocalizedString("OK"),
                LocalizationDatabaseBridge.GetLocalizedString("Cancel")
            );
        }
