...
Click on the icon to the right of a system or component name (![Go to icon - a square with an arrow pointing diagonally top right.](images/editor-go-to.png)), to change the selection to that system or component. For systems, Unity highlights the system in the [Systems window](editor-systems-window.md). For components, Unity opens the [Component Inspector](editor-component-inspector.md) where possible.
