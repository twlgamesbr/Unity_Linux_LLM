...
> [!NOTE]
> `SystemAPI` doesn't provide per-entity accessors for [shared components](components-shared.md) such as `GetComponent` or `SetComponent`. To read or write shared component data, use [`EntityManager.GetSharedComponent`](xref:Unity.Entities.EntityManager.GetSharedComponent*) and [`EntityManager.SetSharedComponent`](xref:Unity.Entities.EntityManager.SetSharedComponent*). To filter entities by a shared component value in a query, use [`WithSharedComponentFilter`](systems-systemapi-query.md). The `GetSharedComponentTypeHandle` API listed above is for chunk-level access in [`IJobChunk`](iterating-data-ijobchunk.md) jobs.
