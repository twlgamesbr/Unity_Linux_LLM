...
The top of the tab displays the query number, which is the order in which the query is declared in the C# system definition. To see more information about any component or entity in this view, select it and the Inspector displays more information about it. To see information about the system associated with this query, click the icon next to the system name (![Go to icon - a square with an arrow pointing diagonally top right.](images/editor-go-to.png)) which highlights the system in the [Systems window](editor-systems-window.md).
