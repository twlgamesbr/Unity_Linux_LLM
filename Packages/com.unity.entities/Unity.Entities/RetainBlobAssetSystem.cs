...
namespace Unity.Entities
{
    ...
    partial class RetainBlobAssetSystem : SystemBase
    {
        ...
        {
            foreach (
                var (blobOwner, entity) in SystemAPI
                    .Query<BlobAssetOwner>()
                    .WithAll<RetainBlobAssets>()
                    .WithNone<RetainBlobAssetBatchPtr>()
                    .WithEntityAccess()
            )
            {
                ...
                EntityManager.AddComponentData(
                    entity,
                    new RetainBlobAssetBatchPtr { BlobAssetBatchPtr = blobOwner.BlobAssetBatchPtr }
                );
            }

            var retainBlobAssetBatchQuery = SystemAPI
                .QueryBuilder()
                .WithNone<BlobAssetOwner>()
                .WithAll<RetainBlobAssets>()
                .WithAll<RetainBlobAssetBatchPtr>()
                .Build();
            ...
            var retainBlobAssetsQuery = SystemAPI
                .QueryBuilder()
                .WithNone<BlobAssetOwner>()
                .WithAll<RetainBlobAssets>()
                .WithAll<RetainBlobAssetPtr>()
                .Build();
        ...
        {
            foreach (var retainPtr in SystemAPI.Query<RefRO<RetainBlobAssetBatchPtr>>().WithAll<RetainBlobAssets>())
            {
                BlobAssetBatch.Release(retainPtr.ValueRO.BlobAssetBatchPtr);
            }

            foreach (var retainPtr in SystemAPI.Query<RefRO<RetainBlobAssetPtr>>().WithAll<RetainBlobAssets>())
            {
