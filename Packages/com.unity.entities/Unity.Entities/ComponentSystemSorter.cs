...
namespace Unity.Entities
{
    ...
    internal class ComponentSystemSorter
    {
        ...
        internal static unsafe int LookupSystemElement(
            SystemTypeIndex typeIndex,
            NativeHashMap<SystemTypeIndex, int>* lookupDictionaryPtr
        )
        {
        ...
        )
        {
            ...
            var sortedElements = new UnsafeList<SystemElement>(elements.Length, Allocator.Temp);
            sortedElements.Length = elements.Length;
            var sortedIndices = new UnsafeList<int>(elements.Length, Allocator.Temp);
                ...
                for (int i = 0; i < elements.Length; i++)
                {
                    if (!tmp.Contains(elements[i].SystemTypeIndex))
                    {
                        ...
                        FindExactCycleInSystemGraph(
                            elements[i].SystemTypeIndex,
                            elements,
                            lookupDictionary,
                            badSystemTypeIndices->m_ListData
                        );
        ...
        )
        {
            ...
            {
                throw new InvalidOperationException(
                    "Internal error starting type index was bad couldn't find it in list"
                );
            }
        ...
        private static unsafe void PopulateSystemElementLookup(
            NativeHashMap<SystemTypeIndex, int>* lookupDictionary,
            UnsafeList<SystemElement> elements
        )
        {
        ...
        internal static unsafe void WarnAboutAnySystemAttributeBadness(int systemTypeIndex, ComponentSystemGroup group)
        {
            ...
            var updateInGroups = TypeManager.GetSystemAttributes(systemType, typeof(UpdateInGroupAttribute));
            Type groupType = null;
            if (group != null)
            {
            ...
            )
            {
                ...
                foreach (var attr in updates)
                {
                    ...
                    if (group != null && attrType.Name.Contains("Update"))
                    {
                            ...
                            for (int i = 0; i < group.m_UnmanagedSystemsToUpdate.Length; i++)
                            {
                                if (
                                    TypeManager.GetSystemType(
                                        group
                                            .World.Unmanaged.ResolveSystemState(group.m_UnmanagedSystemsToUpdate[i])
                                            ->m_SystemTypeIndex
                                    ) == targetType
                                )
                                {
                        ...
                        var thisBucket = ComponentSystemGroup.ComputeSystemOrdering(systemTypeIndex, groupTypeIndex);

                        var otherBucket = ComponentSystemGroup.ComputeSystemOrdering(
                            TypeManager.GetSystemTypeIndex(targetType),
                            groupTypeIndex
                        );
        ...
        private static bool CheckBeforeConstraints(
            int parentTypeIndex,
            TypeManager.SystemAttribute dep,
            int systemTypeIndex,
            out bool warn
        )
        {
        ...
        private static unsafe bool CheckAfterConstraints(
            int parentTypeIndex,
            TypeManager.SystemAttribute dep,
            int systemTypeIndex,
            out bool warn
