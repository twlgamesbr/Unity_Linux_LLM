...
namespace Unity.Entities
{
    public struct ImHexPatternEntitySceneBinaryWriter : IDisposable
    {
        ...
        HashSet<Type> m_ForwardDeclaredTypes;
        HashSet<string> m_ForwardDeclaredTypeNames;
        HashSet<string> m_AlreadyWrittenOuterVariableNames;

        public static unsafe ImHexPatternEntitySceneBinaryWriter Create(
            string fileName,
            string headerAppend = "",
        ...
        )
        {
            if (entityManager.GetUncheckedEntityDataAccess() == null)
                entityManager =
                    currentWorld?.EntityManager ?? World.DefaultGameObjectInjectionWorld?.EntityManager ?? default;
            ...
            var writer = new ImHexPatternEntitySceneBinaryWriter
            {
                ...
                m_ForwardDeclaredTypes = new HashSet<Type>(),
                m_ForwardDeclaredTypeNames = new HashSet<string>(),
        ...
        static string CreateStableTypeNameFunction(EntityManager entityManager)
        {
            ...
            writer.WriteLine(
                "/// StableTypeHash values are used to identify types in the binary format. This function maps the StableTypeHash to a human-readable name."
            );
            ...
            foreach (var componentType in allTypes)
            {
                ...
                writer.WriteLine(
                    $"\t({typeInfo.StableTypeHash}): name = \"{typeInfo.Type?.FullName ?? typeInfo.DebugTypeName.ToString()}\";"
                );
            }
        ...
        void WriteHeader(string headerAppend, string name)
        {
            m_IndentedTextWriter.WriteLine(
                $"// This file is auto-generated by ImHexPatternEntitySceneBinaryWriter for '{name}'"
            );
        ...
        public void WriteArrayOfTypeWithPosition<T>(
            string name,
            long position,
            int elementCount,
            string comment = ""
        ) => WriteArrayOfTypeWithPosition(typeof(T), name, position, elementCount, comment);

        public void WriteArrayOfTypeWithPosition(
            Type type,
            string name,
            long position,
            int elementCount,
        ...
        )
        {
            ...
            m_IndentedTextWriter.WriteLine(
                $"{GetImHexPatternNameForType(type)} {newName}[{elementCount}] @ {position}{GetCommentText(comment)};"
            );
        }
        ...
        void WriteImHexType(Type type)
        {
            ...
            foreach (var queuedType in typeQueue)
                WriteForwardDeclaration(queuedType);

            while (typeQueue.Count > 0)
                WriteType(typeQueue.Dequeue());
        }

        void WriteForwardDeclaration(Type type)
        {
            if (
                type.IsPrimitive
                || type.IsArray
                ...
                || type.IsGenericType
                || type.FullName == null
                || TypeHelper.IsFakeGeneric(type)
                || TypeHelper.IsChunkComponent(type)
            )
            {
                return;
            }

            // Fast path: skip if we've seen this exact type
            if (!m_ForwardDeclaredTypes.Add(type))
                return;

            // Different types can map to the same ImHex name (e.g. Outer+Inner vs Outer_Inner)
            var typeName = TypeHelper.GetImHexSupportedFullName(type);
            if (!m_ForwardDeclaredTypeNames.Add(typeName))
                return;

            m_IndentedTextWriter.WriteLine($"using {typeName};");
        }
        ...
        Queue<Type> CollectTypes(Type currentType)
        {
            ...
            while (processingStack.Count > 0)
            {
                ...
                var hasAddedSameTypeBucketToStack = CollectGenericTypes(
                    currentType,
                    currentType,
                    currentTypeBucket,
                    processedTypes,
                ...
                if (currentType.IsArray)
                {
                    ...
                    )
                    {
                        ...
                        hasAddedSameTypeBucketToStack |= CollectGenericTypes(
                            arrayType,
                            currentType,
                            currentTypeBucket,
                            processedTypes,
        ...
        bool CollectGenericTypes(
            Type type,
            Type currentType,
            TypeBucket currentTypeBucket,
        ...
        void WriteType(Type type)
        {
            switch (type)
            {
                ...
                case var _ when m_AlreadyWrittenTypeNames.Contains(GetWrittenTypeName(type)):
                    break;
                case var _ when type.IsPrimitive:
                    Debug.LogError(
                        $"Should not be hit, primitive type {TypeHelper.GetImHexSupportedFullName(type)} found in queue."
                    );
                    break;
                case var _ when type.IsArray:
                    Debug.LogError(
                        $"Should not be hit, array type {TypeHelper.GetImHexSupportedFullName(type)} found in queue."
                    );
                    break;
                ...
                case var _ when TypeHelper.IsUnityObjectRef(type):
                    ...
                    m_AlreadyWrittenTypeNames.Add(GetWrittenTypeName(type));
                    m_AlreadyWrittenTypes.Add(type);
                    break;
                ...
                default:
                    ...
                    foreach (var fieldInfo in type.GetFields())
                    {
                        ...
                        fieldSizeCounter += fieldInfo.FieldType.IsValueType
                            ? UnsafeUtility.SizeOf(fieldInfo.FieldType)
                            : GetSizeOfUnityClass(fieldInfo.FieldType);
                    }

                    m_IndentedTextWriter.Indent--;
                    var sizeOfType = type.IsValueType ? UnsafeUtility.SizeOf(type) : fieldSizeCounter;

                    m_IndentedTextWriter.WriteLine(
                        $"}} [[fixed_size({sizeOfType})]]{GetCommentText(GetPremadeComment(type))};\n"
                    );
                    m_AlreadyWrittenTypes.Add(type);
                    break;
            }
        ...
        static string GetWrittenTypeName(Type type)
        {
            if (type.IsGenericType || TypeHelper.IsFakeGeneric(type))
                return GetGenericTypeString(type, false);
        ...
        {
            writer.WriteLine(
                $"enum {TypeHelper.GetImHexSupportedFullName(type)} : {GetImHexPatternNameForPrimitiveType(type.GetEnumUnderlyingType())} {{"
            );
            writer.Indent++;
            var enumNames = type.GetEnumNames();
            for (var i = 0; i < enumNames.Length; ++i)
            {
                var enumName = TypeHelper.GetImHexSupportedName(enumNames[i]);
                writer.WriteLine(i < enumNames.Length - 1 ? $"{enumName}," : $"{enumName}");
            }
            writer.Indent--;
            writer.WriteLine("};");
        }
        ...
        static void WriteField(IndentedTextWriter writer, FieldInfo fieldInfo)
        {
            ...
            switch (fieldType)
            {
                case var _ when fieldType.IsPointer || fieldType == typeof(IntPtr):
                    writer.Write(
                        $"{GetImHexPatternNameForPointerType(fieldType)} *{TypeHelper.GetImHexSupportedName(fieldInfo.Name)} : {GetImHexPatternNameForPrimitiveType(typeof(int))}"
                    );
                    break;
                case var _ when fieldType.IsPrimitive:
                    writer.Write(
                        $"{GetImHexPatternNameForPrimitiveType(fieldType)} {TypeHelper.GetImHexSupportedName(fieldInfo.Name)}"
                    );
                    break;
                case var _ when TypeHelper.IsUnityObjectRef(fieldType):
                    writer.Write(
                        $"{TypeHelper.GetImHexSupportedFullName(fieldType)} {TypeHelper.GetImHexSupportedName(fieldInfo.Name)}"
                    );
                    break;
                case var _ when fieldType.IsGenericType || TypeHelper.IsFakeGeneric(fieldType):
                    writer.Write(
                        $"{GetGenericTypeString(fieldType, true)} {TypeHelper.GetImHexSupportedName(fieldInfo.Name)}"
                    );
                    break;
                ...
                case var _ when fieldType.IsArray:
                    ...
                    var typeName =
                        elementType != null && elementType.IsPrimitive
                            ? GetImHexPatternNameForPrimitiveType(elementType)
                            : TypeHelper.GetImHexSupportedFullName(elementType);
            ...
            writer.WriteLine(
                $" @ addressof(this)+{UnsafeUtility.GetFieldOffset(fieldInfo)}{GetCommentText(GetPremadeComment(fieldType, TypeHelper.GetImHexSupportedName(fieldInfo.Name)))};"
            );
        }

        static string GetPremadeComment(Type type, string fieldName = "") =>
            type switch
            {
                { Name: "UInt64" } when fieldName is "StableTypeHash" =>
                    "StableTypeHash is a unique identifier for the type, used for serialization and deserialization.",
                _ => string.Empty,
            };

        static string GetCommentText(string comment) =>
            string.IsNullOrEmpty(comment) ? string.Empty : $" [[comment(\"{comment}\")]]";

        public void WriteTypeWithPosition<T>(string name, long position, string comment = "") =>
            WriteTypeWithPosition(typeof(T), name, position, comment);
        ...
        public void WriteTypeWithPosition(Type type, string name, long position, string comment = "")
        {
            ...
            m_IndentedTextWriter.WriteLine(
                $"{GetImHexPatternNameForType(type)} {newName} @ {position}{GetCommentText(comment)};"
            );
        }
        ...
        static string GetImHexPatternNameForPrimitiveType(Type type) =>
            type switch
            {
                { Name: "Byte" } => "u8",
                { Name: "SByte" } => "s8",
                ...
                _ => throw new NotSupportedException(
                    $"Type '{TypeHelper.GetImHexSupportedFullName(type)}' is not a supported primitive by ImHexPatternEntitySceneBinaryWriter."
                ),
            };

        static string GetImHexPatternNameForPointerType(Type type) =>
            type switch
            {
                { Name: "Void*" } => "u32",
                { Name: "IntPtr" } => "u32",
        ...
        public void Dispose()
        {
            ...
            Debug.Log(
                $"Created an .hexpat file from the recent bake, located at: {filePath} (Size: {new FileInfo(filePath).Length} bytes)"
            );
            m_IndentedTextWriter.Dispose();
        }
        ...
        internal string GetOutput() => m_IndentedTextWriter.InnerWriter.ToString();

        // Struct shallow copy shares m_IndentedTextWriter; call this on the source after transferring ownership
        ...
        {
            static readonly HashSet<string> ImHexReservedKeywords = new HashSet<string>
            {
                "addr",
                "addressof",
                "as",
            ...
            public static string GetImHexSupportedName(string name)
            {
                ...
                if (bracketIdx >= 0 && bracketIdx < end)
                    end = bracketIdx;
                ...
                return ImHexReservedKeywords.Contains(result) ? result + "_" : result;
            }

            public static bool IsChunkComponent(Type type) =>
                type.FullName != null && type.FullName.Contains("Unity.Entities.Chunk+<Buffer>");

            public static bool IsUnityComponent(Type type) =>
                type.IsClass
                && (
            ...
            public static bool IsFakeGeneric(Type type) =>
                type is { FullName: not null } && type.FullName.Contains("`");
