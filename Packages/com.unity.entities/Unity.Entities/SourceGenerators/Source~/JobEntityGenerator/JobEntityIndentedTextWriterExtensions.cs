...
internal static class JobEntityIndentedTextWriterExtensions
{
    internal static void WriteInternalCompilerStruct(this IndentedTextWriter writer, bool requiresEntityManager)
    {
        ...
        if (requiresEntityManager)
            writer.WriteLine(
                "if(scheduleType == 2) throw new global::System.InvalidOperationException(\"Tried to ScheduleParallel a job with a managed execute signature. Please use .Run or .Schedule instead.\");"
            );
        writer.Indent--;
        writer.WriteLine("}");
        writer.Indent--;
    ...
    internal static void WriteExecuteMethod(
    ...
    )
    {
        ...
        writer.WriteLine(
            $"Execute({userExecuteMethodParameters.Select(param => param.ExecuteMethodArgumentValue).SeparateByComma()});"
        );
