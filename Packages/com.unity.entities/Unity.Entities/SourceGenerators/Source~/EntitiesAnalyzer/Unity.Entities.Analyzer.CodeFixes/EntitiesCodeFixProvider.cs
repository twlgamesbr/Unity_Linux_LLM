    ...
    public class EntitiesCodeFixProvider : CodeFixProvider
    {
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            foreach (var diagnostic in context.Diagnostics)
            {
                ...
                switch (node)
                {
                    case LocalDeclarationStatementSyntax { Declaration: { } variableDeclaration }
                        when diagnostic.Id == EntitiesDiagnostics.ID_EA0001:
                        context.RegisterCodeFix(
                            CodeAction.Create(
                                title: "Use non-readonly reference",
                                createChangedDocument: c =>
                                    MakeNonReadonlyReference(context.Document, variableDeclaration, c),
                    ...
                    {
                        if (
                            diagnostic.Id == EntitiesDiagnostics.ID_EA0007
                            || diagnostic.Id == EntitiesDiagnostics.ID_EA0008
                        )
                        {
                            context.RegisterCodeFix(
                                CodeAction.Create(
                        ...
                        else if (diagnostic.Id == EntitiesDiagnostics.ID_EA0010)
                        {
                            context.RegisterCodeFix(
                                CodeAction.Create(
                                    title: "Add BurstCompile attribute",
                                    createChangedDocument: c =>
                                        AddBurstCompilerAttribute(context.Document, typeDeclarationSyntax, c),
                    ...
                    case ElementAccessExpressionSyntax elementAccessExpression
                        when diagnostic.Id == CSharpCompilerDiagnostics.CS1654:
                    {
                        context.RegisterCodeFix(
                            CodeAction.Create(
                                title: "Store element in a new variable",
                                createChangedDocument: c =>
                                    AssignCurrentVariableToNewVariable_AndUseNewVariableInstead(
                                        context.Document,
        ...
        )
        {
            ...
            var newIdentifier = SyntaxFactory
                .IdentifierName($"__new{currentIdentifierText}__")
            ...
            var variableDeclaratorSyntax = SyntaxFactory.VariableDeclarator(
                ...
                initializer: SyntaxFactory.EqualsValueClause(
                    currentIdentifierName.WithLeadingTrivia(SyntaxFactory.Space)
                )
            );
            ...
            // and use its trivia.
            var variableDeclarationUseTriviaFrom = insertNewVariableDeclarationBefore
                .DescendantNodes()
                .OfType<IdentifierNameSyntax>()
                .First();
            var variableDeclarationStatement = SyntaxFactory
                .ParseStatement($"var {variableDeclaratorSyntax.ToString()};")
                .WithLeadingTrivia(variableDeclarationUseTriviaFrom.Identifier.LeadingTrivia)
                .WithTrailingTrivia(SyntaxFactory.EndOfLine("\n"));

            var newElementAccessNode = SyntaxFactory
                .ElementAccessExpression(newIdentifier, currentBracketedArgumentList)
                .WithTriviaFrom(currentIdentifierName);
            ...
            trackedRoot = trackedRoot.InsertNodesBefore(
                trackedStatement,
                new SyntaxNode[] { variableDeclarationStatement }
            );
            var trackedElementAccessNode = trackedRoot.GetCurrentNode(elementAccessExpression);
            trackedRoot = trackedRoot.ReplaceNode(
                trackedElementAccessNode,
                newElementAccessNode.WithTrailingTrivia(SyntaxFactory.Space)
            );
        ...
        static async Task<Document> AddPartial(
            Document document,
            TypeDeclarationSyntax typeDeclarationSyntax,
            CancellationToken cancellationToken
        )
        {
            var partialModifier = SyntaxFactory
                .Token(SyntaxKind.PartialKeyword)
                .WithTrailingTrivia(SyntaxFactory.Space);
            var modifiedSyntax = typeDeclarationSyntax
                .WithoutLeadingTrivia()
                .AddModifiers(partialModifier)
                .WithTriviaFrom(typeDeclarationSyntax);
        ...
        static async Task<Document> AddRef(
            Document document,
            ParameterSyntax typeDeclarationSyntax,
            CancellationToken cancellationToken
        )
        {
        ...
        static async Task<Document> MakeNonReadonlyReference(
            Document document,
        ...
        )
        {
            ...
            modifiedParam = modifiedParam.WithModifiers(
        ...
        )
        {
            ...
            if (modifiedVarDecl.Type is RefTypeSyntax overallRefType)
            {
                ...
                modifiedVarDecl = modifiedVarDecl.WithType(
                    overallRefType.WithReadOnlyKeyword(default).WithTriviaFrom(overallRefType)
                );
            }
            else
            {
                ...
                {
                    var modifiedValue = SyntaxFactory
                        .RefExpression(
                            modifiedVariables[index]
                                .Initializer.Value.WithoutTrivia()
                                .WithLeadingTrivia(SyntaxFactory.Space)
                        )
                        .WithTriviaFrom(modifiedVariables[index].Initializer.Value);
                    modifiedVariables[index] = modifiedVariables[index]
                        .WithInitializer(modifiedVariables[index].Initializer.WithValue(modifiedValue));
                }
        ...
        static async Task<Document> AddBurstCompilerAttribute(
            Document document,
            TypeDeclarationSyntax typeDeclarationSyntax,
            CancellationToken cancellationToken
        )
        {
            ...
            var burstCompilerAttributeList = SyntaxFactory
                .AttributeList(SyntaxFactory.SeparatedList(new[] { burstCompilerAttribute }))
                .NormalizeWhitespace()
