...
public readonly struct TypeHandleFieldDescription : IEquatable<TypeHandleFieldDescription>, IMemberDescription
{
    ...
    public string GetMemberAssignment() =>
        Source switch
        {
            TypeHandleSource.BufferElement =>
                $"{GeneratedFieldName} = state.GetBufferTypeHandle<{TypeSymbol.ToFullName()}>({(IsReadOnly ? "true" : "false")});",
            TypeHandleSource.Component => TypeSymbol.IsReferenceType
    ...
    public TypeHandleFieldDescription(ITypeSymbol typeSymbol, bool isReadOnly, TypeHandleSource forcedTypeHandleSource)
    {
        ...
        Debug.Assert(
            isSpecifiedTypeSymbol || (!isSpecifiedTypeSymbol && forcedTypeHandleSource != TypeHandleSource.None),
            "SG-DBG: Unspecified types require a forced type handle source"
        );
        ...
        else
        {
            ...
            switch (forcedTypeHandleSource)
            {
                case TypeHandleSource.BufferElement:
                    Debug.Assert(
                        constraintTypes.Any(t =>
                            t.ToFullName() == "global::Unity.Entities.IBufferElementData"
                            || t.InheritsFromInterface("Unity.Entities.IBufferElementData")
                        ),
                        "SG-DBG: Specified buffer element types must be buffer element types"
                    );
                    GeneratedFieldName = $"__{typeSymbolValidIdentifier}_{(IsReadOnly ? "RO" : "RW")}_BufferTypeHandle";
                    Source = TypeHandleSource.BufferElement;
                    break;
                case TypeHandleSource.SharedComponent:
                    Debug.Assert(
                        constraintTypes.Any(t =>
                            t.ToFullName() == "global::Unity.Entities.ISharedComponentData" || t.IsSharedComponent()
                        ),
                        "SG-DBG: Specified shared component types must be shared component types"
                    );
                    GeneratedFieldName = $"__{typeSymbolValidIdentifier}_SharedComponentTypeHandle";
                    Source = TypeHandleSource.SharedComponent;
                    break;
                default:
                    Debug.Assert(
                        constraintTypes.Any(t =>
                            t.ToFullName() == "global::Unity.Entities.IComponentData" || t.IsComponent()
                        ),
                        "SG-DBG: Specified component types must be component types"
                    );
                    GeneratedFieldName =
                        $"__{typeSymbolValidIdentifier}_{(IsReadOnly ? "RO" : "RW")}_ComponentTypeHandle";
                    Source = TypeHandleSource.Component;
                    break;
            }
        }
