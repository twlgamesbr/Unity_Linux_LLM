    ...
    public class Service<T>
    {
        ...
        public class ServiceRegistry
        {
            ...
            public IEnumerable<T> FIFO
            {
                get
                {
                    foreach (var v in m_Stack.Reverse())
                        yield return v;
                }
            }
        }

        static ThreadLocal<ServiceRegistry> s_Registry = new ThreadLocal<ServiceRegistry>(() =>
            new ServiceRegistry()
            {
                Name = $"Thread[{Thread.CurrentThread.ManagedThreadId}] '{Thread.CurrentThread.Name}'",
            }
        );
        ...
        /// <param name="service"></param>
        /// <returns></returns>
        public static ScopedDisposableInstance<T2> Scoped<T2>(T2 service)
            where T2 : T, IDisposable => new ScopedDisposableInstance<T2>(service);
    }
}
