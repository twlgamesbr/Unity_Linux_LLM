    ...
    public class SystemApiQueryBuilderModule : ISystemModule
    {
        ...
        public IEnumerable<(SyntaxNode SyntaxNode, TypeDeclarationSyntax SystemType)> Candidates =>
            _queryCandidates.Select(candidate =>
                (SyntaxNode: candidate.BuildNode, ContainingType: candidate.ContainingTypeNode)
            );
                ...
                switch (invocationExpressionSyntax.Expression)
                {
                    case MemberAccessExpressionSyntax memberAccessExpressionSyntax:
                    {
                        switch (memberAccessExpressionSyntax.Name)
                        {
                            ...
                            {
                                Identifier: { ValueText: "QueryBuilder" }
        ...
        public bool RegisterChangesInSystem(SystemDescription systemDescription)
        {
            ...
            var systemApiQueryBuilderDescriptionGroupedByBuildNodes =
                new Dictionary<SyntaxNode, SystemApiQueryBuilderDescription>();
            ...
            foreach (var queryCandidate in CandidatesGroupedByContainingSystemTypes[systemDescription.SystemTypeSyntax])
            {
                ...
                foreach (
                    var archetypeInfo in description.Archetypes.Zip(
                        description.QueryFinalizingLocations,
                        (a, l) => (Archetype: a, Location: l)
                    )
                )
                {
                    ...
                    {
                        description.Success &= QueryVerification.VerifyQueryTypeCorrectness(
                ...
                systemDescription.CandidateNodes.Add(
                    queryCandidate.BuildNode,
                    new CandidateSyntax(CandidateType.QueryBuilder, CandidateFlags.None, queryCandidate.BuildNode)
                );
            }

            foreach (var description in systemApiQueryBuilderDescriptions)
            {
                description.GeneratedEntityQueryFieldName = systemDescription.QueriesAndHandles.GetOrCreateQueryField(
                    new MultipleArchetypeQueryFieldDescription(
                        description.Archetypes.ToArray(),
                        description.GetQueryBuilderBodyBeforeBuild()
                    )
                );
            }
            systemDescription.SyntaxWalkers.Add(
                Module.SystemApiQueryBuilder,
                new SystemApiQueryBuilderSyntaxWalker(systemApiQueryBuilderDescriptionGroupedByBuildNodes)
            );
            return true;
        }
    }
