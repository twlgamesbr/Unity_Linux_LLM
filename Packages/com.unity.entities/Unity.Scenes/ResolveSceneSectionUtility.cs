...
namespace Unity.Scenes
{
    ...
    internal static class ResolveSceneSectionUtility
    {
        ...
        internal static bool ResolveSceneSections(
        ...
        )
        {
            ...
            var sceneHeaderPath = EntityScenesPaths.GetLoadPathFromArtifactPaths(
                paths,
                EntityScenesPaths.PathType.EntitiesHeader
            );
#else
            var sceneHeaderPath = EntityScenesPaths.FullPathForFile(
                sceneLoadDir,
...
#if UNITY_EDITOR
            SceneHeaderUtility.BuildSectionPaths(
                ref sectionPaths,
                ref sceneMetaData.Value,
                sceneGUID,
                paths,
                artifactHash
            );
#else
            SceneHeaderUtility.BuildSectionPathsForContentArchives(
                ref sectionPaths,
                ref sceneMetaData.Value,
                sceneGUID,
                sceneLoadDir
        ...
        internal static unsafe bool ResolveSceneSections(
            EntityManager entityManager,
            Entity sceneEntity,
            RequestSceneLoaded requestSceneLoaded,
            ref SceneMetaData sceneMetaData,
            ResolveSceneSectionArchetypes sectionArchetypes,
            UnsafeList<ResolvedSectionPath> sectionPaths,
            BlobAssetOwner headerBlobOwner
        )
        {
#if UNITY_EDITOR
            ...
            entityManager.AddComponent(
                sceneEntity,
                new ComponentTypeSet(typeof(ResolvedSectionEntity), typeof(LinkedEntityGroup))
            );
            ...
            entityManager.CreateEntity(
                loadSections ? sectionArchetypes.SectionEntityRequestLoad : sectionArchetypes.SectionEntityNoLoad,
                sectionEntities
            );
            ...
            for (int i = 0; i != sceneMetaData.Sections.Length; i++)
            {
                ...
                entityManager.SetComponentData(
                    sectionEntity,
                    new SceneBoundingVolume { Value = sceneMetaData.Sections[i].BoundingVolume }
                );
                ...
                if (entityManager.HasComponent<SceneTag>(sceneEntity))
                    entityManager.AddSharedComponent(
                        sectionEntity,
                        entityManager.GetSharedComponent<SceneTag>(sceneEntity)
                    );

                AddSectionMetadataComponents(
                    sectionEntity,
                    ref sceneMetaData.SceneSectionCustomMetadata[i],
                    entityManager
                );
        ...
        internal static unsafe bool ReadHeader(
            string sceneHeaderPath,
            out BlobAssetReference<SceneMetaData> sceneMetaDataRef,
            Hash128 sceneGUID
        ...
        internal static unsafe bool ReadHeader(
            ...
            out BlobAssetReference<SceneMetaData> sceneMetaDataRef,
            Hash128 sceneGUID,
            out BlobAssetOwner headerBlobOwner,
            bool readBlobAssetHeader = true
        )
        {
            ...
            {
                if (
                    !BlobAssetReference<SceneMetaData>.TryRead(
                        binaryReader,
...
#if UNITY_EDITOR
                    Debug.LogError(
                        $"Loading Entity Scene failed because the entity header file was an old version or doesn't exist: guid={sceneGUID} path={sceneHeaderPath}"
                    );
#else
                    Debug.LogError(
                        $"Loading Entity Scene failed because the entity header file was an old version or doesn't exist: {sceneGUID}\nNOTE: In order to load SubScenes in the player you have to use the new BuildConfiguration asset based workflow to build & run your player.\n{sceneHeaderPath}"
                    );
#endif
                }
            ...
            if (readBlobAssetHeader)
            {
                ...
                {
                    sceneMetaDataRef.Value.Sections[i].BlobHeader = sceneMetaDataRef
                        .Value.Sections[i]
        ...
        internal static unsafe void AddSectionMetadataComponents(
            Entity sectionEntity,
            ref BlobArray<SceneSectionCustomMetadata> sectionMetaDataArray,
            EntityManager entityManager
        )
        {
