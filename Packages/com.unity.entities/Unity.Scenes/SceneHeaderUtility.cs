...
namespace Unity.Scenes
{
    ...
    internal struct SceneHeaderUtility
    {
        ...
        internal static unsafe HeaderLoadResult FinishHeaderLoad(
            RequestSceneHeader requestHeader,
            Hash128 sceneGUID,
            string sceneLoadDir
        )
        {
        ...
        unsafe struct DeserializeHeaderJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                if (
                    !BlobAssetReference<SceneMetaData>.TryReadInplace(
                        headerData,
                        headerSize,
                ...
                {
                    sceneMetaData.Sections[i].BlobHeader = sceneMetaData
                        .Sections[i]
                        .BlobHeader.Resolve(headerBlobOwner);
                }
                ...
                BuildSectionPaths(
                    ref headerLoadResult.SectionPaths,
                    ref sceneMetaData,
                    SceneGUID,
                    managedHeaderData.Paths,
                    ArtifactHash
                );
#else
                BuildSectionPathsForContentArchives(
                    ref headerLoadResult.SectionPaths,
                    ref sceneMetaData,
                    SceneGUID,
                    HeaderData->SceneLoadDir.ToString()
                );
#endif
            }
        }

        internal static unsafe RequestSceneHeader CreateRequestSceneHeader(
            Hash128 sceneGUID,
...
#else
            string sceneHeaderPath = EntityScenesPaths.FullPathForFile(
                sceneLoadDir,
                EntityScenesPaths.RelativePathForSceneFile(sceneGUID, EntityScenesPaths.PathType.EntitiesHeader, -1)
            );
#endif
            var data = (byte*)
                Memory.Unmanaged.Allocate(SerializeUtility.MaxSubsceneHeaderSize, 64, Allocator.Persistent);
            headerData->ReadCommand = new ReadCommand
            {
                Size = SerializeUtility.MaxSubsceneHeaderSize,
                Offset = 0,
                Buffer = data,
            };
        ...
        public static void ScheduleHeaderLoadOnEntity(
        ...
        )
        {
            ...
            EntityManager.AddComponentData(
                sceneEntity,
                CreateRequestSceneHeader(sceneGUID, requestSceneLoaded, artifactHash, sceneLoadDir)
            );
        }

        internal static unsafe void BuildSectionPaths(
            ref UnsafeList<ResolvedSectionPath> sectionPaths,
            ref SceneMetaData sceneMetaData,
            Hash128 sceneGUID,
            RequestSceneHeader requestHeader,
            string sceneLoadDir,
            Hash128 artifactHash
        ...
        {
#if UNITY_EDITOR
            var managedHeaderData = (SceneHeaderUtility.ManagedHeaderData)
                requestHeader.HeaderData->ManagedHeaderData.Target;
        ...
        internal static void BuildSectionPathsForContentArchives(
            ref UnsafeList<ResolvedSectionPath> sectionPaths,
            ref SceneMetaData sceneMetaData,
            Hash128 sceneGUID,
            string sceneLoadDir
        )
        {
            ...
            for (int i = 0; i < sceneMetaData.Sections.Length; ++i)
            {
                ...
                var fullScenePath = EntityScenesPaths.FullPathForFile(
                    sceneLoadDir,
                    EntityScenesPaths.RelativePathForSceneFile(
                        sceneGUID,
        ...
        internal static UntypedWeakReferenceId CreateSceneSectionHash(
            Hash128 sceneGUID,
            int sectionIndex,
            Hash128 artifactHash
        )
...
#if UNITY_EDITOR
        internal static void BuildSectionPaths(
            ref UnsafeList<ResolvedSectionPath> sectionPaths,
            ref SceneMetaData sceneMetaData,
            Hash128 sceneGUID,
            string[] Paths,
        ...
        )
        {
            ...
            for (int i = 0; i < sceneMetaData.Sections.Length; ++i)
            {
                ...
                var scenePath = EntityScenesPaths.GetLoadPathFromArtifactPaths(
                    Paths,
                    EntityScenesPaths.PathType.EntitiesBinary,
                    sectionIndex
                );
                var hybridPath =
                    sceneMetaData.Sections[i].ObjectReferenceCount > 0
                        ? EntityScenesPaths.GetLoadPathFromArtifactPaths(
                            Paths,
        ...
        internal static void LogHeaderLoadError(HeaderLoadStatus status, Hash128 sceneGUID)
        {
            switch (status)
            {
                case HeaderLoadStatus.MissingFile:
                    ...
                    var logPath = System.IO.Path.GetFullPath(
                        System.IO.Path.Combine(UnityEngine.Application.dataPath, "../Logs")
                    );
                    Debug.LogError(
...
#else
                    Debug.LogError(
                        $"Loading Entity Scene failed because the entity header file couldn't be resolved: guid={sceneGUID}."
                    );
#endif
                    break;
...
#if UNITY_EDITOR
                    Debug.LogError(
                        $"Loading Entity Scene failed because the entity header file was an old version: guid={sceneGUID}"
                    );
#else
                    Debug.LogError(
                        $"Loading Entity Scene failed because the entity header file was an old version: {sceneGUID}\nNOTE: In order to load SubScenes in the player you have to use the new BuildConfiguration asset based workflow to build & run your player."
                    );
#endif
                    break;
            }
