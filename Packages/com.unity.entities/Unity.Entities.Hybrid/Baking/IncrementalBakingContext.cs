...
namespace Unity.Entities.Baking
{
    ...
    internal struct IncrementalBakingContext
    {
        ...
        static readonly ProfilerMarker _InitialBakeInstructionsMarker = new ProfilerMarker(
            _InitialBakeInstructionsMarkerStr
        );
        static readonly ProfilerMarker _IncrementalBakeInstructionsMarker = new ProfilerMarker(
            _IncrementalBakeInstructionsMarkerStr
        );
        static readonly ProfilerMarker _AdditionalObjectsInstructionsMarker = new ProfilerMarker(
            _AdditionalObjectsInstructionsMarkerStr
        );
        static readonly ProfilerMarker _changedComponentsIncludingDependenciesMarker = new ProfilerMarker(
            "ChangedComponentsIncludingDependencies"
        );
        ...
        static ProfilerMarker _parentWithChildrenOrderChangedMarker = new ProfilerMarker(
            "ParentWithChildrenOrderChanged"
        );
        ...
        public IncrementalBakeInstructions BuildInitialInstructions(
            Scene scene,
            GameObject[] sceneRoots,
            ref TransformAuthoringBaking transformAuthoringBaking
        )
        {
            ...
            // Flatten full list of game objects we need to create entities for
            {
                ...
                {
                    var additionalGameObjects = root.GetComponentsInChildren<Transform>(true)
                        .Select(transform =>
                        {
            ...
            foreach (var gameObject in _BakeInstructionsCache.CreatedGameObjects)
            {
                ...
                foreach (var com in _ComponentCache)
                {
                    ...
                    _Components.UpdateGameObject(
                        gameObject,
                        _ComponentCache,
                        _ComponentAddedCache,
                        _ComponentsExistingCache,
                        ref revertComponents
                    );
                    // Debug.Log($"BuildInitialInstructions - Bake: {gameObject.GetEntityId()} ({gameObject.name}) Component: {com.GetEntityId()} ({com})");

                    _BakeInstructionsCache.BakeComponents.Add(
                        new IncrementalBakeInstructions.BakeComponent
                        {
                            GameObjectEntityId = gameObject.GetEntityId(),
                            Component = com,
                            ComponentID = com.GetEntityId(),
                        }
                    );
                }
            }
        ...
        public void CollectGameObjectsWithTransformChanged(
            ref IncrementalBakingBatch batch,
        ...
        public IncrementalBakeInstructions BuildIncrementalInstructions(
            IncrementalBakingBatch batch,
            ref TransformAuthoringBaking transformAuthoringBaking,
            bool assetsChanged
        )
        {
            ...
            _BakeInstructionsCache.Clear();
            var changedAuthoringObjectsIncludingDependencies = new UnsafeParallelHashSet<EntityId>(
                1024,
                Allocator.TempJob
            );
            ...
            var prepareJobHandle = transformAuthoringBaking.Prepare(
                _Hierarchy,
                _BakeInstructionsCache.ChangedTransforms
            );

            _BakeInstructionsCache.BakeGameObjects.UnionWith(
                _IncrementalBakingDataCache.ChangedGameObjects.Select(x => x.gameObject)
            );
            ...
            {
                bool recreateEntity = (
                    changedGameObject.mode == IncrementalBakingData.ChangedGameObjectMode.RecreateAll
                );
                bool isForced =
                    recreateEntity || (changedGameObject.mode == IncrementalBakingData.ChangedGameObjectMode.ForceBake);
                ...
                else
                {
                    ...
                    _IncrementalBakingDataCache.ChangedGameObjectProperties.Add(
                        IncrementalBakingData.GameObjectProperties.CalculateProperties(gameObject)
                    );
                }
                ...
                foreach (var com in _ComponentAddedCache)
                {
                    ...
                    IncrementalBakingLog.RecordComponentBake(
                        com.GetEntityId(),
                        ComponentBakeReason.NewComponent,
                        com.GetEntityId(),
                        TypeManager.GetTypeIndex(com.GetType())
                    );
                }
                ...
                if (isForced)
                {
                    foreach (var com in _ComponentsExistingCache)
                    {
                        ...
                        IncrementalBakingLog.RecordComponentBake(
                            comId,
                            ComponentBakeReason.UpdatePrefabInstance,
                            goEntityId,
                            TypeManager.GetTypeIndex(com.GetType())
                        );
                    }
            ...
            _Dependencies.CalculateDependencies(
                ref _Components,
                ref _IncrementalBakingDataCache,
                changedSceneTransforms,
                ref changedAuthoringObjectsIncludingDependencies,
                prepareJobHandle,
                assetsChanged
            );
            ...
            foreach (var componentID in changedAuthoringObjectsIncludingDependencies)
            {
                ...
                // Add for bake
                var bake = new IncrementalBakeInstructions.BakeComponent
                {
                    GameObjectEntityId = gameObjectID,
                    Component = component,
        ...
        public unsafe IncrementalBakeInstructions BuildAdditionalInstructions(
            NativeArray<EntityId> additionalObjects,
            NativeArray<EntityId> destroyedPrefabs,
            ref TransformAuthoringBaking transformAuthoringBaking
        )
        {
        ...
        unsafe struct RemoveFromHierarchy
        {
            ...
            public static void Execute(void* ptr)
            {
                ...
                int capacity =
                    data.DeletedInstanceIds.Length
                    + data.BakeHierarchyInstanceIds.Length
                    + data.ForceBakeHierarchyInstanceIds.Length;
                var deletedInstances = new NativeParallelHashSet<EntityId>(capacity, Allocator.TempJob);
                {
                    ...
                    data.Hierarchy.AsReadOnly()
                        .CollectHierarchyInstanceIds(data.BakeHierarchyInstanceIds, deletedInstances);
                    data.Hierarchy.AsReadOnly()
                        .CollectHierarchyInstanceIds(data.ForceBakeHierarchyInstanceIds, deletedInstances);
        ...
        void UpdateHierarchy(IncrementalBakingBatch batch, ref IncrementalBakingData outData)
        {
            using (var m = _updateHierarchyMarker.Auto())
            {
                ...
                if (!batch.ParentChangeEntityIds.IsEmpty)
                {
                    using (_parentChangeHierarchyMarker.Auto())
                    {
                        ...
                        using (var changeFailed = new NativeList<EntityId>(Allocator.TempJob))
                        {
                            ...
                            IncrementalHierarchyFunctions.ChangeParents(
                                _Hierarchy,
                                parentChanges,
                                changeFailed,
                                changeSuccessful
                            ...
                            using (var visitedInstances = new NativeParallelHashSet<EntityId>(0, Allocator.TempJob))
                            {
                                _Hierarchy
                                    .AsReadOnly()
                                    .CollectHierarchyInstanceIds(changeFailed.AsArray(), visitedInstances);
                        ...
                        // Consider all the transform when reparenting as modified components
                        {
                            if (!outData.ParentChangeInstanceIds.IsEmpty)
                            {
                                ...
                                NativeArray<EntityId> transformIDs = new NativeArray<EntityId>(
                                    outData.ParentChangeInstanceIds.Length,
                                    Allocator.Temp
                                );
                                ...
                                foreach (var obj in objs)
                                {
                                    ...
                                    if (go != null)
                                    {
                                        ...
                                        outData.ChangedComponents.Add(
                                            new IncrementalBakingData.ChangedComponentsInfo()
                                            {
                ...
                if (
                    hasExplicitDeletions
                    || batch.BakeHierarchyEntityIds.Length != 0
                    || batch.ForceBakeHierarchyEntityIds.Length != 0
                )
                {
                ...
                if (!requestHierarchyBake.IsEmpty)
                {
                    ...
                    CollectAllGameObjects(
                        _Scene,
                        ref _Hierarchy,
                        requestHierarchyBake.AsArray(),
                        cleanConversionGameObjects
                    );
                ...
                if (batch.ForceBakeHierarchyEntityIds.Length != 0)
                {
                    ...
                    CollectAllGameObjects(
                        _Scene,
                        ref _Hierarchy,
                        batch.ForceBakeHierarchyEntityIds,
                        cleanConversionGameObjects
                    );
                    using (_registerNewInstancesMarker.Auto())
                    {
                        ...
                        foreach (var go in cleanConversionGameObjects)
                        {
                            if (go != null)
                            {
                                UnityEngine.Debug.LogWarning(
                                    $"The primary entity for the GameObject {go.name} was deleted in a previous baking pass. This forces to rebake the whole GameObject. Consider using BakingOnlyEntityAuthoring instead."
                                );

                                IncrementalHierarchyFunctions.TryAddSingle(_Hierarchy, go);
                                outData.ChangedGameObjects.Add(
                                    (go, IncrementalBakingData.ChangedGameObjectMode.RecreateAll)
                                );
                            }
                        }
                    }
                    ...
                    using (_handleChangedMarker.Auto())
                    {
                        ...
                        if (!hasExplicitDeletions)
                        {
                            ...
                            for (int i = 0; i < objs.Count; i++)
                            {
                                ...
                                outData.ChangedGameObjects.Add(
                                    (obj, IncrementalBakingData.ChangedGameObjectMode.Normal)
                                );

                                IncrementalHierarchyFunctions.UpdateActiveAndStaticState(
                                    _Hierarchy,
                                    batch.ChangedEntityIds[i],
                                    obj.activeSelf,
                                    obj.isStatic
                                );
                            }
        ...
        */
        static void CollectAllGameObjects(
            Scene scene,
            ref IncrementalHierarchy hierarchy,
            NativeArray<EntityId> reconvertedObjects,
            HashSet<GameObject> outputObjects
        )
        {
            ...
            for (int i = 0; i < objs.Count; i++)
            {
                ...
                if (
                    ...
                    && !hierarchy.IndexByEntityId.ContainsKey(obj.transform.parent.gameObject.GetEntityId())
                )
                    continue;
        ...
        static void CollectGameObjects(
            Scene scene,
            NativeArray<EntityId> reconvertedObjects,
            HashSet<GameObject> outputObjects
        )
