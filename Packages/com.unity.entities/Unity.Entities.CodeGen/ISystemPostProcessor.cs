...
namespace Unity.Entities.CodeGen
{
    ...
    {
        private static readonly string RegisterGenericSystemTypeAttributeName =
            typeof(RegisterGenericSystemTypeAttribute).FullName;
        ...
        protected override bool PostProcessUnmanagedImpl(TypeDefinition[] unmanagedComponentSystemTypes)
        {
            ...
            _registrationClassDef = new TypeDefinition(
                "",
                autoClassName,
            ...
            _registrationClassDef.CustomAttributes.Add(
                new CustomAttribute(mod.ImportReference(burstCompileAttributeConstructor))
            );
            _registrationClassDef.CustomAttributes.Add(
                new CustomAttribute(mod.ImportReference(runnerOfMe._preserveAttributeCtorDef))
            );
        ...
        static readonly string[] MethodNames = new string[]
        {
            "OnCreate",
            "OnUpdate",
            "OnDestroy",
        ...
        private TypeMemo AddStaticForwarders(
            TypeDefinition systemType,
            TypeReference specializedSystemType = null,
            TypeDefinition alternateTypeToInsertForwarders = null
        )
        {
            var mod = AssemblyDefinition.MainModule;
            var intPtrRef = mod.ImportReference(typeof(IntPtr));
            var intPtrToVoid = mod.ImportReference(
                intPtrRef.Resolve().Methods.FirstOrDefault(x => x.Name == nameof(IntPtr.ToPointer))
            );
            ...
            var hasStartStop = systemType.Interfaces.Any(
                (x) => x.InterfaceType.FullName == "Unity.Entities.ISystemStartStop"
            );
            var hasCompilerGenerated = systemType.Interfaces.Any(
                (x) => x.InterfaceType.FullName == "Unity.Entities.ISystemCompilerGenerated"
            );
            ...
            for (int i = 0; i < MethodNames.Length; ++i)
            {
                ...
                _methodDef = new MethodDefinition(
                    GeneratedPrefix + wrapperName,
                ...
                _targetMethodDef = systemType.Methods.FirstOrDefault(x =>
                    x.Parameters.Count == 1 && (x.Name == name || x.Name == fullName)
                );
                ...
                {
                    _targetMethodRef = mod.ImportReference(
                        _targetMethodRef.MakeGenericHostMethod(LaunderTypeRef(specializedSystemType))
                    );
                }
                ...
                Func<CustomAttribute, bool> isBurstAttribute = x =>
                ...
                if (burstAttribute != null)
                {
                    if (!_targetMethodDef.DeclaringType.CustomAttributes.Any(isBurstAttribute))
                        _targetMethodDef.DeclaringType.CustomAttributes.Add(
                            new CustomAttribute(
                                mod.ImportReference(typeof(BurstCompileAttribute).GetConstructor(Type.EmptyTypes))
                            )
                        );

                    _methodDef.CustomAttributes.Add(
                        new CustomAttribute(mod.ImportReference(burstAttribute.Constructor), burstAttribute.GetBlob())
                    );
                    memo.m_BurstCompileBits |= 1 << i;
                }

                // Adding MonoPInvokeCallbackAttribute needed for IL2CPP to work when burst is disabled
                var monoPInvokeCallbackAttributeConstructors = typeof(MonoPInvokeCallbackAttribute).GetConstructors(
        ...
        private void AddRegistrationCode(List<TypeMemo> memos)
        {
            ...
            if (!Defines.Contains("UNITY_EDITOR"))
            {
                ...
                var attributeCtor = mod.ImportReference(
            ...
            else
            {
                ...
                var attributeCtor2 = AssemblyDefinition.MainModule.ImportReference(
            ...
            var addMethod = mod.ImportReference(
                registryType.Methods.FirstOrDefault((x) => x.Name == nameof(SystemBaseRegistry.AddUnmanagedSystemType))
            );
            var delegateCtor = mod.ImportReference(
                registryType
                    .NestedTypes.FirstOrDefault((x) => x.Name == nameof(SystemBaseRegistry.ForwardingFunc))
                    .GetConstructors()
                    .FirstOrDefault((x) => x.Parameters.Count == 2)
            );
            var genericHashFunc = mod.ImportReference(typeof(BurstRuntime))
                .Resolve()
                .Methods.FirstOrDefault((x) => x.Name == nameof(BurstRuntime.GetHashCode64) && x.HasGenericParameters);
            var typeType = mod.ImportReference(typeof(Type)).Resolve();
            var getTypeFromHandle = mod.ImportReference(
                typeType.Methods.FirstOrDefault((x) => x.Name == "GetTypeFromHandle")
            );
            ...
            foreach (var memo in memos)
            {
                ...
                processor.Emit(
                    OpCodes.Call,
                    mod.ImportReference(
                        genericHashFunc.MakeGenericInstanceMethod(mod.ImportReference(memo.m_SystemType))
                    )
                );
