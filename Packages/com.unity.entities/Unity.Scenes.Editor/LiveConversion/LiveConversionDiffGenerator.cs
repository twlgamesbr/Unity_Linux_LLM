...
namespace Unity.Scenes.Editor
{
    ...
    class LiveConversionDiffGenerator : IDisposable
    {
        ...
        {
            var didChange =
                _RequestCleanConversion
                || _IncrementalBakingChangeTracker.HasAnyChanges()
                || _ArtifactDependencyVersion != artifactDependencyVersion;
        ...
        LiveConversionDiffGenerator(
            ...
            Hash128 sceneGUID,
            GUID configGUID,
            IEntitiesPlayerSettings settingAsset,
        ...
        )
        {
            ...
            ConstructConversionWorld(
                $"Converted Scene: '{_Scene.name}'",
                ref _ConvertedWorld,
                ref _MissingRenderDataQuery,
                ref _MissingSceneQuery
            );
            ...
            var queryDesc = new EntityQueryDesc
            {
                ...
                None = new[]
                {
                    ComponentType.ReadOnly<RemoveUnusedEntityInBake>(),
                    ComponentType.ReadOnly<BakingOnlyEntity>(),
                },
                Options = EntityQueryOptions.IncludeDisabledEntities | EntityQueryOptions.IncludePrefab,
            };

            _LiveConversionDiffer = new EntityManagerDiffer(
        ...
        void ConstructConversionWorld(
            string name,
            ref World world,
            ref EntityQuery missingRenderDataQuery,
            ref EntityQuery missingSceneQuery
        )
        {
        ...
        void LazyCreateIncrementalConversionDebug()
        {
            if (_IncrementalConversionDebug.World == null)
                ConstructConversionWorld(
                    $"Incremental Conversion Debug: '{_Scene.name}'",
                    ref _IncrementalConversionDebug.World,
                    ref _IncrementalConversionDebug.MissingRenderDataQuery,
                    ref _IncrementalConversionDebug.MissingSceneQuery
                );
        }
        ...
        static readonly ProfilerMarker BlobAssetStoreGarbageCollection = new ProfilerMarker(
            "BlobAssetStoreGarbageCollection"
        );
        ...
        bool Bake(BakingUtility.BakingFlags flags)
        {
            using (LiveConversionConvertMarker.Auto())
            {
                ...
                var didBake = BakingUtility.BakeScene(
                    _ConvertedWorld,
                    _Scene,
                    conversionSettings,
                    !_RequestCleanConversion,
                    _IncrementalBakingChangeTracker
                );
                if (didBake)
        ...
        BakingSettings GetBakeSettings(
            BakingUtility.BakingFlags flags,
        ...
        internal void DebugIncrementalConversion()
        {
            ...
            using (DebugConversionMarker.Auto())
            {
                ...
                AddMissingData(
                    _IncrementalConversionDebug.World,
                    _IncrementalConversionDebug.MissingSceneQuery,
                ...
                EntityDiffer.PrecomputeBlobAssetCache(
                    _ConvertedWorld.EntityManager,
                ...
                using (changes)
                {
...
#if !UNITY_DISABLE_MANAGED_COMPONENTS
                    {
                        ...
                        for (int i = lastComponentRemoved; i >= 0; i--)
                        {
                            ...
                            var idx = TypeManager.GetTypeIndexFromStableTypeHash(types[packedTypeIdx].StableTypeHash);
                            if (
                                idx == companionLinkIndex
                                || idx == companionLinkTransformIndex
                                || idx == companionReferenceIndex
                            )
                            {
                                componentsRemoved[i] = componentsRemoved[
                                    lastComponentRemoved - numCompanionLinkComponentsRemoved
                                ];
                        ...
                        if (
                            numCompanionLinkObjects > 0
                            || numCompanionComponentsSet > 0
                            || numCompanionLinkComponentsRemoved > 0
                        )
                        {
                            // throw away the companion link changes
                            Array.Resize(ref managedComponents, lastManaged + 1 - numCompanionLinkObjects);
                            var setComponentsWithoutCompanions = setComponents.GetSubArray(
                                0,
                                lastComponentSet + 1 - numCompanionComponentsSet
                            );
                            var removeComponentsWithoutCompanions = componentsRemoved.GetSubArray(
                                0,
                                lastComponentRemoved + 1 - numCompanionLinkComponentsRemoved
                            );
                            fwdChanges = new EntityChangeSet(
                                fwdChanges.CreatedEntityCount,
        ...
        static void AddMissingData(
            World world,
            EntityQuery missingSceneQuery,
            EntityQuery missingRenderDataQuery,
            BakingUtility.BakingFlags flags
        )
        {
            ...
            if ((flags & BakingUtility.BakingFlags.SceneViewLiveConversion) == 0)
            {
                ...
                em.AddSharedComponentManaged(
                    missingRenderDataQuery,
                    new EditorRenderData { SceneCullingMask = cullingMask }
                );
            }
        }

        public static bool UpdateLiveConversion(
            Scene scene,
            Hash128 sceneGUID,
            ref LiveConversionDiffGenerator liveConversionData,
            LiveConversionMode mode,
            ulong globalAsssetDependencyVersion,
            GUID configGUID,
            IEntitiesPlayerSettings settingAsset,
            out LiveConversionChangeSet changes
        )
        {
            ...
            if (liveConversionData == null)
                liveConversionData = new LiveConversionDiffGenerator(
                    scene,
                    sceneGUID,
                    configGUID,
                    settingAsset,
            ...
            if (didBake)
            {
                ...
                changes = new LiveConversionChangeSet
                {
                    Changes = liveConversionData
                        ._LiveConversionDiffer.GetChanges(options, Allocator.TempJob)
                        .ForwardChangeSet,
                    UnloadAllPreviousEntities = unloadAllPreviousEntities,
                    SceneName = scene.name,
                    SceneGUID = sceneGUID,
