...
namespace NPCSystem
{
    ...
    public sealed class NPCTransferableItemSpawner : MonoBehaviour
    {
        ...
        [HelpBox("Spawns a single testable network item for ownership-transfer validation. The item starts on the configured NPC, can be picked up by a player with Interact, given to the nearest player with Previous (1), and returned to the nearest NPC with Next (2).", MessageMode.Log, drawAbove: true)]
        ...
        void ResolveReferences()
        {
            ...
            initialNpcSlug = string.IsNullOrWhiteSpace(initialNpcSlug) ? "butler" : initialNpcSlug.Trim().ToLowerInvariant();
        }
        ...
        public void SpawnTestItem()
        {
            ...
            {
                lastSpawnStatus = "Transferable item spawn skipped because this instance is not the server.";
                return;
            }

            if (itemPrefab == null)
            {
                lastSpawnStatus = $"Transferable item prefab could not be loaded from Resources/{itemPrefabResourcesPath}.";
                return;
            }
            ...
            GameObject instance = Instantiate(itemPrefab, fallbackWorldPosition, Quaternion.identity);
            ...
            if (item != null)
            {
                ...
                else
                {
                    ...
                    lastSpawnStatus = "Spawned transferable item in fallback world position because the configured NPC was not found.";
                }
            }
