...
namespace NPCSystem
{
    ...
    public sealed class NPCTransferableItem : NetworkBehaviour
    {
        ...
        void Awake()
        {
            ...
            displayName = string.IsNullOrWhiteSpace(displayName) ? "Evidence Ledger" : displayName.Trim();
        ...
        public void AssignToPlayer(ulong clientId)
        {
            ...
            LogOwnershipEvent(NPCFlowStatus.Success, $"Assigned item '{DisplayName}' to player client {clientId}.", clientId, string.Empty);
        }
        ...
        public void AssignToNpc(string npcSlug)
        {
            ...
            LogOwnershipEvent(NPCFlowStatus.Success, $"Assigned item '{DisplayName}' to NPC '{normalizedNpcSlug}'.", NetworkManager.ServerClientId, normalizedNpcSlug);
        }
        ...
        public void PlaceInWorld(Vector3 worldPosition)
        {
            ...
            LogOwnershipEvent(NPCFlowStatus.Success, $"Placed item '{DisplayName}' in the world.", NetworkManager.ServerClientId, string.Empty);
        }
        ...
        {
            string holder = IsHeldByNpc
                ? _npcHolderSlug.Value.ToString()
                : IsHeldByPlayer
