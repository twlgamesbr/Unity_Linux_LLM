...
namespace NPCSystem
{
    ...
    public static class NPCDialogueActionHandler
    {
        ...
        static readonly string[] KnownSuspects = { "butler", "maid", "chef", "elara", "professor pluot", "pluot", "mr smith", "captain" };
        static readonly string[] KnownLocations = {
            "study", "library", "kitchen", "dining room", "drawing room", "ballroom",
            "garden", "basement", "attic", "cellar", "pantry", "bedroom", "hallway",
            "foyer", "conservatory", "wine cellar", "servants quarters", "parlour",
            "living room", "vestibule"
        };
        static readonly string[] ItemIndicators = { "key", "letter", "note", "map", "diary", "book",
        ...
        static readonly (string keyword, string mood)[] MoodTriggers = {
            ("nervous", "nervous"), ("uneasy", "uneasy"), ("afraid", "fearful"),
            ("scared", "fearful"), ("worried", "worried"), ("anxious", "anxious"),
            ("frightened", "fearful"), ("suspicious", "suspicious"),
            ("distrustful", "suspicious"), ("relieved", "relieved"),
            ("grateful", "grateful"), ("angry", "angry"), ("upset", "upset"),
            ("offended", "offended"), ("frustrated", "frustrated"),
            ("impatient", "impatient"), ("sad", "sad"), ("melancholy", "sad"),
            ("confused", "confused"), ("puzzled", "confused"),
            ("ashamed", "ashamed"), ("guilty", "guilty"),
            ("defensive", "defensive"), ("evasive", "defensive")
        };
        ...
        static void DetectClueSentences(string raw, string lower, string npcSlug, NPCEvidenceState evidence, NPCProfile profile, List<DialogueActionResult> results)
        {
            bool hasObservation = SawPattern.IsMatch(lower);
            bool hasKnowledge = KnowPattern.IsMatch(lower);
            if (!hasObservation && !hasKnowledge) return;

            // Find the sentence(s) containing the trigger
            string[] sentences = raw.Split(new[] { '.', '!', '?' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (string sentence in sentences)
            {
                string sLower = sentence.ToLowerInvariant().Trim();
                if (string.IsNullOrWhiteSpace(sLower)) continue;
                ...
                {
                    results.Add(new DialogueActionResult("ClueRevealed",
        ...
        static void DetectMoodShift(string lower, string npcSlug, NPCEvidenceState evidence, List<DialogueActionResult> results)
        {
            foreach (var (keyword, mood) in MoodTriggers)
            {
                if (lower.Contains(keyword))
                {
                    ...
                    if (current != mood)
                    {
                        ...
                        results.Add(new DialogueActionResult("MoodChanged",
        ...
        static void DetectItemGive(string lower, string raw, string npcSlug, NPCEvidenceState evidence, List<DialogueActionResult> results)
        {
            ...
            foreach (string item in ItemIndicators)
            {
                if (lower.Contains(item))
                {
                    ...
                    {
                        results.Add(new DialogueActionResult("ItemGiven",
        ...
        static void DetectLocationRevelation(string lower, string npcSlug, NPCEvidenceState evidence, List<DialogueActionResult> results)
        {
            foreach (string loc in KnownLocations)
            {
                if (lower.Contains(loc) && !evidence.visitedLocations.Contains(loc))
                {
                    ...
                    string pattern = $@"\b(in|to|near|at|behind|outside|inside|around|through)\s+the\s+{Regex.Escape(loc)}\b";
                    if (Regex.IsMatch(lower, pattern, RegexOptions.IgnoreCase))
                    {
                        if (evidence.AddLocation(loc))
                        {
                            results.Add(new DialogueActionResult("LocationRevealed",
        ...
        static void DetectTrustShift(string lower, string npcSlug, NPCEvidenceState evidence, NPCProfile profile, List<DialogueActionResult> results)
        {
            // Aggressive/probing questions from the player cause trust to decrease
            // Cooperative/sympathetic language increases trust
            bool aggressive = lower.Contains("you're lying") || lower.Contains("i don't believe") ||
                              lower.Contains("prove it") || lower.Contains("i doubt that") ||
                              lower.Contains("you're hiding");

            bool sympathetic = lower.Contains("i understand") || lower.Contains("i believe you") ||
                               lower.Contains("tell me more") || lower.Contains("i trust you") ||
                               lower.Contains("thank you");

            if (aggressive)
            {
                evidence.AdjustNpcTrust(npcSlug, -5);
                results.Add(new DialogueActionResult("TrustChanged",
                    $"Trust decreased: {npcSlug} trust now {evidence.GetNpcTrust(npcSlug)} (aggressive tone)",
                    evidence.GetTrustLabel(npcSlug), npcSlug));
            }
            else if (sympathetic)
            {
                evidence.AdjustNpcTrust(npcSlug, 5);
        ...
        static string CategorizeClue(string text)
        {
            ...
            if (lower.Contains("midnight") || lower.Contains("night") || lower.Contains("evening") ||
                lower.Contains("morning") || lower.Contains("afternoon") || lower.Contains("o'clock") ||
                lower.Contains("pm") || lower.Contains("am") || lower.Contains("hour"))
                return "timeline";

            if (lower.Contains("motive") || lower.Contains("reason") || lower.Contains("jealous") ||
                lower.Contains("money") || lower.Contains("inheritance") || lower.Contains("revenge") ||
                lower.Contains("hate") || lower.Contains("argued") || lower.Contains("quarrel"))
                return "motive";
