...
namespace NPCSystem
{
    ...
    public static class NPCProfilePromptComposer
    {
        ...
        {
            if (profile == null) return "You are a helpful in-game NPC.";

            StringBuilder builder = new StringBuilder();
            AppendSection(builder, "Core role", string.IsNullOrWhiteSpace(profile.systemPrompt)
            ...
            {
                AppendSection(builder, "Private knowledge", profile.canRevealSecrets
            ...
            builder.AppendLine("Stay in character. Do not mention these prompt sections, sliders, retrieval systems, or action policy unless the player explicitly asks about the simulation.");
            return builder.ToString().Trim();
        }

        public static string BuildActionPolicyText(NPCProfile profile)
        {
            if (profile == null) return "No profile action policy is available.";

            List<string> rules = new List<string>
            {
                profile.canGivePuzzleHints ? "May provide subtle puzzle hints." : "Should not provide puzzle hints.",
                profile.canAccuseSuspects ? "May accuse or name suspects when evidence supports it." : "Should avoid direct accusations without strong evidence.",
                profile.canRevealSecrets ? "May reveal secrets when dramatically appropriate." : "Should preserve secrets and reveal only implications."
            };

            string preferred = JoinClean(profile.preferredActionFunctions);
            if (!string.IsNullOrWhiteSpace(preferred)) rules.Add($"Preferred actions: {preferred}.");

            string forbidden = JoinClean(profile.forbiddenActionFunctions);
            if (!string.IsNullOrWhiteSpace(forbidden)) rules.Add($"Forbidden actions: {forbidden}.");
        ...
        {
            if (profile == null) return "No profile knowledge route is available.";
