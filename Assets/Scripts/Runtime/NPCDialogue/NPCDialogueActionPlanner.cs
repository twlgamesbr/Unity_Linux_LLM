...
namespace NPCSystem
{
    ...
    public sealed class NPCDialogueActionPlan
    {
        ...
        {
            return new NPCDialogueActionPlan { actionType = NPCDialogueActionType.None, reason = reason, contextPrompt = string.Empty };
        }
    }

    public class NPCDialogueActionPlanner : MonoBehaviour
    {
        static readonly string[] HintKeywords = { "hint", "stuck", "investigate", "clue", "what should i do", "next" };
        ...
        static readonly string[] EvidenceKeywords = { "remember", "evidence", "suspect", "proof", "alibi" };

        public NPCDialogueActionPlan Plan(string playerMessage, NPCProfile profile)
        {
            if (string.IsNullOrWhiteSpace(playerMessage)) return NPCDialogueActionPlan.None("Player message was empty.");

            string lower = playerMessage.Trim().ToLowerInvariant();
            if (Matches(lower, NotesKeywords))
            {
                return BuildPlan(NPCDialogueActionType.ShowNotes, "Player referenced notes/notebook.",
            ...
            {
                return BuildPlan(NPCDialogueActionType.ShowMap, "Player referenced map/location.",
                    "If useful, anchor the reply in mansion geography, rooms, or where the player should look next.");
            }

            if (Matches(lower, SolveKeywords))
            {
                return BuildPlan(NPCDialogueActionType.ShowSolve, "Player asked about solving/accusing.",
            ...
            {
                return BuildPlan(NPCDialogueActionType.ShowHelp, "Player asked for help.",
            ...
            {
                return BuildPlan(NPCDialogueActionType.PuzzleHint, "Player seems stuck and this NPC may give hints.",
            ...
            {
                return BuildPlan(NPCDialogueActionType.RecallEvidence, "Player asked about memory/evidence/suspects.",
                    "Summarize the strongest known clues, uncertainties, and contradictions from this NPC's perspective.");
            }

            if (profile != null && profile.canAccuseSuspects && (lower.Contains("who") || lower.Contains("suspect") || lower.Contains("culprit")))
            {
                return BuildPlan(NPCDialogueActionType.PressSuspect, "Player asked for suspect judgment and NPC may accuse suspects.",
        ...
        {
            if (plan == null || plan.actionType == NPCDialogueActionType.None || string.IsNullOrWhiteSpace(plan.contextPrompt)) return string.Empty;
            return $"Action guidance ({plan.actionType}): {plan.contextPrompt}";
        }

        static NPCDialogueActionPlan BuildPlan(NPCDialogueActionType actionType, string reason, string contextPrompt)
        {
