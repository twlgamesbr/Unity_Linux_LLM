...
namespace NPCSystem
{
    ...
    public static class NPCRAGMetadataStore
    {
            ...
            int chunkCount = 0)
        {
            ...
            if (profiles != null)
            {
                foreach (NPCProfile profile in profiles)
                {
                    ...
                    string sourcePath = NPCRAGImporter.ResolveStreamingAssetPath(profile.GetKnowledgeSourcePath());
        ...
        public static bool TryLoad(string ragEmbeddingPath, out NPCRAGMetadata metadata)
        {
            ...
            {
                NPCFlowLogger.FindOrCreate().Log(NPCFlowStage.LocalRagReady, NPCFlowStatus.Error, NPCFlowLogLevel.Warning,
        ...
        public static bool IsCurrent(NPCRAGMetadata actual, NPCRAGMetadata expected, out string reason)
        {
            reason = string.Empty;
            if (actual == null)
            {
            ...
            if (!string.Equals(actual.importerVersion, expected.importerVersion, StringComparison.Ordinal))
            {
                reason = $"importer version changed ({actual.importerVersion} != {expected.importerVersion})";
                return false;
            }

            if (!string.Equals(actual.embeddingModel, expected.embeddingModel, StringComparison.Ordinal))
            {
                reason = $"embedding model changed ({actual.embeddingModel} != {expected.embeddingModel})";
                return false;
            }

            if (actual.embeddingLength != expected.embeddingLength)
            {
                reason = $"embedding length changed ({actual.embeddingLength} != {expected.embeddingLength})";
                return false;
            }

            if (actual.chunkCharacters != expected.chunkCharacters)
            {
                reason = $"chunk size changed ({actual.chunkCharacters} != {expected.chunkCharacters})";
                return false;
            }
            ...
            Dictionary<string, NPCRAGSourceMetadata> actualByPath = new Dictionary<string, NPCRAGSourceMetadata>(StringComparer.Ordinal);
            foreach (NPCRAGSourceMetadata source in actual.sources)
            {
                if (source != null) actualByPath[source.sourcePath] = source;
            }

            foreach (NPCRAGSourceMetadata expectedSource in expected.sources)
            {
                if (expectedSource == null) continue;
                if (!actualByPath.TryGetValue(expectedSource.sourcePath, out NPCRAGSourceMetadata actualSource))
                {
                    reason = $"source file missing from metadata: {expectedSource.sourcePath}";
                    return false;
                }

                if (!string.Equals(actualSource.sha256, expectedSource.sha256, StringComparison.Ordinal))
                {
