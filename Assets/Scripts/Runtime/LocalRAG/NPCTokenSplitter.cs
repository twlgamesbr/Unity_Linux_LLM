...
namespace NPCSystem
{
    ...
    public class NPCTokenSplitter : NPCChunking
    {
        ...
        protected int DetermineEndIndex(string input, string detokenised, int startIndex,
            int searchRange = 5, int charsFromEnd = 3)
        {
        ...
        public override async Task<List<(int, int)>> Split(string input)
        {
            ...
            if (startIndex <= input.Length - 1)
                indices.Add((startIndex, input.Length - 1));
            return indices;
        }
    }
}
