...
namespace FlyingWormConsole3.LiteNetLib.Utils
{
    ...
    public class NetSerializer
    {
        ...
        private abstract class FastCallSpecific<TClass, TProperty> : FastCall<TClass>
        {
            ...
            public override void Init(MethodInfo getMethod, MethodInfo setMethod, CallType type)
            {
                ...
                switch (type)
                {
                    case CallType.Array:
                        GetterArr =
                            (Func<TClass, TProperty[]>)
                                Delegate.CreateDelegate(typeof(Func<TClass, TProperty[]>), getMethod);
                        SetterArr =
                            (Action<TClass, TProperty[]>)
                                Delegate.CreateDelegate(typeof(Action<TClass, TProperty[]>), setMethod);
                        break;
                    case CallType.List:
                        GetterList =
                            (Func<TClass, List<TProperty>>)
                                Delegate.CreateDelegate(typeof(Func<TClass, List<TProperty>>), getMethod);
                        SetterList =
                            (Action<TClass, List<TProperty>>)
                                Delegate.CreateDelegate(typeof(Action<TClass, List<TProperty>>), setMethod);
                        break;
                    default:
                        Getter =
                            (Func<TClass, TProperty>)
        ...
        private sealed class FastCallStruct<TClass, TProperty> : FastCallSpecific<TClass, TProperty>
            where TProperty : struct, INetSerializable
        {
        ...
        private sealed class FastCallClass<TClass, TProperty> : FastCallSpecific<TClass, TProperty>
            where TProperty : class, INetSerializable
        {
        ...
        private class StringSerializer<T> : FastCallSpecific<T, string>
        {
            ...
            {
                _maxLength = maxLength > 0 ? maxLength : short.MaxValue;
            }
        ...
        private sealed class CustomTypeStruct<TProperty> : CustomType
            where TProperty : struct, INetSerializable
        {
        ...
        private sealed class CustomTypeClass<TProperty> : CustomType
            where TProperty : class, INetSerializable
        {
            private readonly Func<TProperty> _constructor;
        ...
        public bool Deserialize<
            ...
            [DynamicallyAccessedMembers(Trimming.SerializerMemberTypes)]
#endif
            T>(NetDataReader reader, T target)
            where T : class, new()
        {
            var info = RegisterInternal<T>();
            try
